...
serve(async (req) => {
  ...
  try {
    ...
    const { task, payload, history, drawName } = body

    // Si un drawName est fourni sans task spécifique, on exécute l'ancienne logique (rétrocompatibilité)
    if (drawName && !task) {
    ...
    // Nouvelle logique : Exécution des tâches mathématiques lourdes
    if (!task) {
