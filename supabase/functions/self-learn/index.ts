...
// Fix: declare Deno global for Edge Function environment
...
const evaluateFitness = (weights: any, signalMatrix: any) => {
    ...
    for (let i = 1; i <= 90; i++) {
        ...
        if (!sig) continue;
        const nScore = (sig.freq * (weights.frequency || 0.1)) + (sig.isGapMatch ? (weights.gap || 0.2) * 50 : 0) + (sig.markov * (weights.markov || 0.1) * 10) + (sig.momentum * (weights.momentum || 0.05) * 5);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Fix: Using Deno global which is now declared
    ...
    if (!history || history.length < 20) throw new Error("Data insuffisant");
    ...
    for (let i = 1; i <= 90; i++) {
        ...
        signalMatrix[i] = { freq: freq / 45, isGapMatch: lastSeen >= 8 && lastSeen <= 18, markov: 0.1, momentum: history.slice(0, 5).filter(d => d.gagnants.includes(i)).length, wasRecentlyOut: recent.some(d => d.gagnants.includes(i)) };
    }
    ...
    for (let g = 0; g < 30; g++) {
        ...
        while(population.length < 15) population.push({ ...bestW, gap: Math.random() });
    }

    await supabase.from('algo_weights').upsert({ draw_name: drawName, weights: bestW, updated_at: new Date().toISOString() });

    return new Response(JSON.stringify({ success: true, improved: true, weights: bestW }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
