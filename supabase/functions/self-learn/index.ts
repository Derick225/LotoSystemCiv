...
const WEIGHT_KEYS = ['frequency', 'gap', 'spectral', 'markov', 'wavelet', 'momentum', 'equilibrium', 'orchestration', 'anti_consensus', 'machine_transfer'];

const evaluateFitness = (weights: any, signalMatrix: any, targets: number[]) => {
    let score = 0;
    let candidates = [];
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: history } = await supabase.from('draw_results').select('gagnants, machine').eq('draw_name', drawName).order('date', { ascending: false }).limit(60);
    ...
    const context = history.slice(10, 60);
    ...
    for (let i = 1; i <= 90; i++) {
        ...
        const wasInLastMachine = context[0]?.machine?.includes(i) || false;
        
        signalMatrix[i] = { 
            freq: freq / 50, 
            isGapMatch: lastSeen >= 8 && lastSeen <= 20, 
            markov: 0.1, 
            momentum: context.slice(0, 5).filter(d => d.gagnants.includes(i)).length, 
    ...
    let population = Array(15).fill(null).map((_, i) => i === 0 ? bestW : { ...bestW, frequency: Math.max(0.01, (bestW.frequency || 0.1) + (Math.random() - 0.5) * 0.2) });

    for (let g = 0; g < 40; g++) {
        if (Date.now() - start > 8000) break; // Watchdog
        population.sort((a, b) => evaluateFitness(b, signalMatrix, targets) - evaluateFitness(a, signalMatrix, targets));
        bestW = population[0];
        ...
        while(population.length < 15) {
            ...
            const gene = WEIGHT_KEYS[Math.floor(Math.random() * WEIGHT_KEYS.length)];
            const child = { ...p };
            child[gene] = Math.max(0.01, Math.min(1.0, (child[gene] || 0.1) + (Math.random() - 0.5) * 0.2));
