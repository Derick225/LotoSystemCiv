...
type LiveSessionType = Awaited<ReturnType<InstanceType<typeof GoogleGenAI>["live"]["connect"]>>;
...
const toolsDef: { functionDeclarations: FunctionDeclaration[] }[] = [{
    functionDeclarations: [
        ...
        {
            name: "changeDraw",
            description: "Change le tirage actif (ex: passer de 'Reveil' à 'National').",
            parameters: {
                type: Type.OBJECT,
                properties: {
        ...
        },
        {
            ...
            parameters: {
                type: Type.OBJECT,
                properties: {
                    tab: { type: Type.STRING, enum: ["Flux", "Signaux", "Topologie", "Oracle", "Simulation", "Forensic"], description: "L'onglet cible" },
                    subTab: { type: Type.STRING, description: "Sous-onglet optionnel (ex: 'stats', 'spectral')" }
                },
                required: ["tab"]
            }
        }
...
export const OracleLiveAssistant: React.FC<OracleLiveAssistantProps> = ({ drawName }) => {
    ...
    const stopAssistant = useCallback(async () => {
        if (sessionRef.current) {
            try { sessionRef.current.close(); } catch (e) { logger.error(e instanceof Error ? e : new Error(String(e)), 'Silenced error'); }
        ...
        const closeCtx = async (ctx: AudioContext | null) => {
            if (ctx && ctx.state !== 'closed') try { await ctx.close(); } catch (e) { logger.error(e instanceof Error ? e : new Error(String(e)), 'Silenced error'); }
        };
        await Promise.all([closeCtx(inputAudioContext.current), closeCtx(outputAudioContext.current)]);
    ...
    const handleToolCall = async (toolCall: { functionCalls?: { name?: string, args?: Record<string, unknown>, id?: string }[] }) => {
        const responses = [];
        ...
        for (const fc of toolCall.functionCalls) {
            ...
            if (fc.name === 'inspectNumber') {
                ...
                if (num >= 1 && num <= 90) {
                    ...
                    responses.push({ id: fc.id, name: fc.name, response: { result: `Inspecteur ouvert pour ${num}.` } });
                }
                ...
                if (match) {
                    ...
                    responses.push({ id: fc.id, name: fc.name, response: { result: `Context switched to ${match.name}` } });
                } else {
                    responses.push({ id: fc.id, name: fc.name, response: { result: `Draw ${target} not found` } });
                }
    ...
    const drawVisualizer = () => {
        ...
        const draw = () => {
            ...
            ctx.fillStyle = status === 'speaking' ? '#10b981' : status === 'processing' ? '#f59e0b' : '#6366f1';
            ctx.shadowBlur = 20;
    ...
    const startAssistant = async () => {
        ...
        try {
            ...
            const session = await ai.live.connect({
                ...
                callbacks: {
                    onopen: () => { 
                        setIsActive(true); setIsConnecting(false); setStatus('listening');
                    ...
                    onerror: () => { showToast("Perte signal Oracle.", "error"); stopAssistant(); }
                }
            });
                ...
                <div className="bg-slate-950/90 backdrop-blur-2xl border border-indigo-500/50 p-6 rounded-2xl shadow-2xl w-72 animate-slide-up pointer-events-auto relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-white uppercase tracking-widest">NEXUS LIVE</span>
                        </div>
                        <button onClick={stopAssistant} className="text-slate-500 hover:text-white transition bg-white/5 p-1.5 rounded-full hover:bg-rose-500"><X size={14}/></button>
                    </div>
                    ...
                    <div className="h-32 bg-black/50 rounded-3xl overflow-hidden border border-white/5 flex items-center justify-center relative">
                        <canvas ref={canvasRef} width={280} height={128} className="w-full h-full" />
                        <div className="absolute bottom-2 text-xs font-mono font-bold text-slate-500 uppercase tracking-widest">
                            {status === 'listening' ? 'Écoute...' : status === 'speaking' ? 'Transmission...' : status === 'processing' ? 'Calcul...' : 'Prêt'}
                        </div>
                    </div>

                    <div className="mt-4 flex justify-between items-center px-2">
                        <div className="flex items-center gap-2">
                            <Radio size={12} className={status === 'speaking' ? "text-emerald-400 animate-ping" : "text-slate-600"} />
                            <span className="text-xs font-bold text-slate-400 uppercase">Canal Sécurisé</span>
                        </div>
