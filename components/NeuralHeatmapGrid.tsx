...
export const NeuralHeatmapGrid: React.FC<NeuralHeatmapGridProps> = React.memo(({ breakdown, suggestedNumbers }) => {
            ...
            <div className="flex justify-between items-end mb-8 relative z-10 px-2 gap-4">
                <div>
                    <h4 className="text-sm font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                        <Activity className="text-indigo-400" size={18} /> Matrice Tensorielle 1-90
                    </h4>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Séquençage des points chauds par fusion d'algorithmes (Cliquez pour Analyse)</p>
                </div>
            </div>
                ...
                {grid.map((cell) => {
                    ...
                    return (
                        <div 
                            ...
                            `}
                            style={{ 
                                backgroundColor: isSuggested ? undefined : `rgba(79, 70, 229, ${colorIntensity * 0.25})`,
                                borderColor: isSuggested ? undefined : `rgba(99, 102, 241, ${colorIntensity * 0.5})`
                            }}
                        >
                            {cell.num}
                                ...
                                <div className="bg-slate-900/95 backdrop-blur-xl text-white p-4 rounded-2xl shadow-2xl min-w-[160px] border border-indigo-500/50">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-slate-500 uppercase">Probabilité</span>
                                            <span className="text-sm font-black text-white">{Math.round(cell.intensity)}%</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-slate-500 uppercase">Dominance</span>
                                            <span className="text-xs font-bold text-indigo-300">{cell.topAlgo}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="mt-12 flex flex-wrap justify-center gap-8 border-t border-white/5 pt-10 relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-white uppercase tracking-widest leading-none">Top Inférence</span>
                        <span className="text-[10px] text-slate-400 font-bold mt-1">Cible IA Prioritaire</span>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-md bg-indigo-900/30 ring-1 ring-indigo-500/50"></div>
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-white uppercase tracking-widest leading-none">Activité Haute</span>
                        <span className="text-[10px] text-slate-400 font-bold mt-1">Pression Stochastique</span>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 font-bold mt-1">Bruit de fond</span>
                    </div>
                </div>
            </div>
        </div>
