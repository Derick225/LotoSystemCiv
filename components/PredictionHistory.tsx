...
import { getPredictionHistoryAsync, clearPredictionHistory, linkPredictionToResult, deletePrediction, findMatchingResultForPrediction } from '../services/predictionHistoryService';
...
export const PredictionHistory: React.FC<PredictionHistoryProps> = ({ drawName }) => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            logError(new AppError("Echec chargement historique", "FETCH_HISTORY_ERROR", "low", { error: e }));
    ...
    const handleOpenAudit = async (e: React.MouseEvent, result: DrawResult, predictionItem: PredictionHistoryItem) => {
        ...
        const existingReport = await getForensicReportByPredictionId(predictionItem.id);
    ...
    if (loading || nexusLoading) return <div className="p-8 space-y-4"><div className="h-20 bg-slate-100 dark:bg-slate-800 rounded-2xl animate-pulse"></div><div className="h-20 bg-slate-100 dark:bg-slate-800 rounded-2xl animate-pulse"></div></div>;
            ...
            <div className="flex-1 overflow-hidden">
                {history.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 h-[200px]">
                        ...
                        <h4 className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tighter">Aucune Prédiction</h4>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2 text-center max-w-sm">Générez une prédiction via l'Oracle pour la voir apparaître ici et suivre son évolution.</p>
                    </div>
                ) : (
                    <div ref={parentRef} className="h-full overflow-auto rounded-2xl custom-scrollbar" style={{ contain: 'strict' }}>
                        <div
                            style={{
                        ...
                        >
                            {virtualizer.getVirtualItems().map((virtualItem) => {
                                ...
                                const res = item.drawResultId ? getResultById(item.drawResultId) : findMatchingResultForPrediction(item, results);
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-50 dark:divide-slate-700">
                                                    ...
                                                    <div className="flex justify-between items-start mb-6">
                                                        <div>
                                                            ...
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">Confiance {item.prediction.confidence}%</span>
                                                                {item.drawResultId && <span className="flex items-center gap-1 text-[10px] font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-100 dark:border-emerald-500/20 uppercase"><LinkIcon size={8}/> ID-LINKED</span>}
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                ...
                                                <div className={`p-6 md:w-2/5 flex flex-col justify-center ${res ? 'bg-slate-50 dark:bg-slate-800/20' : 'bg-slate-50/50 dark:bg-slate-900/10'}`}>
                                                    {res ? (
                                                        <div className="space-y-4">
                                                            <div className="flex justify-between items-center">
                                                                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Résultat Officiel</span>
                                                                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded ${hits.length >= 2 ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>{hits.length} HITS</span>
                                                            </div>
                                                            ...
                                                            {res.machine && res.machine.length > 0 && (
                                                                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-700/50">
                                                                    ...
                                                                    <div className="flex gap-1">
                                                                        {res.machine.map(n => (
                                                                            <div key={n} className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-900/50 text-[10px] font-black text-slate-500 border border-slate-200 dark:border-slate-700 flex items-center justify-center">{n}</div>
                                                                        ))}
                                                                    </div>
                                                                </div>
                                                            )}
