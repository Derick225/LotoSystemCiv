...
import { analyzeTicketStrength, calculateGeneticDiversityIndex } from '../services/predictionEngine';

import { saveTicket } from '../services/userPreferencesService';
import { useToast } from './ui/Toast';
import type { TicketAnalysisResult } from '../types';
import { Activity, Save, RefreshCw, Layers, Wand2, ThermometerSun, ScanLine } from 'lucide-react';
...
export const PatternSequencer: React.FC<{ drawName: string }> = ({ drawName }) => {
    ...
    const empiricalCalibration = useNexusStore(state => state.empiricalCalibration);
    ...
    const diversityResult = useMemo(() => {
        ...
        return calculateGeneticDiversityIndex(selection, lastPrediction.breakdown as any);
    ...
    useEffect(() => {
        if (selection.length > 0) {
            setMetrics(analyzeTicketStrength(selection, calibration) as TicketAnalysisResult);
        } else {
            setMetrics(null);
        }
    ...
    const handleSave = async () => {
        ...
        const finalScore = metrics ? Math.max(0, Math.min(100, Math.round(metrics.score - penalty))) : 0;
        await saveTicket({
            numbers: selection,
            drawName,
                ...
                <div className="lg:w-80 flex flex-col gap-6">
                    <div className="bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-800 shadow-xl min-h-[300px] flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            {metrics && (() => {
                                const penalty = diversityResult?.penalty || 0;
                                const finalScore = Math.max(0, Math.min(100, Math.round(metrics.score - penalty)));
                                const finalVerdict = finalScore >= 80 ? "Premium" : finalScore > 50 ? "Standard" : "Fragile";
                                return (
                                    <div className="space-y-4 animate-slide-up bg-white/5 p-4 rounded-2xl border border-white/5">
                                        <div className="space-y-2">
                                            <div className="flex justify-between text-[10px] font-black uppercase text-slate-400">
                                                ...
                                                <span className={finalScore >= 80 ? 'text-emerald-400' : finalScore > 50 ? 'text-indigo-400' : 'text-amber-400'}>{finalScore}%</span>
                                            </div>
                                            <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                                                <div className={`h-full transition-all duration-500 ${finalScore >= 80 ? 'bg-emerald-500' : finalScore > 50 ? 'bg-indigo-500' : 'bg-amber-500'}`} style={{width: `${finalScore}%`}}></div>
                                            </div>
                                        </div>
                                        ...
                                        {diversityResult && (
                                            <div className="space-y-2 mt-3 p-3 bg-black/20 rounded-xl border border-white/5">
                                                <div className="flex justify-between text-[10px] font-black uppercase text-slate-400">
                                                    ...
                                                    <span className={diversityResult.diversityScore >= 0.5 ? 'text-emerald-400' : diversityResult.diversityScore >= 0.3 ? 'text-indigo-400' : 'text-rose-400'}>
                                                        {(diversityResult.diversityScore * 100).toFixed(0)}%
                                                    </span>
                                                </div>
                                                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                                                    <div 
                                                        ...
                                                        style={{ width: `${diversityResult.diversityScore * 100}%` }}
                                                    ></div>
                                                </div>
                                                <div className="flex justify-between text-[8px] text-slate-500 font-bold mt-1">
                                                    <span>Similarité: {diversityResult.meanSimilarity}</span>
                                                    {diversityResult.dominantAlgo && (
                                                        <span className="uppercase tracking-wider text-indigo-400">Dominé par: {diversityResult.dominantAlgo}</span>
                                                    )}
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {selection.length >= 2 && (
                                            <div className="grid grid-cols-2 gap-2 text-[10px] font-mono mt-3">
                                                ...
                                                <div className="bg-black/30 p-2 rounded-lg border border-white/5 flex flex-col items-center">
                                                    ...
                                                    <span className={metrics.ac < 7 ? 'text-amber-400' : 'text-emerald-400'}>{metrics.ac}</span>
                                                </div>
                                                <div className="bg-black/30 p-2 rounded-lg border border-white/5 flex flex-col items-center">
                                                    <span className="text-slate-500">AMPLITUDE</span>
                                                    <span className={metrics.amplitude < 20 ? 'text-rose-400' : 'text-white'}>{metrics.amplitude}</span>
                                                </div>
                                                <div className="bg-black/30 p-2 rounded-lg border border-white/5 flex flex-col items-center">
                                                    <span className="text-slate-500">PARITÉ</span>
                                                    <span className="text-indigo-300">{metrics.parity.split(' / ').map(p => p[0]).join('')}</span>
                                                </div>
                                            </div>
                                        )}

                                        <div className="text-center pt-2">
                                            <span className={`text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-tighter border ${finalScore >= 80 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : finalScore > 50 ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'}`}>
                                                {finalVerdict}
                                            </span>
                                        </div>
                                        {(metrics.warnings.length > 0 || (diversityResult?.isMonoculture)) && (
                                            <div className="mt-2 text-xs text-rose-400/80 bg-rose-500/10 p-2 rounded border border-rose-500/20 space-y-1">
                                                {diversityResult?.isMonoculture && (
                                                    <div>• ⚠️ MONOCULTURE : Similarité trop élevée. Les numéros manquent de diversité algorithmique.</div>
                                                )}
