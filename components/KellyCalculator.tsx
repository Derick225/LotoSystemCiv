...
import { ChevronDown, Percent, Layers, Shuffle, Bot, Activity, Briefcase } from 'lucide-react';
...
export const KellyCalculator: React.FC<KellyCalculatorProps> = ({ confidence }) => {
    ...
    const betOptions = [
        ...Object.entries(LOTO_PAYOUTS[gameMode].SIMPLE).map(([key, val]: [string, { label: string, odds: number, gain: number }]) => ({ key, ...val, group: 'Simple' })),
        ...Object.entries(LOTO_PAYOUTS[gameMode].TURBO).map(([key, val]: [string, { label: string, odds: number, gain: number }]) => ({ key, ...val, group: 'Turbo' })),
    ];
    ...
    const regime = useMemo(() => {
        if (!history || history.length < 10) return { regime: 'stable', volatility: 0.1 };
    ...
    useEffect(() => {
        ...
        if (f <= 0) {
            ...
            result = {
                ...
                advice: portfolioMode ? `Mise par ticket (Portefeuille diversifié de 4 tickets)` : `Mise Optimale (Demi-Kelly)`
            };
        }
        ...
        <div className="bg-gradient-to-r from-emerald-900 to-teal-900 p-5 md:p-6 rounded-3xl md:rounded-[2rem] text-white shadow-lg border border-emerald-700/50 mt-6 relative overflow-hidden">
            ...
            <div className="flex flex-col gap-4 md:gap-6 mb-5 md:mb-6 relative z-10">
                ...
                <div className="grid grid-cols-4 gap-1.5 bg-black/20 p-1 rounded-2xl">
                    ...
                    <button 
                        onClick={() => { audioEngine.play('click'); setPortfolioMode(!portfolioMode); }}
            ...
            <div className="flex flex-col lg:flex-row gap-4 md:gap-6 items-start relative z-10">
                ...
                <div className="flex-1 w-full bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-sm flex flex-col justify-center min-h-[100px] md:min-h-[120px]">
                    <div className="flex justify-between items-start mb-1.5 md:mb-2">
                        <div className="text-[10px] md:text-[10px] font-black uppercase text-emerald-200 tracking-widest">Mise Conseillée</div>
