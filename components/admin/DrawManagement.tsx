...
import { fetchResults, addResult, updateResult, deleteResult, bulkAddResults, bulkDeleteResults, purgeAllResults } from '../../services/lotteryService';
import { ExportService } from '../../services/exportService';
import type { DrawResult } from '../../types';
import { useToast } from '../ui/Toast';
import { Pencil, Trash2, Plus, Save, RotateCcw, Upload, LayoutGrid, Calendar, Download, Stethoscope, RefreshCw, FileSpreadsheet, CheckCircle2, AlertTriangle, Clipboard, DownloadCloud, FileText, Sparkles, Binary, Code2 } from 'lucide-react';
...
export const DrawManagement: React.FC<DrawManagementProps> = ({ drawName }) => {
    ...
    const [activeSubTab, setActiveSubTab] = useState<'manual' | 'bulk' | 'audit' | 'export' | 'purge'>('manual');
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            resetForm(); loadData();
        } catch (e: unknown) { audioEngine.play('error'); showToast((e instanceof Error ? e.message : String(e)), "error"); } finally { setIsSaving(false); }
    };
    ...
    const processRawData = (content: string) => {
        ...
        lines.forEach((line, index) => {
            ...
            if (index === 0 && (lowerLine.includes('date') || lowerLine.includes('g1') || lowerLine.includes('tirage') || lowerLine.includes('id'))) return;
            ...
            if (isValid) {
                if (winners.length !== 5) {
                    ...
                    isValid = false;
                    error = 'Les numéros gagnants doivent être strictement compris entre 1 et 90';
                }
            }
            ...
            if (isValid && machine.length > 0) {
                if (machine.length !== 5 && machine.length !== 0) {
                    ...
                    isValid = false;
                    error = 'Les numéros machine doivent être strictement compris entre 1 et 90';
                }
            }
    ...
    const confirmImport = async () => {
        ...
        try {
            ...
            showToast(`Erreur d'import : ${(e instanceof Error ? e.message : String(e))}`, "error");
        } finally {
    ...
    const handleDeleteSelected = async () => {
        ...
        if (!confirm(`Supprimer définitivement les ${selectedIds.size} tirages sélectionnés ?`)) return;
        ...
        try {
            ...
            audioEngine.play('success');
            showToast(`${selectedIds.size} tirages supprimés avec succès.`, "success");
            setSelectedIds(new Set());
            loadData();
        } catch (e) {
            audioEngine.play('error');
            showToast(`Échec de la suppression : ${e instanceof Error ? e.message : String(e)}`, "error");
        } finally {
            setIsPurging(false);
        }
    };
    ...
    const handleDeleteDateRange = async () => {
        ...
        if (!confirm(`Supprimer définitivement les ${idsToDelete.length} tirages de cette période ?`)) return;
        ...
        try {
            ...
            audioEngine.play('success');
            showToast(`${idsToDelete.length} tirages supprimés pour la période indiquée.`, "success");
            setDeleteStartDate('');
            setDeleteEndDate('');
            loadData();
        } catch (e) {
    ...
    const handlePurgeAll = async () => {
        ...
        if (!confirm(`ATTENTION DANGER : Voulez-vous vraiment purger TOUT l'historique de ${drawName} ? Cette action supprimera TOUT en local et sur le cloud.`)) return;
        if (!confirm(`Confirmation finale : Tapez OK pour détruire définitivement les ${results.length} enregistrements de ${drawName}.`)) return;
        ...
        try {
            ...
            audioEngine.play('success');
            showToast(`L'historique de ${drawName} a été entièrement vidé.`, "success");
            setPurgeConfirmed(false);
            loadData();
        } catch (e) {
            audioEngine.play('error');
            showToast(`Échec de la purge : ${e instanceof Error ? e.message : String(e)}`, "error");
        } finally {
            setIsPurging(false);
        }
    };
        ...
        <div className="space-y-6 animate-fade-in w-full max-w-7xl mx-auto px-2 md:px-0">
            ...
            <div className="bg-slate-900 text-white p-4 md:p-6 rounded-[2.2rem] md:rounded-3xl flex flex-col md:flex-row items-center justify-between shadow-2xl border border-slate-800 gap-4 w-full">
                <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <div>
                        <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-indigo-400">Registre Master</span>
                ...
                <div className="flex gap-2 w-full md:w-auto overflow-x-auto scrollbar-hide pb-1">
                    <button onClick={() => { audioEngine.play('click'); setActiveSubTab('manual'); }} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl transition-all border border-white/5 text-[10px] md:text-xs font-black uppercase flex items-center justify-center gap-2 whitespace-nowrap ${activeSubTab === 'manual' ? 'bg-white text-slate-900 shadow-xl' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                        <Pencil size={12}/> Saisie
                    </button>
                    <button onClick={() => { audioEngine.play('click'); setActiveSubTab('bulk'); }} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl transition-all border border-white/5 text-[10px] md:text-xs font-black uppercase flex items-center justify-center gap-2 whitespace-nowrap ${activeSubTab === 'bulk' ? 'bg-white text-slate-900 shadow-xl' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                        <LayoutGrid size={12}/> Import CSV
                    </button>
                    <button onClick={() => { audioEngine.play('click'); setActiveSubTab('export'); }} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl transition-all border border-white/5 text-[10px] md:text-xs font-black uppercase flex items-center justify-center gap-2 whitespace-nowrap ${activeSubTab === 'export' ? 'bg-white text-slate-900 shadow-xl' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                        <DownloadCloud size={12}/> Export
                    </button>
                    <button onClick={() => { audioEngine.play('click'); setActiveSubTab('audit'); }} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl transition-all border border-white/5 text-[10px] md:text-xs font-black uppercase flex items-center justify-center gap-2 whitespace-nowrap ${activeSubTab === 'audit' ? 'bg-emerald-50 text-white shadow-xl' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                        <Stethoscope size={12}/> Audit
                    </button>
                    <button onClick={() => { audioEngine.play('click'); setActiveSubTab('purge'); }} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl transition-all border border-white/5 text-[10px] md:text-xs font-black uppercase flex items-center justify-center gap-2 whitespace-nowrap ${activeSubTab === 'purge' ? 'bg-rose-600 text-white shadow-xl' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}>
                ...
                <div className="flex flex-col md:grid md:grid-cols-2 gap-8 w-full">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl">
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="text-xs font-black uppercase text-slate-400 tracking-widest mb-2 block">Date</label>
                            ...
                            <div>
                                <label className="text-xs font-black uppercase text-slate-400 tracking-widest mb-2 flex justify-between">
                                    <span>Gagnants</span>
                                    <span className="text-indigo-500 flex items-center gap-1 text-[10px]"><Sparkles size={10}/> 5</span>
                                </label>
                            ...
                            <div>
                                <label className="text-xs font-black uppercase text-slate-400 tracking-widest mb-2 flex justify-between">
                                    <span>Machine</span>
                                    <span className="text-slate-500 flex items-center gap-1 text-[10px]"><Binary size={10}/> Opt.</span>
                                </label>
                                <div className="grid grid-cols-5 gap-2">
                                    {formMac.map((val, idx) => (
                                ...
                                <div key={r.id} className="bg-white dark:bg-slate-800 p-3 md:p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex justify-between items-center group">
                                    <div>
                                        ...
                                        <div className="flex gap-1 mt-1.5 flex-wrap">
                                            {r.gagnants.map(n => <span key={n} className="w-5 h-5 md:w-6 md:h-6 rounded bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 flex items-center justify-center text-[10px] md:text-xs font-bold">{n}</span>)}
                                        </div>
                                    </div>
                    ...
                    {importStep === 'upload' ? (
                        <div className="animate-slide-up">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs md:text-[10px] text-slate-400 font-bold uppercase tracking-widest">Supporte : Date(DD/MM/YYYY), G1-G5, M1-M5</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-2 mb-6 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl w-full md:w-fit overflow-x-auto scrollbar-hide">
                                <button onClick={() => setUploadMode('text')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${uploadMode === 'text' ? 'bg-white dark:bg-slate-800 shadow-md text-indigo-600 dark:text-white' : 'text-slate-400'}`}><Clipboard size={14}/> Coller</button>
                                <button onClick={() => setUploadMode('file')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-xs md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${uploadMode === 'file' ? 'bg-white dark:bg-slate-800 shadow-md text-indigo-600 dark:text-white' : 'text-slate-400'}`}>Fichier</button>
                            </div>
                            ...
                            <div className="flex flex-col gap-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex gap-1">
                                        <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded text-xs font-bold">{validCount} Valides</span>
                                        {errorCount > 0 && <span className="px-2 py-0.5 bg-rose-100 text-rose-700 rounded text-xs font-bold">{errorCount} Erreurs</span>}
                                    </div>
                                </div>
                                <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl">
                                    <button onClick={() => setViewFilter('all')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${viewFilter === 'all' ? 'bg-white dark:bg-slate-700 shadow' : 'text-slate-500'}`}>Tous</button>
                                    <button onClick={() => setViewFilter('valid')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${viewFilter === 'valid' ? 'bg-white dark:bg-slate-700 shadow' : 'text-slate-500'}`}>OK</button>
                                    <button onClick={() => setViewFilter('error')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${viewFilter === 'error' ? 'bg-white dark:bg-slate-700 shadow' : 'text-slate-500'}`}>Error</button>
                                </div>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        {filteredPreview.map((row, i) => (
                                            <tr key={i} className={row.isValid ? '' : 'bg-rose-50 dark:bg-rose-900/10'}>
                                                ...
                                                <td className="p-3">
                                                    <div className="flex gap-1">
                                                        {row.gagnants.map((n, j) => <span key={j} className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-[10px]">{n}</span>)}
                                                    </div>
                                                </td>
                                                <td className="p-3">
                                                    {row.machine.length > 0 ? (
                                                        <div className="flex gap-1">
                                                            {row.machine.map((n, j) => <span key={j} className="w-5 h-5 rounded bg-slate-200 dark:bg-slate-800 text-slate-500 flex items-center justify-center text-[10px]">{n}</span>)}
                                                        </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl animate-scale-in">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        ...
                        >
                            ...
                            <h4 className="font-black text-slate-800 dark:text-white uppercase tracking-tight">Algorithmes (Python)</h4>
                            <p className="text-[10px] text-slate-400 text-center mt-2 max-w-[200px]">
                ...
                <div className="space-y-8 animate-fade-in w-full">
                    ...
                    <div className="bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 p-6 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-black text-rose-900 dark:text-rose-100 uppercase text-sm md:text-base">Zone de Purge & Nettoyage de Masse</h3>
                                <p className="text-xs text-rose-600 dark:text-rose-400 font-medium">Vous gérez actuellement {results.length} enregistrements pour le tirage {drawName}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl flex flex-col h-[600px]">
                            <div className="mb-6 flex justify-between items-center">
                                <div>
                                    <h4 className="font-black text-slate-800 dark:text-white uppercase text-xs md:text-sm">1. Sélection et Suppression</h4>
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Cochez les tirages à éliminer</p>
                                </div>
                                {selectedIds.size > 0 && (
                                    <button 
                            ...
                            <div className="flex gap-2 mb-4 bg-slate-50 dark:bg-slate-900 p-2 rounded-xl text-xs font-bold text-slate-500 justify-between items-center">
                                ...
                                <span className="text-[10px] font-black uppercase text-indigo-500">{selectedIds.size} sélectionné(s)</span>
                            </div>

                            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                                {results.length === 0 ? (
                                    <div className="h-full flex items-center justify-center text-xs text-slate-400 italic">Aucun tirage disponible.</div>
                                ) : (
                                    results.map(r => {
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <input 
                                                        type="checkbox" 
                                                        checked={isChecked} 
                                                        onChange={(e) => { e.stopPropagation(); toggleSelectId(r.id); }}
                                                        className="rounded border-slate-300 text-rose-600 focus:ring-rose-500 w-4 h-4 cursor-pointer"
                                                    />
                                                    <div>
                                                        <div className="text-xs font-black text-slate-700 dark:text-white">{r.date}</div>
                                                        <div className="flex gap-1 mt-1">
                                                            {r.gagnants.map(n => <span key={n} className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-700/60 text-slate-700 dark:text-slate-300 flex items-center justify-center text-[10px] font-bold">{n}</span>)}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-8 flex flex-col justify-between">
                            ...
                            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl flex flex-col">
                                <div className="mb-6">
                                    <h4 className="font-black text-slate-800 dark:text-white uppercase text-xs md:text-sm">2. Suppression par Intervalle</h4>
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Éliminer une plage historique complète</p>
                                </div>

                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div>
                                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-2 block">Date de début</label>
                                        <input 
                                            type="date" 
                                    ...
                                    <div>
                                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-2 block">Date de fin</label>
                                        <input 
                                            type="date" 
                                            value={deleteEndDate} 
                                ...
                                {rangeMatchCount > 0 && (
                                    <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 text-xs font-bold rounded-xl flex items-center gap-2">
                                        ...
                                        <span>{rangeMatchCount} tirages correspondent à cette période et seront supprimés.</span>
                                    </div>
                                )}

                                <button 
                                    onClick={handleDeleteDateRange}
                                    disabled={isPurging || !deleteStartDate || !deleteEndDate || rangeMatchCount === 0}
                            ...
                            <div className="bg-rose-50/30 dark:bg-rose-950/5 border-2 border-dashed border-rose-200 dark:border-rose-900/30 p-6 md:p-8 rounded-3xl flex flex-col justify-between">
                                <div>
                                    <div className="flex items-center gap-2 mb-3 text-rose-600">
                                        ...
                                        <h4 className="font-black uppercase text-xs md:text-sm text-rose-700 dark:text-rose-400">3. Zone de Danger : Purge Absolue</h4>
                                    </div>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed mb-6">
                                        Cette action supprimera **définitivement** tout l'historique disponible de **{drawName}** ({results.length} enregistrements) à la fois sur votre stockage local et sur les serveurs cloud.
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <label className="flex items-start gap-2.5 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide">Je confirme vouloir effacer définitivement tout l'historique de {drawName}.</span>
                                    </label>

                                    <button 
                                        onClick={handlePurgeAll}
                                        disabled={isPurging || !purgeConfirmed || results.length === 0}
                                    ...
                                    >
                                        {isPurging ? <RefreshCw className="animate-spin mx-auto" size={14}/> : `Purger complètement ${drawName}`}
                                    </button>
                                </div>
                            </div>
                        </div>
