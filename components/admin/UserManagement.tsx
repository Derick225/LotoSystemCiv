...
import { Users, ShieldAlert, Trash2, Search, UserCheck, Crown, Clock, RefreshCw, AlertTriangle } from 'lucide-react';
...
export const UserManagement: React.FC = () => {
    ...
    const loadUsers = async () => {
        ...
        try {
            ...
            console.error(e);
            setError((e instanceof Error ? e.message : String(e)) || "Erreur de chargement.");
            showToast((e instanceof Error ? e.message : String(e)) || "Erreur chargement utilisateurs", "error");
    ...
    const getSubscriptionBadge = (sub: AdminUser['subscription']) => {
        if (!sub) return <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded text-xs font-bold">Inactif</span>;
        
        const isActive = sub.status === 'active' || sub.status === 'trial';
        const isPremium = sub.plan === 'premium';
        
        return (
            <span className={`px-2 py-1 rounded text-xs font-bold flex items-center gap-1 w-fit ${isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 animate-slide-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <button onClick={loadUsers} className="p-3 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-2xl hover:rotate-180 transition-all">
