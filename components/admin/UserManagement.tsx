...
export const UserManagement: React.FC = () => {
    ...
    const getSubscriptionBadge = (sub: AdminUser['subscription']) => {
        if (!sub) return <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded text-[9px] font-bold">Inactif</span>;
        
        const isActive = sub.status === 'active' || sub.status === 'trial';
        const isPremium = sub.plan === 'premium';
        
        return (
            <span className={`px-2 py-1 rounded text-[9px] font-bold flex items-center gap-1 w-fit ${isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-700 animate-slide-up">
