...
import { Database, RefreshCw, AlertTriangle, Zap } from "lucide-react";
...
export const DataIntegrityMonitor: React.FC<{ drawName: string }> = ({
  ...
  const analyzeIntegrity = async () => {
    ...
    try {
      ...
      const daysSinceLast = Math.floor(
        (new Date().getTime() - lastDrawDate.getTime()) / (1000 * 60 * 60 * 24),
      );
    ...
    try {
      ...
      for (const corrupt of report.corruptData) {
        try {
          ...
          console.error("Failed to delete corrupt", e);
        }
      }
      audioEngine.play("success");
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl animate-fade-in">
      <div className="flex justify-between items-center mb-10">
        <h3 className="font-black text-slate-800 dark:text-white flex items-center gap-4 uppercase tracking-tighter">
          <Database className="w-6 h-6 text-indigo-500" /> Moniteur d'Intégrité
          HPC
        </h3>
        <button
          onClick={() => {
          ...
          className="p-3 bg-slate-100 dark:bg-slate-700 rounded-2xl hover:rotate-180 transition-all"
        >
          <RefreshCw
            className={
      ...
      {report && (
        <div className="space-y-8">
          <div className="grid md:grid-cols-3 gap-8">
            <div className="p-8 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center text-center">
              <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">
                Health Score
              </div>
              <div
                className={`text-6xl font-black ${report.healthScore > 80 ? "text-emerald-500" : "text-rose-500"}`}
              >
                {report.healthScore}%
              </div>
              <span className="text-[10px] font-black px-3 py-1 bg-white dark:bg-slate-800 rounded-full mt-4 border border-black/5 dark:border-white/5 uppercase tracking-widest">
                {report.status}
              </span>
            </div>

            <div className="p-8 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col justify-center space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-lg font-black ${report.duplicates.length > 0 ? "text-rose-500" : "text-emerald-500"}`}
                >
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-lg font-black ${report.corruptData.length > 0 ? "text-rose-500" : "text-emerald-500"}`}
                >
                  {report.corruptData.length}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-slate-400 uppercase">
                ...
                <span className="text-lg font-black text-indigo-500">
                  {report.lastGapDays}j
                </span>
              </div>
            </div>

            <div className="p-8 bg-indigo-600 rounded-2xl text-white flex flex-col justify-center items-center text-center shadow-xl shadow-indigo-600/20 relative overflow-hidden group">
            ...
            <div className="p-6 bg-rose-50 dark:bg-rose-900/10 rounded-[2rem] border border-rose-100 dark:border-rose-800">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                {report.corruptData.map((c, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center text-[10px] font-mono text-rose-800 dark:text-rose-300 bg-white/50 dark:bg-black/20 p-2 rounded-lg"
                  >
