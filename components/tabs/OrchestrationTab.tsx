...
const VectorFlowChart: React.FC<{
  prevDraw: number[];
  candidates: number[];
}> = ({ prevDraw, candidates }) => {
    ...
    prevDraw.forEach((src) => {
      topCands.forEach((tgt) => {
        ...
        // Inversion Chiffres (12 <-> 21)
          ...
          src.toString().split("").reverse().join("") === tgt.toString() &&
          src > 10
        ) {
          type = "Shadow";
          color = "#f59e0b";
    ...
    <div className="bg-slate-950 text-white p-6 md:p-8 rounded-2xl shadow-2xl border border-slate-800 overflow-hidden relative min-h-[450px] flex flex-col justify-between group select-none">
      ...
      <div className="flex justify-between items-stretch relative z-10 h-full gap-8">
        ...
        <div className="flex flex-col justify-around items-center w-24 py-4 relative z-20">
          ...
          {prevDraw.map((n) => {
            const isActive = hoveredNode
              ? (hoveredNode.type === "src" && hoveredNode.id === n) ||
                links.some(
                  (l) =>
                    l.src === n &&
                    l.tgt ===
                      (hoveredNode.type === "tgt" ? hoveredNode.id : -1),
                )
              : true;
            return (
              <div
        ...
        <div className="flex-1 relative">
          ...
          {hoveredNode &&
            links.map((link, i) => {
              ...
              const tIdx = topCands.indexOf(link.tgt);
              const topPos =
                ((sIdx + tIdx + 1) / (prevDraw.length + topCands.length)) * 100;

              return (
                <div
                  key={`lbl-${i}`}
                  className="absolute left-1/2 -translate-x-1/2 transition-all duration-300 pointer-events-none z-30"
                  style={{ top: `${topPos}%` }}
                >
                  <span
                    className="text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded bg-slate-900 border border-slate-700 text-white shadow-xl"
        ...
        <div className="flex flex-col justify-around items-center w-24 py-4 relative z-20">
          ...
          {topCands.map((n) => {
            const isActive = hoveredNode
              ? (hoveredNode.type === "tgt" && hoveredNode.id === n) ||
                links.some(
                  (l) =>
                    l.tgt === n &&
                    l.src ===
                      (hoveredNode.type === "src" ? hoveredNode.id : -1),
                )
              : true;
            return (
              <div
              ...
              >
                <NumberBall
                  ...
                  size="md"
                  selected={
                    hoveredNode?.id === n && hoveredNode?.type === "tgt"
                  }
                />
              </div>
            );
      ...
      {mimicry.length === 0 ? (
        ...
        <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar max-h-[300px] pr-2">
          {mimicry.slice(0, 5).map((m, _i) => (
            <div
              key={m.number}
              className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-black/20 border border-slate-100 dark:border-slate-800 hover:border-amber-500/30 transition-colors"
            >
              ...
              <div className="flex flex-col items-end">
                <span
                  className={`text-sm font-black ${m.score >= 50 ? "text-emerald-500" : "text-indigo-500"}`}
                >
                  {m.score}pts
                </span>
                <div className="h-1 w-12 bg-slate-200 dark:bg-slate-700 rounded-full mt-1 overflow-hidden">
                  <div
                    className={`h-full rounded-full ${m.score >= 50 ? "bg-emerald-500" : "bg-indigo-500"}`}
      ...
      try {
        if (history.length > 2 && isMounted.current) {
          const validHistory = history.filter(
            (d) => d && Array.isArray(d.gagnants) && d.gagnants.length > 0,
          );
  ...
  if (nexusLoading || loading)
    return (
      <div className="flex flex-col items-center justify-center p-24 gap-6 animate-pulse">
        ...
        <p className="font-black text-indigo-500 uppercase tracking-[0.4em] text-xs">
          Triangulation Vectorielle (ADN Actif)...
        </p>
      </div>
    );
            ...
            <div className="bg-white dark:bg-slate-800 p-2 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 relative overflow-hidden">
              <div className="px-8 pt-6 pb-2 flex items-center gap-3 relative z-10">
                ...
                <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">
                  Flux d'Influence Directe (T-1 ➜ IA)
                </h4>
              </div>
              <VectorFlowChart
                prevDraw={prevDraw.gagnants}
                candidates={metrics.topCandidates.map((c) => c.number)}
              />
            </div>
          </div>
        )}
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-950 p-8 md:p-6 rounded-3xl border border-slate-800 shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6">
          <div className="max-w-md">
            ...
            <p className="text-slate-400 text-xs font-medium leading-relaxed">
              Générez un ticket optimisé qui respecte à la fois les scores
              d'orchestration et les règles de cohérence géométrique (AC,
              Spread).
            </p>
          </div>
          ...
          >
            {isGenerating ? (
              <>
                <Layers size={18} className="animate-spin" /> Analyse en
                cours...
              </>
            ) : (
              <>
          ...
          <div className="mt-10 pt-10 border-t border-white/10 flex flex-col items-center justify-center gap-6 animate-pulse">
            <div className="flex gap-4">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="w-12 h-12 rounded-full bg-slate-800 border border-slate-700 animate-pulse"
        ...
        {generatedTicket && !isGenerating && (
          <div className="mt-10 pt-10 border-t border-white/10 animate-slide-up">
            <div
              id="generated-ticket-view"
              className="bg-white/5 rounded-3xl p-6 border border-emerald-500/30 flex flex-col items-center gap-8"
            >
              ...
              {currentPrediction?.xapExp &&
                currentPrediction.xapExp.length > 0 && (
                  <div className="w-full mt-6 bg-slate-900/50 rounded-2xl p-6 border border-slate-800 backdrop-blur-xl">
                    <h4 className="text-[10px] font-black tracking-[0.2em] text-slate-400 uppercase mb-6 flex items-center justify-center gap-2">
                      ...
                      Attribution XAP (Orchestra)
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                      {currentPrediction.xapExp.map((xap) => (
                        <div
                          key={xap.number}
                          className="bg-slate-800/80 rounded-xl p-3 border border-slate-700/50 flex flex-col items-center text-center"
                        >
                          ...
                          <span
                            className="text-[9px] uppercase font-bold text-indigo-400 mb-1 leading-tight line-clamp-1"
