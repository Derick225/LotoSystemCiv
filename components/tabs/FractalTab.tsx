...
import { TrendingUp, AlertTriangle, CheckCircle2, Compass, Activity } from 'lucide-react';
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-16">
        ...
        <div className="glass-card neural-border p-8 rounded-3xl shadow-xl space-y-6">
            <div>
                <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-[0.2em]">HPC INFERENCE</span>
                ...
                <p className="text-xs text-slate-500 font-medium leading-relaxed mt-2 max-w-3xl">
                    Ces dimensions évaluent la structure mathématique intrinsèque de l'attracteur dynamique sans aucun bruit stochastique ou coefficient arbitraire.
                </p>
            </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span className="text-[10px] font-bold text-slate-400">ÉQUIRÉPARTITION CRITIQUES</span>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                            Mesure l'écart suprémum cumulé par rapport à une distribution uniforme uniforme modulaire. Plus ce score est proche de 0.0, plus le flux est fluide.
                        </p>
                    </div>

                    <div className="mt-6 space-y-2">
                        <div className="flex justify-between items-baseline mb-1">
                            <span className="text-xs font-black text-slate-400">Weyl D* Stat</span>
                            <span className="text-2xl font-black font-mono text-indigo-500">{weylDiscrepancy.toFixed(4)}</span>
                        </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span className="text-[10px] font-bold text-slate-400">DIMENSION FRACTALE DYNAMIQUE</span>
                            </div>
                        </div>
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                            Estime la dimension fractale dynamique d'auto-corrélation d'espace-phase du système. Représente le nombre d'équations couplées nécessaires pour modéliser le flux.
                        </p>
                    </div>

                    <div className="mt-6 space-y-2">
                        <div className="flex justify-between items-baseline mb-1">
                            <span className="text-xs font-black text-slate-400">Dimension Fractale ν</span>
                            <span className="text-2xl font-black font-mono text-violet-500">{chaosDimension.toFixed(3)}</span>
                        </div>
                        <div className="h-2 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-violet-500 rounded-full transition-all duration-1000" 
                                style={{ width: `${Math.min(100, ((chaosDimension - 1.0) / 2.0) * 100)}%` }}
                            ></div>
                        </div>
                        <div className="flex justify-between text-[9px] font-bold text-slate-400 uppercase tracking-widest pt-1">
