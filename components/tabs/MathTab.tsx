...
export const MathTab: React.FC<MathTabProps> = ({ drawName }) => {
      ...
      return (
          <div className="space-y-4">
              <div className="flex justify-between text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">
                  <span>Inhabituel</span>
                  <span>Normalité</span>
                  <span>Chaotique</span>
              </div>
              <div className="h-4 w-full bg-slate-100 dark:bg-slate-900 rounded-full relative overflow-hidden border border-slate-200 dark:border-slate-800 shadow-inner">
  ...
  return (
    <div className="space-y-8 animate-fade-in w-full overflow-hidden">
        {/* MÉTÉO DU FLUX */}
        <div className="bg-slate-900 text-white p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] shadow-2xl border border-slate-800 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
            <div className={`p-8 rounded-full ${tempStatus.color} bg-opacity-10 border-2 border-dashed border-current animate-pulse-slow`}>
                {React.cloneElement(tempStatus.icon, { size: 48 })}
            </div>
            
            <div className="flex-1 text-center md:text-left">
                <h3 className={`text-4xl font-black uppercase tracking-tighter ${tempStatus.color}`}>
                ...
                <div className="mt-8 grid grid-cols-2 md:grid-cols-3 gap-6">
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                        <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Score Z</div>
                        <div className="text-2xl font-black">{report.runsTest.zScore.toFixed(2)}</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                        <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Momentum</div>
                        <div className="text-2xl font-black text-indigo-400">{tempStatus.value.toFixed(1)}°</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-700 space-y-8">
                <div className="flex items-center gap-3">
                    <RotateCw className="text-indigo-600" size={24} />
                    <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Audit de Brassage</h3>
                </div>
                <p className="text-sm text-slate-500 leading-relaxed font-medium">
                    Mesure si la séquence des numéros sortis respecte les lois naturelles du hasard. Un score trop élevé indique une séquence "suspecte" ou dirigée.
                </p>
                {renderZScoreGauge(report.runsTest.zScore)}
                <div className={`p-4 rounded-2xl border text-center font-black text-xs uppercase tracking-widest ${report.runsTest.isRandom ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
            ...
            {/* GÉOMÉTRIE SHADOW */}
            {shadows && (
                <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-xl border border-indigo-100 dark:border-indigo-900/50">
                    <div className="flex items-center gap-3 mb-8">
                        <Layers className="text-indigo-600" size={24} />
                        <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Géométrie de l'Ombre</h3>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                        {[
                            { label: 'Modulo Sigma', val: shadows.sumModulo },
                            { label: 'Nombre d\'Or', val: shadows.goldenNumber },
                            { label: 'Complément 1', val: shadows.firstCompliment },
                            { label: 'Lien Écart', val: shadows.gapLink }
                        ].map(item => (
                            <div key={item.label} className="bg-slate-50 dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 text-center transition-all hover:scale-105">
                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{item.label}</div>
                                <div className="text-3xl font-black text-indigo-600 dark:text-indigo-400 font-mono">{item.val}</div>
                            </div>
                        ))}
                    </div>
                </div>
