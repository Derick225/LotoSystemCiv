...
import { RotateCw, Layers, AlertOctagon, CheckCircle2, Waves, Wind } from 'lucide-react';
...
export const MathTab: React.FC<MathTabProps> = ({ drawName }) => {
  ...
  const [tempStatus, setTempStatus] = useState<{ value: number, label: string, color: string, icon: React.ReactElement<{ size?: number }>, desc: string }>({ 
  ...
  useEffect(() => {
    if (history.length > 0) {
        ...
        const analysis: MathAnalysisReport = {
            ...
            sumHistory: history.slice(0, 50).map(d => ({ date: d.date, sum: d.gagnants.reduce((a: number, b: number)=>a+b, 0), avg: calibration.meanSum })),
    ...
    <div className="space-y-6 md:space-y-8 animate-fade-in w-full overflow-hidden">
            ...
            <div className="flex-1 text-center md:text-left">
                ...
                <div className="mt-6 md:mt-8 grid grid-cols-2 gap-4">
                    <div className="bg-white/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-white/5">
                        <div className="text-[10px] md:text-[10px] font-black text-slate-500 uppercase mb-1">Score Z</div>
                        <div className="text-lg md:text-2xl font-black">{report.runsTest.zScore.toFixed(2)}</div>
                    </div>
                    <div className="bg-white/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-white/5">
                        <div className="text-[10px] md:text-[10px] font-black text-slate-500 uppercase mb-1">Momentum</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2rem] md:rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 space-y-6">
                ...
                <div className={`p-3 md:p-4 rounded-xl md:rounded-2xl border text-center font-black text-xs md:text-xs uppercase tracking-widest ${report.runsTest.isRandom ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                        {[
                            ...
                            <div key={item.label} className="bg-slate-50 dark:bg-slate-900 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100 dark:border-slate-800 text-center transition-all">
                                <div className="text-[10px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2">{item.label}</div>
