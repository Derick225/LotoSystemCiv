...
export const MathTab: React.FC<MathTabProps> = ({ drawName }) => {
    ...
    if (history.length > 0) {
      ...
      const analysis: MathAnalysisReport = {
        ...
        consecutiveStats: {
          ...
          percentage: Math.round(
            (drawsWithConsecutive / recentDraws.length) * 100,
          ),
        },
      ...
      if (lastMomentum > 20)
        setTempStatus({
          ...
          icon: <AlertOctagon />,
          desc: "Le système produit des anomalies fréquentes.",
        });
      else if (lastMomentum < -20)
        setTempStatus({
          value: lastMomentum,
          label: "Bruit Blanc",
          color: "text-indigo-500",
          icon: <Wind />,
          desc: "Déséquilibre vers le hasard pur.",
        });
      else
        setTempStatus({
          value: lastMomentum,
          ...
          icon: <CheckCircle2 />,
          desc: "Conditions optimales pour les patterns classiques.",
        });
    }
  }, [history]);
  ...
  const renderZScoreGauge = (z: number) => {
    ...
    return (
      <div className="space-y-3">
        <div className="flex justify-between text-[10px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">
          <span>Rare</span>
          <span>Normal</span>
          <span>Rare</span>
        </div>
        <div className="h-3 md:h-4 w-full bg-slate-100 dark:bg-slate-900 rounded-full relative overflow-hidden border border-slate-200 dark:border-slate-800 shadow-inner">
          ...
          <div
            className={`absolute top-0 bottom-0 w-2 rounded-full transition-all duration-500 ${color} shadow-lg border-2 border-white dark:border-slate-900 z-10`}
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in w-full overflow-hidden">
      ...
      <div className="bg-slate-900 text-white p-6 md:p-8 rounded-[2rem] md:rounded-3xl shadow-2xl border border-slate-800 relative overflow-hidden flex flex-col md:flex-row items-center gap-6 md:gap-6">
        <div
          className={`p-6 md:p-8 rounded-full ${tempStatus.color} bg-opacity-10 border-2 border-dashed border-current animate-pulse-slow shrink-0`}
        >
          {React.cloneElement(tempStatus.icon, {
        ...
        <div className="flex-1 text-center md:text-left">
          <h3
            className={`text-2xl md:text-4xl font-black uppercase tracking-tighter ${tempStatus.color}`}
          >
          ...
          <div className="mt-6 md:mt-8 grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-white/5">
              <div className="text-[10px] md:text-[10px] font-black text-slate-500 uppercase mb-1">
                Score Z
              </div>
              <div className="text-lg md:text-2xl font-black">
                {report.runsTest.zScore.toFixed(2)}
              </div>
            </div>
            <div className="bg-white/5 p-3 md:p-4 rounded-xl md:rounded-2xl border border-white/5">
              <div className="text-[10px] md:text-[10px] font-black text-slate-500 uppercase mb-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        {/* AUDIT DE BRASSAGE */}
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2rem] md:rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 space-y-6">
          <div className="flex items-center gap-3">
            <RotateCw className="text-indigo-600" size={20} />
            <h3 className="text-lg md:text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">
              Audit de Brassage
            </h3>
          </div>
          ...
          <div
            className={`p-3 md:p-4 rounded-xl md:rounded-2xl border text-center font-black text-xs md:text-xs uppercase tracking-widest ${report.runsTest.isRandom ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-rose-50 text-rose-600 border-rose-200"}`}
          >
            Verdict :{" "}
            {report.runsTest.isRandom
              ? "Aléas Naturel"
        ...
        {/* GÉOMÉTRIE SHADOW */}
        {shadows && (
          <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2rem] md:rounded-3xl shadow-xl border border-indigo-100 dark:border-indigo-900/50">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              {[
                ...
                <div
                  key={item.label}
                  className="bg-slate-50 dark:bg-slate-900 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100 dark:border-slate-800 text-center transition-all"
                >
                  <div className="text-[10px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2">
                    {item.label}
                  </div>
                  <div className="text-xl md:text-3xl font-black text-indigo-600 dark:text-indigo-400 font-mono">
                    {item.val}
                  </div>
                </div>
              ))}
