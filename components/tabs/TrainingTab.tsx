...
import {
  ...
  calculatePositionalDNAProfiles,
  runLoopSimulation,
...
const getDrawSpecifics = (name: string) => {
  ...
  let picks = 5;
  let description =
    "Un univers stochastique étendu nécessitant un ajustement précis des composantes chaotiques (Entropy Regime) et spectrales.";

  if (cleaned.includes("EUROMILLIONS") || cleaned.includes("EURO")) {
    balls = 50;
    picks = 5;
    description =
      "Une densité réduite avec un coefficient d'affinité optimal (Co-occurrence) centré sur les décades médianes.";
  } else if (cleaned.includes("POWERBALL") || cleaned.includes("AMERICA")) {
    balls = 69;
    picks = 5;
    description =
      "Un système d'affinité asymétrique favorisant l'analyse fine des lacunes de vitesse (Gap Velocity).";
  } else if (cleaned.includes("49") || cleaned.includes("6/")) {
    balls = 49;
    picks = 6;
    description =
      "Régime d'autocorrélation harmonique élevée. Les estimations de processus de point (Poisson) sont prioritaires.";
  }
...
const GlowingHelix: React.FC<{ active: boolean }> = ({ active }) => {
  ...
  return (
    <svg
      viewBox="0 0 100 240"
      className="w-24 h-56 mx-auto drop-shadow-[0_0_15px_rgba(99,102,241,0.25)]"
    >
      <defs>
...
const CyberneticValidation: React.FC<{
  weights: AlgoWeights;
  drawName: string;
  history: any[];
}> = ({ weights, drawName, history }) => {
  const [running, setRunning] = useState(false);
  const [report, setReport] = useState<BacktestReport | null>(null);
  const [strategy, setStrategy] = useState<
    "FLAT" | "KELLY" | "CONFIDENCE_SMART"
  >("CONFIDENCE_SMART");
      ...
      {report ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
            ...
            <div
              className={`text-lg font-black ${report.roi >= 0 ? "text-emerald-500" : "text-rose-500"}`}
            >
              {report.roi > 0 ? "+" : ""}
    ...
    <div className="bg-[#040815] rounded-2xl border border-slate-800 p-4 font-mono text-[10px] h-48 overflow-hidden flex flex-col shadow-inner">
      <div className="flex items-center gap-2 border-b border-slate-850 pb-2 mb-2 text-slate-500 justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold">NEXUS_KERNEL_LOGS</span>
        </div>
      ...
      >
        {logs.map((log, i) => (
          <div
            key={i}
            className="text-emerald-500/90 leading-relaxed text-[9.5px]"
          >
            <span className="text-slate-600 mr-2 font-light">
        ...
        {logs.length === 0 && (
          <span className="text-slate-700 italic">
            En attente du processus de calcul...
          </span>
        )}
      </div>
    </div>
  ...
  if (!snapshot) {
    return (
      <div className="bg-[#05091a]/80 border border-slate-800/80 p-6 rounded-2xl shadow-xl text-center text-slate-500 text-[10px] font-bold uppercase tracking-wider">
        ...
        <span>
          Lancez l'entraînement pour capturer l'ADN initial de chaque numéro
        </span>
      </div>
    );
  }
    ...
    <div className="bg-[#05091a]/85 border border-slate-800/80 p-6 rounded-2xl shadow-xl relative overflow-hidden min-w-0">
      ...
      <div className="mb-6">
        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
          <BrainCircuit size={14} className="text-emerald-400" /> Surveillance
          ADN Initial des Numéros (Étape 1)
        </h4>
        <p className="text-[10px] text-slate-500 mt-1">
          Visualisation de l'ADN algorithmique de chaque numéro au moment de
          générer la toute première prédiction chronologique.
        </p>
      </div>
        ...
        <div className="lg:col-span-7">
          ...
          <div className="grid grid-cols-10 gap-1 bg-slate-950 p-3 rounded-xl border border-white/5 max-h-56 overflow-y-auto custom-scrollbar">
            {Array.from({ length: maxBalls }).map((_, idx) => {
              ...
              const dnaSum = hasDNA
                ? Object.values(snapshot[num]).reduce((s, w) => s + w, 0)
                : 0;
            ...
            <div className="space-y-2.5 max-h-52 overflow-y-auto custom-scrollbar pr-1">
              {sortedAlgos.slice(0, 8).map(({ algo, weight }) => {
                ...
                return (
                  <div
                    key={algo}
                    className="bg-black/35 p-2 rounded-lg border border-white/5"
                  >
                    <div className="flex justify-between items-center text-[9px] mb-1">
                      ...
                      <span
                        className={`font-black ${isDominant ? "text-emerald-400" : "text-slate-400"}`}
                      >
                        {percentage}%
                      </span>
                    </div>
...
export const TrainingTab: React.FC<{ drawName: string }> = ({ drawName }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<"standard" | "darwinian">(
    "standard",
  );
  ...
  const [originalWeights, setOriginalWeights] =
    useState<AlgoWeights>(DEFAULT_ALGO_WEIGHTS);
  const [liveWeights, setLiveWeights] =
    useState<AlgoWeights>(DEFAULT_ALGO_WEIGHTS);
  const [logs, setLogs] = useState<string[]>([]);
  const [finalReport, setFinalReport] = useState<TrainingReport | null>(null);
  const [improvement, setImprovement] = useState(0);
  const [firstPredictionDNASnapshot, setFirstPredictionDNASnapshot] =
    useState<Record<number, Record<AlgoKey, number>> | null>(null);
  ...
  const handleStartLoopSimulation = async () => {
    if (history.length === 0) {
      showToast(
        "Aucun historique disponible pour lancer la simulation.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      audioEngine.play("success");
      showToast(
        `Simulation de boucle terminée ! Amélioration: ${result.improvement}%`,
        "success",
      );
    } catch (e: any) {
  ...
  const [applyingState, setApplyingState] = useState<
    "idle" | "normalizing" | "kalman" | "finalizing" | "completed"
  >("idle");
  ...
  const [positionalProfiles, setPositionalProfiles] = useState<
    Record<number, Record<AlgoKey, number>>
  >({});
  const [calculatingPositional, setCalculatingPositional] =
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const profiles = calculatePositionalDNAProfiles(
        history,
        liveWeights || originalWeights,
      );
  ...
  const handleStartTraining = async () => {
    if (history.length < 15) {
      showToast(
        "Garde-Fou d'Échantillonnage Réel : Un minimum de 15 tirages réels est exigé pour entraîner le réseau.",
        "error",
      );
      audioEngine.play("error");
      return;
    ...
    try {
      const result = await evolveNeuralDNA(
        ...
        (data) => {
          ...
          if (data.gen % 10 === 0) {
            addLog(
              `Génération ${data.gen}: Fitness Best = ${data.bestFitness.toFixed(1)} | Diversité = ${(data.diversity * 100).toFixed(1)}%`,
            );
          }

          const numFeatures = Object.keys(data.bestGenome).length || 1;
          const theoreticalMinDiversity = 1.0 / numFeatures;
          const diversityThreshold = theoreticalMinDiversity * 0.2;

          if (data.diversity < diversityThreshold) {
            addLog(
              "⚠️ ALERTE : Collapsus de Simplex détecté ! Perturbation déterministe orthogonale appliquée.",
            );
          }
        },
      );
      ...
      if (result.report) {
        ...
        // @ts-ignore
        setFirstPredictionDNASnapshot(
          result.firstPredictionDNASnapshot || null,
        );
        setStatus("completed");
        addLog(
          `✓ Convergence stabilisée atteinte. Solution optimale identifiée.`,
        );

        if (result.isGeneralizable === false) {
          addLog(
            `⚖️ SURAPPRENTISSAGE COULÉ (Ratio: ${result.overfittingRatio?.toFixed(2)}). Blending d'atténuation appliqué continûment.`,
          );
          showToast("Atténuation de surapprentissage appliquée.", "info");
        } else if (result.isGeneralizable === "unverifiable") {
          addLog(
            `ℹ️ VALIDATION INSUFFISANTE : Historique trop court pour tester la généralisation de façon robuste. Mode confiance neutre actif.`,
          );
      ...
      setStatus("idle");
      addLog(
        `[EXCEPTION ERROR] Échec du calcul : ${e instanceof Error ? e.message : String(e)}`,
      );
  ...
  const handleApply = async () => {
    ...
    audioEngine.play("scan");
    setCalibrationLogs([
      "DÉBUT DE L'INJECTION GÉNOMIQUE DANS LE NOYAU CORE...",
    ]);
    ...
    setCalibrationLogs((prev) => [
      ...prev,
      "✓ PROJECTION DE NORMES DE PROBABILITÉS L1 COMPLÉTÉE.",
      "PROXIMAL BIAS ET ENCODAGES DE GRILLES SÉCURISÉS.",
    ]);
    audioEngine.play("click");
    ...
    setCalibrationLogs((prev) => [
      ...prev,
      "✓ FILTRE ADAPTATIF DE KALMAN : ÉTATS DE COVARIANCE ENTIÈREMENT INTÉGRÉS.",
      "PÉNALITÉS DU BRIER SCORE INJECTÉES.",
      "COEFFICIENTS DE CAUCHY STRUCTURE POUR EVITER L'EXTINCTION DE L'HISTORIQUE DE STRATÉGIES.",
    ]);
    audioEngine.play("click");
    ...
    setCalibrationLogs((prev) => [
      ...prev,
      "✓ ALIGNEMENT CHRONOLOGIQUE PARÉ POUR LE TIRAGE ACTIF.",
      "ÉCRÊTAGE ET NORMALISATION RE-EQUILIBRES.",
    ]);
    audioEngine.play("success");
    ...
    setStatus("idle");
    showToast(
      "L'ADN du moteur de prédiction a été mis à jour avec brio.",
      "success",
    );
  };
  ...
  const handleImportDNA = async () => {
    ...
    try {
      ...
      setOriginalWeights(safeWeights);
      showToast(
        "ADN Algorithmique importé et appliqué avec succès.",
        "success",
      );
      audioEngine.play("success");
    } catch (e: any) {
  ...
  const benchmarkData = [
    ...
    {
      name: "Optimisé",
      score: finalReport ? finalReport.score : 0,
      fill: "#10b981",
    },
  ];
      ...
      {activeSubTab === "darwinian" ? (
        ...
        <>
            ...
            <div className="fixed inset-0 bg-[#020512]/95 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="max-w-md w-full bg-[#050b1a] rounded-3xl border border-indigo-900/40 p-8 shadow-2xl relative overflow-hidden">
                ...
                <div className="text-center mb-6 relative z-10">
                  <Dna
                    size={48}
                    className="text-indigo-400 mx-auto animate-spin duration-3000 mb-4"
                  />
                  <h3 className="text-xl font-bold tracking-tight text-white uppercase">
                    Mise à jour génomique
                  </h3>
                  <p className="text-slate-400 text-xs mt-1">
                    Calibration dynamique des vecteurs algorithmiques...
                  </p>
                </div>
                ...
                <div className="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden">
                  <div
                    ...
                    style={{
                      width:
                        applyingState === "normalizing"
                          ? "25%"
                          : applyingState === "kalman"
                            ? "65%"
                            : applyingState === "finalizing"
                              ? "90%"
                              : "100%",
                    }}
                  ></div>
          ...
          <div className="bg-[#090e1f] border border-slate-800/80 p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-teal-400 pointer-events-none hidden lg:block">
              <Dna size={220} />
            </div>
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-slate-400 text-xs md:text-sm font-medium mt-3 max-w-lg leading-relaxed">
                    Analyse continûment les erreurs de prévision pour
                    auto-ajuster les poids des algorithmes. Système protégé
                    contre le surapprentissage par filtre de Kalman adaptatif.
                  </p>
                </div>
                ...
                <div className="mt-6 p-4 bg-indigo-950/20 border border-indigo-900/30 rounded-2xl flex items-start gap-3.5 max-w-xl">
                  ...
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-wider text-teal-300">
                      Objectif du Tirage : {drawName}
                    </h4>
                    <div className="text-[11px] text-slate-350 leading-relaxed mt-1">
                      {drawSpecifics.description}{" "}
                      <span className="text-indigo-400 font-bold">
                        Domaine de numéros : 1-{drawSpecifics.balls} &bull;
                        Taille de ticket : {drawSpecifics.picks}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-4 mt-6">
                  <button
                    ...
                    className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-white px-4 py-2 bg-slate-950/40 hover:bg-slate-900 border border-slate-800 rounded-xl transition-all cursor-pointer"
                  >
                    <Download size={12} /> Exporter ADN (Config)
                  </button>
                  <button
                    onClick={handleImportDNA}
                    className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-indigo-450 px-4 py-2 bg-slate-950/40 hover:bg-slate-900 border border-slate-800 hover:border-indigo-900/50 rounded-xl transition-all cursor-pointer"
                  >
              ...
              <div className="flex flex-col gap-4 min-w-[325px] w-full xl:w-auto bg-[#040815]/90 p-6 rounded-2xl border border-white/5 backdrop-blur-sm self-center">
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                    <Sliders size={12} /> Solutionneur d'Équilibre
                  </span>
                ...
                {history.length < 15 && (
                  <div className="p-3 bg-rose-950/80 border border-rose-500/40 rounded-xl text-rose-200 text-xs flex items-start gap-2.5 my-1">
                    ...
                    <div className="space-y-0.5">
                      <span className="font-bold uppercase text-[10px] text-rose-400 block tracking-wider">
                        Garde-Fou Réel (&lt; 15 Tirages)
                      </span>
                      <p className="text-[11px] text-rose-200/90 leading-tight">
                        Entraînement verrouillé sur moins de 15 tirages réels (
                        {history.length} reçus).
                      </p>
                    </div>
                  </div>
                )}
                ...
                {history.length < 15 ? (
                  ...
                  >
                    ...
                    <span>
                      Interrompre (
                      {evolutionData[evolutionData.length - 1]?.gen || 0}/
                      {generations})
                    </span>
                  </button>
                ) : (
                  <div className="flex gap-2 mt-2">
                    <button
                      ...
                      className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold text-xs uppercase cursor-pointer flex items-center justify-center"
                    >
                      <X size={14} />
                    </button>
                    <button
                      ...
                      className="flex-[4] py-3 bg-emerald-600 hover:bg-emerald-550 text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 shadow-lg hover:shadow-emerald-900/40 animate-pulse cursor-pointer"
                    >
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-6 min-w-0">
              ...
              <div className="bg-[#05091a]/80 p-4 md:p-6 rounded-2xl shadow-xl border border-slate-850 h-80 relative overflow-hidden min-w-0 w-full">
                ...
                {evolutionData.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-500 text-[10px] uppercase font-bold tracking-[0.2em] opacity-40">
                    <History
                      size={28}
                      className="mb-2 animate-pulse text-indigo-400"
                    />
                    <span>
                      Démarrer l'évolution pour projeter la trajectoire
                      d'apprentissage
                    </span>
                  </div>
                )}
              ...
              {(() => {
                const currentGen =
                  evolutionData[evolutionData.length - 1]?.gen || 0;
                const dynamicLR = computeDynamicLearningRate(
                  0.05,
                  currentGen,
                  0.5,
                );
                ...
                const actualWinningNumbers = lastDraw
                  ? lastDraw.gagnants
                  : [5, 12, 34, 56, 88];
                ...
                algoKeys.forEach((key) => {
                  ...
                  const signalValue =
                    weightVal *
                    (1 + 0.2 * Math.sin(history.length * 0.05 + keySeed));
                  realAlgoScores[key] = Math.max(
                    0.01,
                    Math.min(1.0, Math.abs(signalValue)),
                  );
                });

                const { activations } = predictMultiHeadModel(
                  realAlgoScores,
                  liveWeights,
                );
                const { topDriver } = computeIntegratedGradients(
                  realAlgoScores,
                  liveWeights,
                );
                const wLoss = computeWassersteinSoftLoss(
                ...
                return (
                  <div className="mb-6">
                    <TensorBoardActivationGrid
                      ...
                      topDriverFeature={
                        LABELS_MAP[topDriver as AlgoKey] || topDriver
                      }
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                  <div>
                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                      ...
                      Cartographie d'ADN Positionnel ($5 \times N_{"{algos}"}$)
                    </h4>
                    <p className="text-[10px] text-slate-500 mt-1">
                      Estimations optimales par case de sortie (du premier au
                      dernier sortant)
                    </p>
                  </div>
                ...
                {calculatingPositional ? (
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {Object.keys(currentPosDNA)
                      .sort(
                        (a, b) =>
                          currentPosDNA[b as AlgoKey] -
                          currentPosDNA[a as AlgoKey],
                      )
                      .slice(0, 14)
                      .map((key) => {
                        ...
                        const isDominant =
                          weightVal > 1.2 / Object.keys(currentPosDNA).length;
                        const label = LABELS_MAP[key as AlgoKey] || key;

                        return (
                          <div
                            key={key}
                            className="bg-black/30 p-3 rounded-xl border border-white/5 flex flex-col justify-between hover:border-slate-800 transition-all"
                          >
                            ...
                            <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Math.max(2, weightVal * 500))}%`,
                                }}
                              ></div>
                            </div>
                          </div>
              ...
              <FirstPredictionDNASnapshotViewer
                snapshot={firstPredictionDNASnapshot}
                maxBalls={drawSpecifics.balls}
              />
              ...
              <div className="bg-slate-950 p-6 rounded-3xl border border-slate-900 shadow-xl flex flex-col lg:flex-row items-center gap-8 relative overflow-hidden min-w-0 w-full">
                ...
                <div className="w-full lg:w-1/2 relative z-10 min-w-0">
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Activity size={14} className="text-indigo-400" /> Mutation
                    de l'ADN Global
                  </h4>
                  <div className="w-full space-y-3 px-2">
                    {Object.entries(liveWeights)
                      .sort(([, a], [, b]) => (b as number) - (a as number))
                      .map(([key, val]) => {
                        const baseVal = originalWeights
                          ? originalWeights[
                              key as keyof typeof originalWeights
                            ] || 0
                          : (val as number);
                          ...
                          <div key={key} className="w-full">
                            <div className="flex justify-between items-center mb-1">
                              <span className="text-[10px] font-bold text-slate-400 capitalize">
                                {LABELS_MAP[key as keyof typeof LABELS_MAP] ||
                                  key}
                              </span>
                            ...
                            <div className="w-full h-1 bg-slate-800/50 rounded-full overflow-hidden relative">
                              {status !== "idle" && (
                                <div
                                  className="absolute top-0 left-0 h-full bg-slate-600 opacity-50"
                ...
                <div className="w-full lg:w-1/2 relative z-10 flex flex-col gap-4 min-w-0">
                  ...
                  <div className="bg-emerald-500/[0.04] p-4 rounded-xl border border-emerald-500/20 relative overflow-hidden">
                    ...
                    <div className="text-[10px] font-black text-emerald-400 uppercase mb-1">
                      Score Historique d'Alignement Evolué
                    </div>
                    <div className="text-3xl font-black text-emerald-400 flex items-center gap-2">
                      {finalReport
                        ? finalReport.score.toFixed(1) + "/100"
                        : "--"}
                      {improvement > 0 && (
                        <span className="text-xs bg-emerald-500 text-white px-2 py-0.5 rounded-lg">
                    ...
                    {finalReport?.confidence_intervals && (
                      <div className="mt-2 text-[10px] text-emerald-400 leading-relaxed font-mono">
                        <div className="flex justify-between border-t border-emerald-500/10 pt-1.5">
                          ...
                          <span>
                            {finalReport.averageHits.toFixed(1)} &plusmn;{" "}
                            {(
                              (finalReport.confidence_intervals.avgHits[1] -
                                finalReport.confidence_intervals.avgHits[0]) /
                              2
                            ).toFixed(1)}{" "}
                            hits/tirage
                          </span>
                        ...
                        {finalReport.mrr !== undefined && (
                          <div className="flex justify-between border-t border-emerald-500/10 pt-1.5 mt-1.5">
                            <span>MRR / NDCG:</span>
                            <span>
                              {finalReport.mrr.toFixed(3)} /{" "}
                              {finalReport.ndcg?.toFixed(3)}
                            </span>
                          </div>
                        )}
                        {finalReport.topologicalLoss !== undefined && (
                          <div className="flex justify-between border-t border-emerald-500/10 pt-1.5 mt-1.5">
                            ...
                            <span>
                              {finalReport.topologicalLoss.toFixed(3)}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="bg-[#05091a]/85 p-6 rounded-2xl shadow-xl border border-slate-850 text-center relative overflow-hidden">
                ...
                {status === "running" ? (
                  <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest animate-pulse">
                    Mutations stochastiques en cours...
                  </span>
                ) : status === "completed" ? (
                  <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">
                    Séquence stabilisée &amp; convergée
                  </span>
                ) : (
              ...
              <div className="bg-[#030712] p-5 rounded-2xl shadow-md border border-slate-800">
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <Cpu size={14} className="text-indigo-400" /> Télémétrie de
                  recalcul
                </h4>
                <div className="grid grid-cols-2 gap-3 font-mono">
                  <div className="p-3 bg-black/40 rounded-xl border border-white/5 text-center">
                    ...
                    <div className="text-base font-black text-indigo-400 mt-1">
                      {evolutionData.length > 0
                        ? (
                            evolutionData[evolutionData.length - 1].diversity *
                            100
                          ).toFixed(1) + "%"
                        : "100%"}
                    </div>
              ...
              <div className="p-5 bg-indigo-950/20 rounded-2xl border border-indigo-900/40">
                ...
                <p className="text-[10px] text-slate-400 leading-relaxed">
                  L'analyse est segmentée par position stochastique ordonnée.
                  Les décades ont un comportement asymétrique : le Numéro 1 est
                  majoritairement calibré par des indices à faible dérogeance
                  gauche (Gaps/Poisson), contrastant avec le Numéro 5 qui suit
                  d'autres corrélations de persistance.
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-400">
                      Simulation de Replay Déterministe
                    </h4>
                    <h2 className="text-xl md:text-2xl font-black text-white tracking-tight">
                      Mode "Simulation de Boucle"
                    </h2>
                    <p className="text-[10px] text-slate-400 mt-0.5 leading-relaxed max-w-xl">
                      Rejoue chronologiquement une tranche de tirages
                      historiques pour évaluer l'amélioration d'impact de
                      l'apprentissage forensic adaptatif contre des poids
                      statiques.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 self-stretch md:self-auto justify-between md:justify-start">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="text-xs font-mono font-black text-indigo-400 bg-indigo-950/40 border border-indigo-500/20 px-2 py-0.5 rounded-md">
                        {loopSize}
                      </span>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={
                      loopRunning
                        ? handleStopLoopSimulation
                        : handleStartLoopSimulation
                    }
                      ...
                      <table className="w-full text-left border-collapse font-mono text-[10px]">
                        <thead>
                          <tr className="bg-slate-950 border-b border-slate-800 text-slate-400 font-bold uppercase tracking-wider">
                            ...
                            <th className="p-3 text-center text-emerald-400">
                              Hits Feedback
                            </th>
                          </tr>
                        </thead>
                          ...
                          {loopResults.map((res, i) => (
                            ...
                            >
                              ...
                              <td className="p-3 text-[9px]">
                                {res.regime.replace("_", " ")}
                              </td>
                              <td className="p-3 text-center">
                                <div className="flex items-center justify-center gap-1">
                                  {res.actual.map((n: number) => {
                                    const hitInLoop =
                                      res.predictedLoop.includes(n);
                                    return (
                                      <span
                                        key={n}
                              ...
                              <td className="p-3 text-center font-black text-emerald-400 bg-emerald-500/[0.01]">
                                <div className="flex items-center justify-center gap-1">
                                  ...
                                  {res.hitsLoop > res.hitsStatic && (
                                    <span className="text-[8px] px-1 bg-emerald-500/10 rounded font-bold">
                                      ▲
                                    </span>
                                  )}
                                  {res.hitsLoop < res.hitsStatic && (
                                    <span className="text-[8px] px-1 bg-rose-500/10 rounded font-bold">
                                      ▼
                                    </span>
                                  )}
                                </div>
                        ...
                        <div className="space-y-4">
                          <div className="flex justify-between items-center">
                            <span className="text-[9px] font-bold text-slate-500 uppercase">
                              Hits Totaux (Statique)
                            </span>
                            <span className="font-mono font-bold text-slate-400">
                              {loopSummary.totalHitsStatic}
                            </span>
                          </div>
                          <div className="flex justify-between items-center border-b border-slate-900 pb-3">
                            <span className="text-[9px] font-bold text-slate-500 uppercase">
                              Hits Totaux (Feedback)
                            </span>
                            <span className="font-mono font-black text-emerald-400 text-base">
                              {loopSummary.totalHitsLoop}
                            </span>
                          </div>
                          <div className="pt-2 text-center">
                            ...
                            <div
                              className={`text-3xl font-black ${loopSummary.improvement >= 0 ? "text-emerald-400" : "text-rose-400"} tracking-tighter flex items-center justify-center gap-1.5`}
                            >
                              <span>
                                {loopSummary.improvement >= 0 ? "+" : ""}
                                {loopSummary.improvement}%
                              </span>
                              {loopSummary.improvement >= 0 ? (
                                <TrendingUp
                                  size={24}
                            ...
                            <p className="text-[9px] text-slate-400 leading-normal mt-2 max-w-xs mx-auto font-semibold">
                              Le feedback forensic continu permet d'isoler les
                              déviances et d'auto-ajuster l'influence de chaque
