                         ...
                         <div className="w-full lg:w-1/2 relative z-10 min-w-0">
                            ...
                            <div className="w-full space-y-3 px-2">
                                {Object.entries(liveWeights)
                                    ...
                                    .map(([key, val]) => {
                                    const baseVal = originalWeights ? originalWeights[key as keyof typeof originalWeights] || 0 : (val as number);
                                    const diff = (val as number) - baseVal;
                                    return (
                                    <div key={key} className="w-full">
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="text-[10px] font-bold text-slate-400 capitalize">
                                                {LABELS_MAP[key as keyof typeof LABELS_MAP] || key}
                                            </span>
                                            <div className="flex gap-2 text-[10px] font-mono">
                                                <span className="text-slate-500">{(baseVal * 100).toFixed(0)}%</span>
                                                <span className="text-indigo-400 font-bold">{((val as number) * 100).toFixed(0)}%</span>
                                                {diff !== 0 && (
