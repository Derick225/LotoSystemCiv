...
export const MetaAnalystTab: React.FC<MetaAnalystTabProps> = ({ drawName }) => {
    ...
    const runMetaAnalysis = async () => {
        ...
        try {
            ...
            if (isMounted.current) {
                ...
                showToast("Protocole Platinum activé.", "success");
            }
        } catch (e: any) {
    ...
    const handleAutoTune = () => {
        ...
        showToast("Configuration ADN optimisée.", "success");
    };

    const BIAS_LABELS: Record<string, { label: string, desc: string, icon: any }> = {
        stability: { label: "Inertie", desc: "Favorise les numéros récurrents.", icon: <ShieldCheck size={14}/> },
        chaos: { label: "Rupture", desc: "Favorise les grands écarts.", icon: <Zap size={14}/> },
        harmony: { label: "Vibration", desc: "Favorise les cycles stables.", icon: <Star size={14}/> },
        wavelet: { label: "Impulsion", desc: "Focus sur le court-terme.", icon: <Layers size={14}/> },
        orchestration: { label: "Symétrie", desc: "Focus sur les suites/voisins.", icon: <Dna size={14}/> }
    };
    ...
    if (nexusLoading || (loading && !result)) return (
        <div className="flex flex-col items-center justify-center min-h-[400px] gap-8 bg-slate-900/50 rounded-[3rem] p-10 animate-pulse border border-dashed border-indigo-500/20">
            ...
            <p className="text-indigo-400 font-black uppercase tracking-[0.3em] text-xs">Exécution du Protocole Platinum Fusion...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 w-full overflow-hidden">
            {/* Protocole Info Box */}
            <div className="bg-indigo-600/10 border border-indigo-500/20 p-4 rounded-3xl flex items-center gap-4">
                <div className="p-2 bg-indigo-600 rounded-xl text-white shadow-lg"><Filter size={18}/></div>
                <p className="text-[10px] md:text-xs text-indigo-800 dark:text-indigo-300 font-bold uppercase tracking-tight">
                    <strong>Protocole Actif :</strong> Synergies T-1 → Orchestrations → Tamisage IA
                </p>
            </div>
            ...
            <div className="bg-slate-900 text-white p-8 md:p-12 rounded-[3.5rem] shadow-2xl border border-white/5 relative overflow-hidden group">
                <div className="relative z-10 grid lg:grid-cols-12 gap-12 items-start">
                    <div className="lg:col-span-7 space-y-8">
                        <div>
                            <div className="px-3 py-1 bg-indigo-600 text-white text-[9px] font-black uppercase tracking-widest rounded-full w-fit mb-4">PLATINUM FUSION v8.5</div>
                            ...
                            <p className="text-slate-400 mt-4 text-sm font-medium leading-relaxed">
                                Ajustez les paramètres pour affiner le tamisage final. Le système a déjà extrait les synergies et orchestrations du dernier tirage.
                            </p>
                        </div>
                    ...
                    <div className="lg:col-span-5 space-y-6 sticky top-0">
                        <div className="bg-black/40 p-8 rounded-[3rem] border border-white/10 text-center">
                            ...
                            <p className="text-xs text-slate-500 mb-8">Lancez le tamisage sur le pool de synergies identifiées.</p>
                        ...
                        <div className="p-5 bg-amber-500/10 rounded-[2rem] border border-amber-500/20 flex gap-4">
                            <GitBranch size={18} className="text-amber-500 shrink-0" />
                            <p className="text-[10px] text-amber-300 font-medium leading-relaxed italic">
                                "Le protocole Orchestration permet de détecter les numéros qui vont sortir par 'écho' des synergies principales."
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {result && (
                <div className="space-y-6 animate-slide-up">
                    <div className="bg-indigo-600 p-8 rounded-[3rem] shadow-2xl relative overflow-hidden text-white">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                            <div>
                                ...
                                <p className="text-indigo-100 text-xs font-bold uppercase mt-1">Noyaux de convergence validés par le tamis</p>
                            </div>
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                                    ...
                                    <div className="flex items-center gap-8">
                                        <div className="text-right">
                                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Confiance IA</div>
                    ...
                    <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-800">
                        <h4 className="text-white font-black uppercase text-sm mb-4">Note de l'Analyste</h4>
