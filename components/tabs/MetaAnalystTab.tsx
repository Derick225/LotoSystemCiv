...
import { 
    ...
    Sliders, Settings, Play, CheckCircle2, History, Cpu
} from 'lucide-react';
import { 
...
export const MetaAnalystTab: React.FC<MetaAnalystTabProps> = ({ drawName }) => {
    ...
    const [regimePivot, setRegimePivot] = useState<number>(0.80);
    const [forensicGain, setForensicGain] = useState<number>(1.0);
    const [phaseFrequency, setPhaseFrequency] = useState<number>(1.0);
    const [shannonEntropyFilter, setShannonEntropyFilter] = useState<boolean>(false);
    ...
    const [backtestResults, setBacktestResults] = useState<{
        trials: number;
        scenariosStats: Record<string, { name: string; meanHits: number; successRate: number; totalHits: number }>;
        details: { drawDate: string; actualWinners: number[]; scenarioHits: Record<string, number> }[];
        bestScenario: string;
    } | null>(null);
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            audioEngine.play('error');
            showToast("Erreur Hyper-Convergence : " + (e instanceof Error ? e.message : String(e)), "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const runBacktest = async () => {
        ...
        if (history.length < 18) {
             showToast("Dataset insuffisant pour un backtest rétroactif (min. 18 tirages requis).", "error");
             return;
        }
        ...
        try {
            ...
            const scenarioAccumulator: Record<string, { name: string; totalHits: number; wins: number }> = {
            ...
            for (let j = trialsCount; j >= 1; j--) {
                ...
                const pred = await generatePlatinumPrediction(
                    ...
                    (progress, message) => {
                        ...
                        const totalProgress = Math.round(((trialIndex + progress / 100) / trialsCount) * 100);
                        setLoadingProgress(totalProgress);
                        setLoadingStep(`Backtest rétroactif ${trialIndex + 1}/${trialsCount} : ${message}`);
                    }
                );
            ...
            showToast(`Rétro-audit complété sur ${trialsCount} cycles de test stochastique.`, "success");
            audioEngine.play('success');
        } catch (err: any) {
    ...
    const handleSave = async (scenario: PlatinumScenario) => {
        ...
        if (result) {
            ...
            await savePredictionToHistory(drawName, predictionObj, undefined, {
                spectral, fractal, volatility: volatility ?? undefined, regularity
            });
        }
    ...
    if (nexusLoading || loading || isBacktesting) {
        return (
            <div className="max-w-4xl mx-auto p-4">
                <PredictionComputationOverlay
                    isComputing={true}
                    computingStep={loadingStep || (isBacktesting ? "Rétro-audit temporel..." : "Fusion des tenseurs probabilistes...")}
        ...
        <div className="space-y-6 animate-fade-in pb-20 w-full overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-slate-900 p-4 rounded-3xl border border-white/5 flex flex-col justify-between">
                    <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Cohérence</span>
                ...
                <div className="bg-slate-900 p-4 rounded-3xl border border-white/5 flex flex-col justify-between">
                    <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Entropie</span>
                    <div className="text-2xl font-black text-white flex items-center gap-2">
                ...
                <div className="bg-slate-900 p-4 rounded-3xl border border-white/5 flex flex-col justify-between">
                    <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Régime</span>
                ...
                <button onClick={runAnalysis} className="bg-indigo-600 hover:bg-indigo-500 rounded-3xl flex flex-col items-center justify-center text-white transition-colors group">
                    <RefreshCw size={20} className="mb-1 group-hover:rotate-180 transition-transform duration-300"/>
                    <span className="text-xs font-black uppercase tracking-widest">Re-Scan</span>
                </button>
            </div>
                ...
                <div className="flex justify-between items-center mb-8 px-2">
                    <div>
                        <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                            <BarChart3 className="text-indigo-400" size={12} /> Spectre de Probabilité
                        </h3>
                        <span className="text-2xl sm:text-3xl font-black text-white tracking-tighter">Hyper-Spectre Harmonique</span>
                    </div>
                    {hoveredIndex !== null && (
                        <div className="flex items-center gap-2.5 bg-indigo-500/10 px-4 py-1.5 rounded-full border border-indigo-500/20 shadow-inner animate-fade-in">
                            <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Vecteur {hoveredIndex}</span>
                            <span className="text-sm font-black text-emerald-400">{spectrumData[hoveredIndex-1]?.v}%</span>
                        </div>
                    )}
                </div>
            ...
            <div id="platinum-calibration-panel" className="bg-slate-900/60 backdrop-blur-md rounded-3xl p-6 border border-white/5 space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-1">
                            Ajustez l'algorithme hyper-convergé et lancez un rétro-audit temporel pour calibrer vos tickets.
                        </p>
                    </div>
                    <button 
                        onClick={() => { audioEngine.play('click'); setShowCalibration(!showCalibration); }}
                    ...
                    >
                        <Settings size={12} className={showCalibration ? 'rotate-90 transition-transform' : 'transition-transform'} />
                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-wider">
                                            <span className="text-slate-400 flex items-center gap-1.5">
                                                <Cpu size={12} className="text-indigo-400" /> Seuil Pivot de Régime
                                            </span>
                                            <span className="text-indigo-400 font-mono">{regimePivot.toFixed(2)}</span>
                                        </div>
                                        <input 
                                            type="range"
                                            min="0.60"
                                        ...
                                        <p className="text-[9px] text-slate-500 leading-normal">
                                            Seuil d'inflexion stochastique. Détermine la limite de sensibilité entre les régimes ordonnés et chaotiques.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-wider">
                                            <span className="text-slate-400 flex items-center gap-1.5">
                                                <History size={12} className="text-violet-400" /> Gain Rétroactif Forensic
                                            </span>
                                            <span className="text-violet-400 font-mono">{forensicGain.toFixed(1)}x</span>
                                        </div>
                                        <input 
                                            type="range"
                                            min="0.1"
                                        ...
                                        />
                                        <p className="text-[9px] text-slate-500 leading-normal">
                                            Intensité de correction d'erreurs. Multiplie la rétroaction des dérives et manques historiques.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-wider">
                                            <span className="text-slate-400 flex items-center gap-1.5">
                                                <Activity size={12} className="text-rose-400" /> Phase Trigonométrique
                                            </span>
                                            <span className="text-rose-400 font-mono">{phaseFrequency.toFixed(1)} rad</span>
                                        </div>
                                        <input 
                                            type="range"
                                            min="0.2"
                                            ...
                                            onChange={(e) => setPhaseFrequency(Number(e.target.value))}
                                            className="w-full accent-rose-500 bg-slate-800 h-1.5 rounded-lg cursor-pointer"
                                        />
                                        <p className="text-[9px] text-slate-500 leading-normal">
                                            Dispersion orbitale des scénarios. Modifie le décalage fréquentiel angulaire pour éviter les chevauchements.
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-slate-800/50 rounded-2xl border border-white/5">
                                        <div>
                                            ...
                                            <span className="text-[9px] text-slate-500 mt-0.5 block leading-normal max-w-xs">
                                                Élimine de façon différentielle les bruits blancs sous le spectre d'énergie moyenne.
                                            </span>
                                        </div>
                                        <button 
                                            onClick={() => { audioEngine.play('click'); setShannonEntropyFilter(!shannonEntropyFilter); }}
                            ...
                            <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-white/5 justify-end">
                                ...
                                >
                                    {isBacktesting ? (
                                        <>
                                            <RefreshCw size={12} className="animate-spin" /> Audit rétrograde...
                                        </>
                                    ) : (
                                        <>
                                            <History size={12} className="text-violet-400" /> Rétro-Audit Temporel
                                        </>
                                    )}
                                </button>
                                <button
                    ...
                    {backtestResults && (
                        ...
                        >
                            <div className="flex justify-between items-center border-b border-white/5 pb-3">
                                <div>
                                    <h4 className="text-[10px] font-black text-violet-400 uppercase tracking-widest flex items-center gap-1.5">
                                        <CheckCircle2 size={12} /> Diagnostic de l'Audit Temporel (Time Machine)
                                    </h4>
                                    <p className="text-[10px] text-slate-400 mt-0.5">
                                        Rapport d'autopsie stochastique simulé sur les <strong>{backtestResults.trials}</strong> derniers tirages réels.
                                    </p>
                                </div>
                                <button 
                                    onClick={() => { audioEngine.play('click'); setBacktestResults(null); }}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {Object.entries(backtestResults.scenariosStats).map(([id, stats]: any) => (
                                    <div key={id} className="p-4 bg-slate-900 rounded-xl border border-white/5 space-y-2">
                                        <div className="flex justify-between items-start">
                                            <span className="text-[11px] font-black text-white uppercase truncate max-w-[130px]">{stats.name}</span>
                                            <span className="text-[10px] font-mono text-slate-500">Succès: <strong className="text-emerald-400">{stats.successRate}%</strong></span>
                                        </div>
                                        <div className="space-y-1">
                                            <div className="flex justify-between text-[9px] font-mono text-slate-400">
                                                <span>Ratio Hits moyen</span>
                                                <span className="text-slate-300">{stats.meanHits} / 5</span>
                                            </div>
                                            {/* Progress Hits Bar */}
                                        ...
                                        <span className="text-[9px] font-mono text-slate-500 block">
                                            Nombre total de hits : <strong className="text-indigo-400">{stats.totalHits}</strong>
                                        </span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="p-3 bg-indigo-500/5 rounded-xl border border-indigo-500/15 flex items-center justify-between text-[10px]">
                                <span className="text-slate-400">Scénario de résonance optimal identifié :</span>
                                <span className="font-black text-emerald-400 uppercase tracking-wider">{backtestResults.bestScenario}</span>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            <AnimatePresence>
                {selectedScenario && (
                    ...
                    >
                        <div id={`scenario-xray-${selectedScenario.id}`} className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 md:p-10 border border-slate-200/60 dark:border-slate-800 shadow-xl shadow-slate-200/20 dark:shadow-none relative">
