...
export const SpectralTab: React.FC<{ drawName: string }> = ({ drawName }) => {
  ...
  const highWavelet = useMemo(() => {
    return [...wavelet].sort((a, b) => b.energy - a.energy).slice(0, 5);
  }, [wavelet]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-6 animate-pulse">
        ...
        <p className="text-xs font-black uppercase tracking-[0.3em] text-slate-400">
          Scan des potentiels énergétiques...
        </p>
      </div>
    );
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex-1">
            <h3 className="text-3xl md:text-4xl font-black tracking-tighter mb-4 flex items-center gap-3">
              <Zap className="text-amber-400" fill="currentColor" /> État de
              Charge des Numéros
            </h3>
            <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-xl">
              Plus la jauge est pleine, plus le numéro a accumulé une tension
              cyclique. Ces numéros sont statistiquement "dus" et cherchent à
              décharger leur énergie en sortant au tirage.
            </p>
          </div>
          <div className="bg-white/5 p-6 rounded-3xl border border-white/10 text-center min-w-[200px]">
            <div className="text-amber-400 font-black text-4xl mb-1">
              {highEnergy[0]?.energy || 0}%
            </div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">
              Potentiel de Charge Max
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card neural-border p-6 rounded-3xl shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center gap-3">
              ...
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                  Facteur Périodique (T)
                </div>
                <div className="text-sm font-mono font-black text-slate-900 dark:text-amber-400">
                  {dominantPeriod} Cycles
                </div>
              </div>
            </div>
          </div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs font-medium leading-relaxed">
            Cette synthèse traite l'historique non pas comme des événements
            isolés, mais comme des ondes de probabilité superposées. En
            extrayant les principaux vecteurs d'activations par{" "}
            <strong>Décomposition en Valeurs Singulières (SVD)</strong>, nous
            obtenons les modes spectraux majeurs (ondes stationnaires). Leurs
            fréquences dominantes calibrent continûment le facteur périodique
            des algorithmes spectraux.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between gap-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  Mode Ondulatoire 1 (Ψ1)
                </span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 font-black uppercase">
                  Principal
                </span>
              </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between gap-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  Mode Ondulatoire 2 (Ψ2)
                </span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-purple-500/10 text-purple-500 dark:text-purple-400 font-black uppercase">
                  Secondaire
                </span>
              </div>
              <div className="space-y-1">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-sm font-black font-mono text-slate-900 dark:text-white">
                    {svdModes[1]?.toFixed(1)}%
                  </span>
                </div>
                <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-900 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-purple-500 to-purple-400 rounded-full"
                    style={{ width: `${svdModes[1]}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between gap-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  Mode Ondulatoire 3 (Ψ3)
                </span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-pink-500/10 text-pink-500 dark:text-pink-400 font-black uppercase">
                  Harmonique
                </span>
              </div>
              <div className="space-y-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
                {highWavelet.map((w, index) => {
                  ...
                  return (
                    <div
                      key={w.number}
                      className={`p-4 rounded-2xl border flex flex-col justify-between gap-3 transition-all duration-300 ${isResonant ? "bg-amber-500/10 border-amber-500/40" : "bg-slate-950/40 border-slate-800"}`}
                    >
                      <div className="flex justify-between items-center">
                        ...
                        {isResonant && (
                          <span className="text-[8px] px-1.5 py-0.5 rounded bg-amber-400 text-slate-950 font-black uppercase">
                            Résonance
                          </span>
                        )}
                      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {highEnergy.map((m) => {
          ...
          const saturationRatio =
            1 /
            (1 + Math.exp(-(m.energy - energyStats.mean) / energyStats.stdDev));
          ...
          return (
            <div
              key={m.number}
              className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col gap-6 group hover:border-indigo-500/50 transition-all duration-300"
            >
              <div className="flex justify-between items-center">
