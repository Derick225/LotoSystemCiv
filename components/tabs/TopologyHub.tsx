            ...
            <div className="relative z-20 bg-nexus-950 py-2 -mx-4 px-4 md:mx-0 md:px-0 md:bg-transparent">
                <div className="overflow-x-auto scrollbar-hide pb-2 mask-fade-right">
                    <div className="flex bg-slate-100 dark:bg-slate-800/80 p-1 rounded-2xl md:rounded-2xl w-max border border-slate-200 dark:border-slate-700 shadow-inner">
                        {[
                            { id: 'spatial', label: 'Géométrie', icon: Grid, color: 'text-indigo-500' },
                            { id: 'neural', label: 'Architecture', icon: Network, color: 'text-purple-500' },
                            { id: 'synergy', label: 'Synergie', icon: Users, color: 'text-blue-500' },
                            { id: 'decision', label: 'Décision', icon: GitBranch, color: 'text-emerald-500' },
                            { id: 'combinations', label: 'Architecte', icon: Calculator, color: 'text-rose-500' },
                            { id: 'python', label: 'Deep Kernel', icon: Terminal, color: 'text-emerald-400' }
                        ].map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => {
                                    audioEngine.play("click");
                                    setSubTab(tab.id as 'spatial' | 'synergy' | 'decision' | 'combinations' | 'python' | 'neural');
                                }}
                                className={`
                                    ...
                                    ${subTab === tab.id
                                        ? tab.id === 'python' ? 'bg-emerald-600 text-white shadow-xl scale-105 z-10 ring-1 ring-black/5 dark:ring-white/10' : 'bg-white dark:bg-slate-700 shadow-lg scale-105 z-10 text-slate-800 dark:text-white ring-1 ring-black/5 dark:ring-white/10'
                                        : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'
                                    }
                                `}
                            >
                                <tab.icon size={14} className={subTab === tab.id ? tab.id === 'python' ? 'text-white' : tab.color : ''} />
                                <span>{tab.label}</span>
                            </button>
                        ))}
                    </div>
