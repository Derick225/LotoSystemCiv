...
import { Terminal, Play, Activity, BarChart2, CheckCircle, RefreshCw, Code, Copy, Sliders, GitCompare } from 'lucide-react';
...
const CodeCell: React.FC<{ content: string, onExecute?: () => void, isExecuting?: boolean }> = ({ content, onExecute, isExecuting }) => (
    <div className="bg-[#0d1117] rounded-xl border border-slate-700 overflow-hidden mb-4 shadow-lg group">
        <div className="flex items-center justify-between px-4 py-2 bg-[#161b22] border-b border-slate-700">
            <span className="text-xs font-mono text-slate-400 font-bold flex items-center gap-2"><Code size={12}/> python_kernel_v14.py</span>
...
export const PythonAnalystTab: React.FC<{ drawName: string }> = ({ drawName }) => {
    ...
    const [activeSidebarTab, setActiveSidebarTab] = useState<'mc' | 'importance' | 'interactions'>('mc');
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            setStatus('idle');
            setLogs(prev => [...prev, `! [FATAL] ${(e instanceof Error ? e.message : String(e))}`]);
        }
    };
            ...
            <div className="flex justify-between items-center bg-slate-900 p-4 rounded-[2rem] border border-slate-800 shadow-lg">
                <div className="flex items-center gap-4 px-2">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-mono">Kernel: Python 3.11 (WASM/XGBoost Mode)</p>
                    </div>
                </div>
                <button 
                    onClick={runAnalysis}
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar" ref={scrollRef}>
                        ...
                        {status === 'idle' && logs.length < 5 && (
                            <div className="flex flex-col items-center justify-center h-40 opacity-30">
                                ...
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Prêt à exécuter l'Analyse XGBoost</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl flex-1 flex flex-col min-h-[300px]">
                        <div className="flex justify-between items-center mb-4">
                            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                <BarChart2 size={14} className="text-indigo-500"/> Analyses Diagnostics
                            </h4>
                        </div>
                        ...
                        {result && (
                            <div className="grid grid-cols-3 bg-slate-950 p-1 rounded-xl border border-white/5 mb-4">
                                <button
                                    onClick={() => { audioEngine.play('click'); setActiveSidebarTab('mc'); }}
                                ...
                                <button
                                    onClick={() => { audioEngine.play('click'); setActiveSidebarTab('importance'); }}
                                ...
                                <button
                                    onClick={() => { audioEngine.play('click'); setActiveSidebarTab('interactions'); }}
                                ...
                                {activeSidebarTab === 'mc' && (
                                    <div className="flex-1 w-full relative">
                                        <div className="absolute top-1 left-2 text-[8px] font-mono font-bold text-emerald-500 uppercase flex items-center gap-1">
                                            <Activity size={10} className="animate-pulse" /> Densité du Trajet Multi-Agent
                                        </div>
                                ...
                                {activeSidebarTab === 'importance' && (
                                    <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar max-h-[260px] pr-1">
                                        <div className="text-[8px] font-mono font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-1 mb-2">
                                            <Sliders size={10} /> Coefficients d'Importance XGBoost (Information Gain)
                                        </div>
                                        {result.featureImportances ? (
                                            result.featureImportances.map((imp, idx) => (
                                                <div key={idx} className="space-y-1">
                                                    <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wide">
                                                        ...
                                                        <span className="font-mono text-indigo-400">{imp.importance.toFixed(1)}%</span>
                                                    </div>
                                            ...
                                            <p className="text-[10px] text-slate-500 text-center font-mono py-8">Aucun indicateur d'importance disponible.</p>
                                        )}
                                    </div>
                                )}
                                ...
                                {activeSidebarTab === 'interactions' && (
                                    <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar max-h-[260px] pr-1">
                                        <div className="text-[8px] font-mono font-bold text-fuchsia-400 uppercase tracking-widest flex items-center gap-1 mb-2">
                                            <GitCompare size={10} /> Synergie Non-linéaire des Caractéristiques (H-Statistic)
                                        </div>
                                        {result.featureInteractions ? (
                                            result.featureInteractions.map((inter, idx) => (
                                                <div key={idx} className="bg-slate-950/80 p-3 rounded-xl border border-white/5 space-y-2">
                                                    <div className="flex justify-between text-[9px] font-black tracking-wide uppercase">
                                                        <div className="flex items-center gap-1.5 text-slate-300">
                                                            <span className="text-white font-semibold">{inter.f1.replace('Historique', '').replace('Fractal', '')}</span>
                                                            <span className="text-slate-500">×</span>
                                                            <span className="text-indigo-400 font-semibold">{inter.f2.replace('Historique', '').replace('Fractal', '')}</span>
                                                        </div>
                                                        <span className="font-mono text-emerald-400">{inter.strength.toFixed(2)}%</span>
                                                    </div>
                                            ...
                                            <p className="text-[10px] text-slate-500 text-center font-mono py-8">Aucune combinaison interactive repérée.</p>
                                        )}
                                    </div>
                                )}
                            </div>
                    ...
                    {result && (
                        <div className="bg-emerald-900/10 p-6 rounded-2xl border border-emerald-500/20 animate-scale-in">
                            ...
                            <div className="mt-4 pt-4 border-t border-emerald-500/10 flex justify-between items-center">
                                <span className="text-xs font-bold text-emerald-700 dark:text-emerald-300">P-Value: {result.findings.p_value.toExponential(3)}</span>
