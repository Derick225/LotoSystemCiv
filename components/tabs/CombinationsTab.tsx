...
import { Calculator, Zap, Ghost, Terminal, Network, Edit3, Cpu, Save, Lock, Unlock, Layers, ShieldCheck, RefreshCw } from 'lucide-react';
...
export const CombinationsTab: React.FC<CombinationsTabProps> = ({ drawName }) => {
    ...
    const [inputs, setInputs] = useState<{val: string, isBanker: boolean}[]>(Array(12).fill({val: '', isBanker: false})); 
        ...
        try {
            ...
            if (systemType === 'full') {
                ...
                if (uniqueBankers.length > 0) {
                     ...
                     const subTickets = generateAbbreviatedWheel(subPool, [], subSize, guarantee === 5 ? subSize : Math.max(2, guarantee - uniqueBankers.length));
                     baseTickets = subTickets.map(t => [...uniqueBankers, ...t].sort((a,b)=>a-b));
                } else {
            ...
            for (let i = 0; i < baseTickets.length; i += CHUNK_SIZE) {
                ...
                const processedChunk = chunk.map(t => {
                    ...
                    nexusScore = Math.min(100, Math.round((nexusScore * harmonicPass) / (wSpectral + wChaos + 0.5)));
            ...
            console.error(e);
            audioEngine.play('error');
            showToast("Erreur critique : " + (e instanceof Error ? e.message : String(e)), "error"); 
        } finally { 
            setIsGenerating(false); 
            setGenProgress(100);
        }
            ...
            {mode === 'manual' ? (
                ...
                <>
                    ...
                    <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-2xl border border-slate-800 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:rotate-12 transition-transform duration-300"><Ghost size={140} /></div>
                        ...
                        <div className="relative z-10 grid lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-8">
                                <div>
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <h3 className="text-sm font-black uppercase tracking-[0.4em] text-indigo-400">Architecte v3.0</h3>
                                    </div>
                                    ...
                                    <p className="text-xs text-slate-400 mt-2 font-mono flex items-center gap-2">
                                        ...
                                        Moteur Combinatoire : {nexusLoading ? 'Initialisation...' : 'Prêt'}
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {acoPaths.slice(0, 3).map((path, i) => (
                                            ...
                                            >
                                                <div className="w-6 h-6 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center text-xs font-black">#{i+1}</div>
                                                <div className="flex gap-1">
                                                    {path.numbers.map(n => (
                                                        <span key={n} className="w-5 h-5 rounded-md bg-black/40 flex items-center justify-center text-xs font-bold text-white">{n}</span>
                                                    ))}
                                                </div>
                                            </button>
                                        ))}
                                        {acoPaths.length === 0 && <div className="text-xs text-slate-500 italic">En attente des fourmis...</div>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-black/40 p-6 rounded-2xl border border-white/5 flex flex-col justify-between h-full min-h-[200px]">
                                <div className="flex items-center gap-3 mb-4 border-b border-white/5 pb-2">
                                    ...
                                    <span className="text-xs font-black uppercase text-slate-500 tracking-widest">Logs Système</span>
                                </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                <div className="flex justify-between items-end">
                                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pool de Numéros</h4>
                                        ...
                                        <div key={idx} className="relative group">
                                            ...
                                            {input.val && (
                                                <button 
                                                    ...
                                                    `}
                                                    title={input.isBanker ? "Base (Verrouillé)" : "Définir comme Base"}
                                                >
                                                    {input.isBanker ? <Lock size={10} fill="currentColor" /> : <Unlock size={10} />}
                                                </button>
                                            )}
                                        </div>
                                    ))}
                                ...
                                <p className="text-xs text-slate-400 italic">
                                    <Lock size={10} className="inline mb-0.5 mr-1"/> Cliquez sur le cadenas pour définir une "Base" (Numéro fixe).
                                </p>
                            </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex gap-4">
                                        <div className="flex-1 space-y-2">
                                            ...
                                            <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl">
                                                <button onClick={() => setSystemType('full')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${systemType === 'full' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-white' : 'text-slate-500'}`}>Intégral</button>
                                                <button onClick={() => setSystemType('reduced')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${systemType === 'reduced' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-white' : 'text-slate-500'}`}>Réduit</button>
                                            </div>
                                        </div>
                                        {systemType === 'reduced' && (
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl">
                                                    {[3,4,5].map(g => (
                                                        <button key={g} onClick={() => setGuarantee(g as 3|4|5)} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${guarantee === g ? 'bg-white dark:bg-slate-700 shadow text-emerald-500' : 'text-slate-500'}`}>{g}/5</button>
                                                    ))}
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-2"><Layers size={12}/> Filtres Avancés</span>
                                            <div className={`w-2 h-2 rounded-full ${useHarmonicFilter ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                                        </div>
                        ...
                        <div className="space-y-4 animate-slide-up">
                            <div className="flex items-center justify-between px-2">
                                <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                    <ShieldCheck size={14} className="text-emerald-500"/> Résultats Optimisés
                                </h4>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {generatedTickets.slice(0, 50).map((t, idx) => (
                                    <motion.div 
                                        key={t.id}
                                    ...
                                    >
                                        <div className="flex justify-between items-center relative z-10">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black border ${t.nexusScore > 80 ? 'bg-emerald-100 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                                                    {t.nexusScore}
                                                </div>
                                                <div className="flex gap-1">
                                                    {t.numbers.map(n => (
                                                        <span key={n} onClick={(e) => { e.stopPropagation(); useNexusStore.getState().setInspectingNumber(n); }} className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-700 dark:text-slate-300 group-hover:bg-white dark:group-hover:bg-slate-800 transition-colors cursor-pointer hover:scale-110">
                                                            {n}
                                                        </span>
                                                    ))}
                                                </div>
                                            ...
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleSaveTicket(t); }}
                                        ...
                                        {expandedTicketId === t.id && (
                                            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 animate-fade-in">
                                                <TicketXRay numbers={t.numbers} score={t.nexusScore} showTitle={false} />
                                            </div>
                                        )}
                                    </motion.div>
                                ))}
