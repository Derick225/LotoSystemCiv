...
import { 
    Cpu, Zap, Brain, Hexagon, ArrowDown, Save, RefreshCw, 
    Layers, GitMerge, Activity, Network, Sliders, Info, 
    Settings, HelpCircle, CheckCircle2, Calculator, Play 
} from 'lucide-react';
...
export const ConvergenceTab: React.FC<{ drawName: string }> = ({ drawName }) => {
    ...
    const [selectionMethod, setSelectionMethod] = useState<'map' | 'balanced' | 'harmonic_consensus'>('map');
    ...
    }, [history, stats, spectral, lastPrediction, globalWeights, biasLogic, biasPhysics, biasIntuition, selectionMethod, showToast]);
    ...
    const handleSave = async () => {
        ...
        fusionResult.finalTicket.forEach(num => {
            const convergedData = fusionResult.convergedNumbers.find(cn => cn.number === num);
    ...
    const runBacktest = () => {
        ...
        setTimeout(() => {
            ...
            const hits = result.finalTicket.filter(n => lastActualDraw.gagnants.includes(n));
            
            setBacktestResult({
                hits,
                accuracy: Math.round((hits.length / lastActualDraw.gagnants.length) * 100),
                simulatedTicket: result.finalTicket,
                actualGagnants: lastActualDraw.gagnants
            });
            setIsBacktesting(false);
            audioEngine.play('success');
            showToast(`Backtest calibré : ${hits.length} hits identifiés sur le tirage précédent.`, "success");
        }, 1200);
    };
        ...
        <div className="space-y-10 animate-fade-in pb-20 w-full overflow-hidden">
            ...
            <div className="bg-slate-900 border border-indigo-500/20 p-8 rounded-3xl shadow-2xl relative overflow-hidden text-center group">
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase mb-4 drop-shadow-lg">
                        Synthèse <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Vectorielle</span>
                    </h2>
                    <p className="text-slate-400 text-xs md:text-sm font-medium max-w-xl mx-auto mb-8 leading-relaxed">
                        Le moteur de synthèse agrège les signaux <strong className="text-emerald-400">Logiques</strong> (Algorithmes), <strong className="text-purple-400">Physiques</strong> (Spectral) et <strong className="text-amber-400">Intuitifs</strong> (Oracle) pour modéliser une topologie absolue d'équilibrage, unique à votre ADN.
                    </p>
                ...
                <div className="lg:col-span-2 bg-slate-900/60 p-6 md:p-8 rounded-3xl border border-white/5 space-y-6">
                    <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-black uppercase tracking-widest text-white">Rétro-Ajustement de l'ADN de Fusion</h3>
                        </div>
                        <button 
                            onClick={resetBiases}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2 p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 transition-colors hover:border-emerald-500/20">
                            <div className="flex justify-between items-center text-xs font-bold">
                                ...
                                <span className="font-mono text-emerald-300">{biasLogic.toFixed(1)}x</span>
                            </div>
                            <input 
                                type="range" 
                                min="0.2" 
                            ...
                            <p className="text-[10px] text-slate-500 leading-relaxed">
                                Module l'impact des estimateurs fréquentiels, d'historiques bruts, de momentums et de tendances de fond.
                            </p>
                        </div>
                        ...
                        <div className="space-y-2 p-4 bg-purple-500/5 rounded-2xl border border-purple-500/10 transition-colors hover:border-purple-500/20">
                            <div className="flex justify-between items-center text-xs font-bold">
                                ...
                                <span className="font-mono text-purple-300">{biasPhysics.toFixed(1)}x</span>
                            </div>
                            <input 
                                type="range" 
                                min="0.2" 
                            ...
                            />
                            <p className="text-[10px] text-slate-500 leading-relaxed">
                                Augmente ou diminue l'importance des oscillations d'ondes temporelles et des spectres résonnants continus.
                            </p>
                        </div>
                        ...
                        <div className="space-y-2 p-4 bg-amber-500/5 rounded-2xl border border-amber-500/10 transition-colors hover:border-amber-500/20">
                            <div className="flex justify-between items-center text-xs font-bold">
                                ...
                                <span className="font-mono text-amber-300">{biasIntuition.toFixed(1)}x</span>
                            </div>
                            <input 
                                type="range" 
                                min="0.2" 
                            ...
                            />
                            <p className="text-[10px] text-slate-500 leading-relaxed">
                                Module l'influence des signatures markoviennes, d'affinités croisées et des prévisions du réseau cybernétique principal.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 border border-white/5 p-6 md:p-8 rounded-3xl flex flex-col justify-between">
                    <div>
                        <div className="flex items-center gap-2 border-b border-white/5 pb-4 mb-4">
                            ...
                            <h3 className="text-sm font-black uppercase tracking-widest text-white">Validation Rétro-Chrono</h3>
                        </div>
                        <p className="text-slate-400 text-xs leading-relaxed mb-6">
                            Permet d'évaluer l'efficacité de vos configurations en masquant virtuellement le tirage le plus récent et en mesurant si la fusion l'aurait correctement intercepté.
                        </p>
                            ...
                            {backtestResult ? (
                                ...
                                >
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-slate-400 font-bold">Précision simulée :</span>
                                        <span className={`font-black uppercase tracking-wider ${backtestResult.hits.length > 0 ? "text-emerald-400" : "text-amber-400"}`}>
                                            {backtestResult.hits.length} hits ({backtestResult.accuracy}%)
                                        </span>
                                    </div>
                                    
                                    <div className="space-y-1">
                                        <div className="text-[10px] font-bold text-slate-500 uppercase">Ticket Proposé</div>
                                    ...
                                    <div className="space-y-1 pt-1 border-t border-white/5">
                                        <div className="text-[10px] font-bold text-slate-500 uppercase">Tirage Réel</div>
                                        <div className="flex gap-1.5">
                    ...
                    >
                        {isBacktesting ? <RefreshCw size={14} className="animate-spin" /> : <Play size={14} />}
                        {isBacktesting ? 'Calibration...' : 'Simuler sur le Tirage Précédent'}
                    </button>
                </div>
            </div>
            ...
            <div className="bg-slate-950 border border-white/5 rounded-3xl p-6 relative overflow-hidden h-[300px] flex items-center justify-center">
                <div className="absolute top-4 left-6 flex items-center gap-1.5">
                    ...
                    <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Matrice de flux synaptique multi-capteurs</h4>
                </div>
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M 120 70 Q 250 150 450 150" fill="none" stroke={step === 1 ? "url(#emit-green)" : "#334155"} strokeWidth={step === 1 ? 3 : 1.5} className={step === 1 ? "animate-pulse" : ""} />
                    <path d="M 120 150 L 450 150" fill="none" stroke={step === 2 ? "url(#emit-purple)" : "#334155"} strokeWidth={step === 2 ? 3 : 1.5} className={step === 2 ? "animate-pulse" : ""} />
                    <path d="M 120 230 Q 250 150 450 150" fill="none" stroke={step === 3 ? "url(#emit-amber)" : "#334155"} strokeWidth={step === 3 ? 3 : 1.5} className={step === 3 ? "animate-pulse" : ""} />

                    {/* Sensor dots flying along path if fusing */}
                    {isFusing && (
                        <>
                            <circle r="4" fill="#10b981" filter="url(#glow-heavy)">
                                <animateMotion dur="1s" repeatCount="indefinite" path="M 120 70 Q 250 150 450 150" />
                            </circle>
                            <circle r="4" fill="#a855f7" filter="url(#glow-heavy)">
                                <animateMotion dur="0.9s" repeatCount="indefinite" path="M 120 150 L 450 150" />
                            </circle>
                            <circle r="4" fill="#f59e0b" filter="url(#glow-heavy)">
                                <animateMotion dur="1.1s" repeatCount="indefinite" path="M 120 230 Q 250 150 450 150" />
                            </circle>
                        </>
                    )}
                </svg>
                ...
                <div className="absolute left-6 inset-y-0 flex flex-col justify-center gap-10">
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all duration-300 ${step === 1 || fusionResult ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/40 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'bg-slate-900 text-slate-500 border-white/5'}`}>
                            <Cpu size={18} className={step === 1 ? "animate-bounce" : ""} />
                        </div>
                        <div className="text-left select-none">
                            <span className="text-[10px] text-slate-500 font-black tracking-widest block">LOGIQUE</span>
                            <span className="text-xs font-mono text-slate-300 font-bold">{biasLogic.toFixed(1)}x</span>
                        </div>
                    </div>

                    {/* Physique */}
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all duration-300 ${step === 2 || fusionResult ? 'bg-purple-500/10 text-purple-400 border-purple-500/40 shadow-[0_0_20px_rgba(168,85,247,0.3)]' : 'bg-slate-900 text-slate-500 border-white/5'}`}>
                            <Zap size={18} className={step === 2 ? "animate-bounce" : ""} />
                        </div>
                        <div className="text-left select-none">
                            <span className="text-[10px] text-slate-500 font-black tracking-widest block">PHYSIQUE</span>
                            <span className="text-xs font-mono text-slate-300 font-bold">{biasPhysics.toFixed(1)}x</span>
                        </div>
                    </div>

                    {/* Intuition */}
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all duration-300 ${step === 3 || fusionResult ? 'bg-amber-500/10 text-amber-400 border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.3)]' : 'bg-slate-900 text-slate-500 border-white/5'}`}>
                            <Brain size={18} className={step === 3 ? "animate-bounce" : ""} />
                        </div>
                        <div className="text-left select-none">
                            <span className="text-[10px] text-slate-500 font-black tracking-widest block">INTUITION</span>
                            <span className="text-xs font-mono text-slate-300 font-bold">{biasIntuition.toFixed(1)}x</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="absolute right-12 flex flex-col items-center">
                    <div className={`w-28 h-28 rounded-full flex flex-col items-center justify-center border transition-all duration-1000 relative ${isFusing ? 'bg-indigo-500/20 border-indigo-400 shadow-[0_0_60px_rgba(99,102,241,0.5)] scale-110' : fusionResult ? 'bg-indigo-950/40 border-indigo-500/60 shadow-[0_0_30px_rgba(99,102,241,0.2)]' : 'bg-slate-900 border-white/5'}`}>
                        ...
                        <Hexagon size={32} className={`text-indigo-400 ${isFusing ? "animate-pulse" : ""}`} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6 relative">
                ...
                {/* Python Node */}
                <div className={`p-4 rounded-2xl border transition-all duration-500 relative overflow-hidden text-center flex flex-col justify-between min-h-[140px] shadow-sm transform-gpu ${step === 1 || (fusionResult && selectionMethod === 'map' && biasLogic > 1.0) ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500/40 shadow-[0_0_30px_rgba(16,185,129,0.15)] scale-105 z-10' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 opacity-60 hover:opacity-100 hover:scale-[1.02]'}`}>
                    {step === 1 && <div className="absolute inset-x-0 bottom-0 h-1 bg-emerald-500 animate-[shimmer_2s_infinite]"></div>}
                    <div className="mt-2 relative z-10">
                        <div className={`w-10 h-10 rounded-2xl mx-auto mb-3 flex items-center justify-center transition-colors ${step === 1 || fusionResult ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200 dark:bg-slate-800 text-slate-400'}`}>
                            <Cpu size={20} className={step === 1 ? 'animate-pulse' : ''} />
                        </div>
                        <h3 className={`text-xs md:text-[10px] font-black uppercase tracking-widest ${step === 1 || fusionResult ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500'}`}>Logique (Python)</h3>
                    </div>
                    <div className="flex flex-wrap items-center gap-1 justify-center pb-2 relative z-10">
                        {fusionResult ? fusionResult.sources.python.slice(0,3).map(n => <span key={n} className="px-1.5 py-0.5 rounded-md bg-emerald-500/10 text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400">{n}</span>) : <span className="text-xs text-slate-400 italic">--</span>}
                    </div>
                </div>

                {/* Quantum Node */}
                <div className={`p-4 rounded-2xl border transition-all duration-500 relative overflow-hidden text-center flex flex-col justify-between min-h-[140px] shadow-sm transform-gpu ${step === 2 || (fusionResult && selectionMethod === 'map' && biasPhysics > 1.0) ? 'bg-purple-50 dark:bg-purple-900/20 border-purple-500/40 shadow-[0_0_30px_rgba(168,85,247,0.15)] scale-105 z-10' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 opacity-60 hover:opacity-100 hover:scale-[1.02]'}`}>
                    {step === 2 && <div className="absolute inset-x-0 bottom-0 h-1 bg-purple-500 animate-[shimmer_2s_infinite]"></div>}
                    <div className="mt-2 relative z-10">
                        <div className={`w-10 h-10 rounded-2xl mx-auto mb-3 flex items-center justify-center transition-colors ${step === 2 || fusionResult ? 'bg-purple-500/10 text-purple-500' : 'bg-slate-200 dark:bg-slate-800 text-slate-400'}`}>
                            <Zap size={20} className={step === 2 ? 'animate-pulse' : ''} />
                        </div>
                        <h3 className={`text-xs md:text-[10px] font-black uppercase tracking-widest ${step === 2 || fusionResult ? 'text-purple-600 dark:text-purple-400' : 'text-slate-500'}`}>Physique (Spectral)</h3>
                    </div>
                    <div className="flex flex-wrap items-center gap-1 justify-center pb-2 relative z-10">
                        {fusionResult ? fusionResult.sources.quantum.slice(0,3).map(n => <span key={n} className="px-1.5 py-0.5 rounded-md bg-purple-500/10 text-xs font-mono font-bold text-purple-600 dark:text-purple-400">{n}</span>) : <span className="text-xs text-slate-400 italic">--</span>}
                    </div>
                </div>

                {/* Oracle Node */}
                <div className={`p-4 rounded-2xl border transition-all duration-500 relative overflow-hidden text-center flex flex-col justify-between min-h-[140px] shadow-sm transform-gpu ${step === 3 || (fusionResult && selectionMethod === 'map' && biasIntuition > 1.0) ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-500/40 shadow-[0_0_30px_rgba(245,158,11,0.15)] scale-105 z-10' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 opacity-60 hover:opacity-100 hover:scale-[1.02]'}`}>
                    {step === 3 && <div className="absolute inset-x-0 bottom-0 h-1 bg-amber-500 animate-[shimmer_2s_infinite]"></div>}
                    <div className="mt-2 relative z-10">
                        <div className={`w-10 h-10 rounded-2xl mx-auto mb-3 flex items-center justify-center transition-colors ${step === 3 || fusionResult ? 'bg-amber-500/10 text-amber-500' : 'bg-slate-200 dark:bg-slate-800 text-slate-400'}`}>
                            <Brain size={20} className={step === 3 ? 'animate-pulse' : ''} />
                        </div>
                        <h3 className={`text-xs md:text-[10px] font-black uppercase tracking-widest ${step === 3 || fusionResult ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500'}`}>Intuition (Oracle)</h3>
                    </div>
                    <div className="flex flex-wrap items-center gap-1 justify-center pb-2 relative z-10">
                        {fusionResult ? fusionResult.sources.oracle.slice(0,3).map(n => <span key={n} className="px-1.5 py-0.5 rounded-md bg-amber-500/10 text-xs font-mono font-bold text-amber-600 dark:text-amber-400">{n}</span>) : <span className="text-xs text-slate-400 italic">--</span>}
                    </div>
                </div>
            </div>
            ...
            <AnimatePresence>
                {fusionResult && (
                    ...
                    >
                        <div className="absolute top-0 right-0 p-6 opacity-5"><Layers size={140}/></div>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-8">
                            <div className="text-center relative z-10 w-full mb-6">
                                <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-[0.2em] mb-8 border border-indigo-500/20 shadow-sm backdrop-blur-sm">
                                    <Hexagon size={12} className="animate-pulse-slow" /> Convergence Atteinte via {selectionMethod.toUpperCase()}
                                </div>
                            ...
                            <div className="w-full max-w-2xl space-y-8">
                                ...
                                <div className="bg-slate-950 p-6 rounded-[2rem] border border-white/5 space-y-6">
                                    <h4 className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.2em] flex items-center gap-2 border-b border-white/5 pb-3">
                                        <Calculator size={14} className="text-indigo-400" /> Audit Mathématique (Filtre de Kalman Continu)
                                    </h4>
                                    
                                    {/* Equation Render */}
                                    <div className="p-4 bg-slate-900/50 rounded-2xl border border-white/5 text-center font-mono text-xs text-slate-300">
                                        <div className="text-[10px] text-slate-500 mb-1">Équation d'État Multi-Capteur Réelle</div>
                                    ...
                                    <div className="text-[9px] text-slate-500 italic text-center">
                                        Les gains de Kalman s'adaptent dynamiquement de façon inversement proportionnelle à la variance historique du capteur.
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-6 sm:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 relative overflow-hidden group">
                                    ...
                                    <h4 className="text-[10px] font-black uppercase text-indigo-500 tracking-[0.2em] mb-6 flex items-center gap-2">
                                        <Activity size={14} className="text-indigo-400 animate-pulse" /> Matrice de Convergence
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                                        {fusionResult.convergedNumbers.filter(cn => fusionResult.finalTicket.includes(cn.number)).map(cn => (
                                            <div key={cn.number} className="flex items-center justify-between p-4 bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex gap-1.5">
                                                        {cn.sources.map(src => (
                                                            <div key={src} className="p-2 bg-slate-50 dark:bg-slate-900 rounded-xl text-slate-500" title={getSourceLabel(src)}>
                                                                {getSourceIcon(src)}
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                <div className="text-right">
                                                    <div className="text-sm font-black text-slate-900 dark:text-white">{Math.round(cn.score)} <span className="text-[10px] text-slate-400">pts</span></div>
                                                    <div className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mt-0.5">Score</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 sm:p-6 rounded-2xl border border-slate-100 dark:border-slate-700/50 min-w-0 font-medium">
                                    ...
                                    <div className="mb-8">
                                        <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-3 px-2 flex items-center gap-2">
                                            <Layers size={14} className="text-indigo-500" /> Poids des Vecteurs Réels
                                        </h4>
                                        <div className="flex h-4 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden p-0.5 gap-0.5">
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        <div className="h-full bg-emerald-500 rounded-l-full transition-all duration-500" style={{ width: `${pPct}%` }} title={`Logique: ${Math.round(pPct)}%`}></div>
                                                        <div className="h-full bg-purple-500 transition-all duration-500" style={{ width: `${qPct}%` }} title={`Physique: ${Math.round(qPct)}%`}></div>
                                                        <div className="h-full bg-amber-500 rounded-r-full transition-all duration-500" style={{ width: `${oPct}%` }} title={`Intuition: ${Math.round(oPct)}%`}></div>
                                                    </>
                                                );
                                            })()}
                                        </div>
