...
import { Brain, Sparkles, Activity, ShieldCheck, Terminal, FileText, BrainCircuit, Target, Copy, RefreshCw } from 'lucide-react';
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8">
                    ...
                    {narrative ? (
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-8 pb-6 border-b border-slate-100 dark:border-slate-700">
                                ...
                                <button onClick={() => copyReport(narrative.summary)} className="p-2 text-slate-400 hover:text-indigo-500 transition-colors">
                                    <Copy size={18} />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-3xl border border-emerald-100 dark:border-emerald-800/30">
                                        <div className="text-xs font-black text-emerald-600/70 uppercase mb-2 tracking-wider flex items-center gap-2"><Target size={10}/> Verdict</div>
                                    ...
                                    <div className="p-5 bg-orange-50 dark:bg-orange-900/10 rounded-3xl border border-orange-100 dark:border-orange-800/30">
                                        <div className="text-xs font-black text-orange-600/70 uppercase mb-2 tracking-wider flex items-center gap-2"><ShieldCheck size={10}/> Risque</div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    ...
                    {contextMetrics && (() => {
                        ...
                        const entropyRatio = 1 / (1 + Math.exp(-15 * (contextMetrics.entropy - 0.88)));
                        ...
                        const hurstRatio = 1 / (1 + Math.exp(-10 * (contextMetrics.hurst - 0.55)));
                        ...
                        return (
                            <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-lg grid grid-cols-3 gap-2 animate-scale-in">
                                <div className="bg-black/30 p-3 rounded-2xl text-center border border-white/5">
                                    <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Entropie</div>
                                    <div className="text-lg font-black transition-colors duration-300" style={{ color: entropyColor }}>
                                        {Math.round(contextMetrics.entropy * 100)}%
                                    </div>
                                </div>
                                <div className="bg-black/30 p-3 rounded-2xl text-center border border-white/5">
                                    <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Hurst</div>
                                    <div className="text-lg font-black transition-colors duration-300" style={{ color: hurstColor }}>
                                        {contextMetrics.hurst.toFixed(2)}
                                    </div>
                                </div>
                                <div className="bg-black/30 p-3 rounded-2xl text-center border border-white/5">
                                    <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Volatilité</div>
                        ...
                        >
                            ...
                            {analysis && (
                                <div className="space-y-6 animate-slide-up">
                                    {/* Reasoning Section */}
                                    <div>
                                        <span className="text-xs text-slate-500 block mb-2 uppercase">/// LOGICAL_TRACE</span>
                                    ...
                                    {analysis.anomalies.length > 0 && (
                                        <div className="p-3 border border-rose-900/50 bg-rose-900/10 rounded-lg">
                                            <span className="text-xs text-rose-500 block mb-1 uppercase">!!! ANOMALY_DETECTED</span>
                                    ...
                                    {/* Next Sequence */}
                                    <div>
                                        <span className="text-xs text-indigo-400 block mb-2 uppercase">&gt;&gt;&gt; PREDICTIVE_VECTOR</span>
                                    ...
                                    {analysis.suggestedFocus.length > 0 && (
                                        <div className="pt-4 border-t border-white/10">
                                             <span className="text-xs text-emerald-500 block mb-3 uppercase">/// TARGET_LOCK</span>
