...
const GravityCompass: React.FC<{
  vector: { x: number; y: number; angle: number; zone: string };
}> = ({ vector }) => {
  return (
    <div className="relative w-40 h-40 flex items-center justify-center bg-slate-900 rounded-full border-4 border-slate-800 shadow-inner">
      ...
      <div className="absolute top-2 text-[10px] font-black text-slate-500">
        N
      </div>
      <div className="absolute bottom-2 text-[10px] font-black text-slate-500">
        S
      </div>
      <div className="absolute left-2 text-[10px] font-black text-slate-500">
        O
      </div>
      <div className="absolute right-2 text-[10px] font-black text-slate-500">
        E
      </div>
  ...
  if (nexusLoading || (localLoading && !metrics)) {
    return (
      <div className="flex flex-col items-center justify-center p-24 gap-6 bg-slate-900/5 rounded-3xl border border-dashed border-indigo-200 animate-pulse">
        ...
        <p className="text-xs font-black uppercase tracking-[0.4em] text-indigo-500">
          Triangulation GPS...
        </p>
      </div>
    );
          ...
          <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 relative overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-4 mb-6 px-2">
              <h3 className="text-lg sm:text-xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                <MapIcon className="text-indigo-600 dark:text-indigo-400 shrink-0" />{" "}
                Carte des Impacts
              </h3>
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-[10px] sm:text-xs font-bold text-slate-400 uppercase">
                <div className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-slate-50 dark:bg-slate-900 rounded-sm border border-slate-100 dark:border-slate-850 shrink-0"></span>{" "}
                  Froid
                </div>
                <div className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-indigo-100 dark:bg-indigo-900/40 rounded-sm shrink-0"></span>{" "}
                  Moyen
                </div>
                <div className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-indigo-500 rounded-sm shrink-0"></span>{" "}
                  Chaud
                </div>
                <div className="flex items-center gap-1">
            ...
            {(() => {
              const wellNumbers =
                metrics?.gravityWells?.map(
                  (well: any) => well.y * 10 + well.x + 1,
                ) || [];
              ...
              return (
                <div className="grid grid-cols-10 gap-1 sm:gap-1.5 md:gap-2.5 relative">
                  {Array.from({ length: 90 }, (_, i) => i + 1).map((n) => {
                    ...
                    const dynamicShadow =
                      sIntensity > 0.5
                        ? `0 10px 15px -3px rgba(${r}, ${g}, ${b}, ${0.3 * sIntensity})`
                        : "none";
                    ...
                    return (
                      <div key={n} className={baseClass} style={dynamicStyle}>
                        {n}
                        {isWell && (
                          <>
                  ...
                  {metrics?.advancedClusters.map((cluster, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className="absolute -top-3 right-0 bg-slate-950 text-[10px] font-black text-white px-2 py-0.5 rounded-full border shadow-sm"
                          style={{
                            borderColor: cluster.color,
                            color: cluster.color,
                          }}
          ...
          <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 relative overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <h3 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                ...
                Trajectoire Orbitale du Centride
              </h3>
                    ...
                    (() => {
                      ...
                      return (
                        <g>
                          ...
                          />
                          <circle cx={px} cy={py} r="2" fill="#10b981" />
                        </g>
                      );
                    })()}
                </svg>
              ...
              <div className="flex-1 space-y-4 text-left">
                <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-850">
                  <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
                    <Activity size={12} className="text-indigo-500" /> Analyse
                    Cinématique
                  </div>
                  <p className="text-xs text-slate-550 dark:text-slate-400 leading-relaxed font-semibold">
                    L'orbite du centroid modélise les oscillations spatiales de
                    l'énergie. Le vecteur{" "}
                    <span className="text-emerald-500 dark:text-emerald-400 font-bold">
                      H+1 (Prédiction)
                    </span>{" "}
                    est calculé de manière continue en atténuant l'historique
                    par une exponentielle de Hurst. L'absence de bifurcation
                    brute préserve la régularité dynamique du paysage
                    d'inférence.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-850">
                    <div className="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-wider mb-1">
                      Centroid Présent (H0)
                    </div>
                    <div className="text-base font-black text-slate-800 dark:text-white font-mono leading-none">
                      X:{metrics?.barycenter.x.toFixed(2)} Y:
                      {metrics?.barycenter.y.toFixed(2)}
                    </div>
                  </div>
                  <div className="p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/10">
                    <div className="text-[9px] text-emerald-400 font-black uppercase tracking-wider mb-1 text-emerald-600 dark:text-emerald-400">
                      Vecteur Projété (H+1)
                    </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 md:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 space-y-6">
            ...
            {sideTab === "clusters" ? (
                      ...
                      <div className="flex justify-between items-center mb-2.5">
                        ...
                        <div className="flex gap-1">
                          {cluster.numbers.slice(0, 3).map((n) => (
                            <span
                              key={n}
                              className="w-5 h-5 flex items-center justify-center bg-white dark:bg-slate-800 rounded-full text-[9px] font-black shadow-sm text-slate-700 dark:text-slate-300 border border-slate-150 dark:border-slate-700"
                            >
                              {n}
                            </span>
                          ))}
                          {cluster.numbers.length > 3 && (
                            <span className="text-[9px] font-bold text-slate-400 self-center pl-1">
                              +{cluster.numbers.length - 3}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="w-full bg-slate-200 dark:bg-slate-700 h-1 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, cluster.potential)}%`,
                            }}
                          ></div>
                        </div>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-[280px] overflow-y-auto custom-scrollbar pr-1">
                  {metrics?.gravityWells
                    ?.slice(0, 3)
                    .map((well: any, idx: number) => {
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-black text-violet-600 dark:text-violet-400 uppercase tracking-wide">
                                Puits #{wellNum}
                              </span>
                            </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-100 dark:border-slate-850">
                              ...
                              <div className="flex gap-1 flex-wrap">
                                {well.subordinateNumbers
                                  .slice(0, 5)
                                  .map((sub: number) => (
                                    <span
                                      key={sub}
                                      className="text-[9px] font-bold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 px-1 rounded border border-slate-250/20"
                                    >
                                      {sub}
                                    </span>
                                  ))}
                                {well.subordinateNumbers.length > 5 && (
                                  <span className="text-[8px] text-slate-400 self-center font-bold pl-0.5">
                                    ...
                                  </span>
                                )}
                              </div>
          ...
          <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-3xl border border-indigo-100 dark:border-indigo-800/50 flex gap-3 text-left">
            ...
            <p className="text-[10px] text-indigo-800 dark:text-indigo-250 font-medium leading-relaxed">
              "Si la boussole pointe vers le <strong>Sud-Est</strong>,
              privilégiez les numéros élevés (ex: 70-90). Si elle pointe vers le{" "}
              <strong>Nord</strong>, jouez les petits numéros (1-30)."
            </p>
          </div>
        </div>
      </div>
