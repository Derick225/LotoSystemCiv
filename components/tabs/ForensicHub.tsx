            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 md:p-10 border border-slate-200/60 dark:border-slate-800 shadow-xl shadow-slate-200/20 dark:shadow-none">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">En attente de résultat ({pendingPredictions.length})</h4>
                  {pendingPredictions.length === 0 ? (
                    <p className="text-sm text-slate-500 italic">Aucune prédiction en attente de résultat.</p>
                  ) : (
                    pendingPredictions.map((p: PredictionHistoryItem) => (
                      <div key={p.id} className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                        <div className="flex gap-2">
                          {p.prediction.suggestedNumbers.map((n: number) => (
                            <div key={n} className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold">{n}</div>
                          ))}
                        </div>
                        <button onClick={(e) => handleDeletePending(p.id, e)} className="p-2 text-slate-400 hover:text-rose-500"><Trash2 size={16} /></button>
                      </div>
                    ))
                  )}
                </div>
                ...
                <div className="space-y-4 mt-8">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Rapports d'Audit ({reports.length})</h4>
                  {reports.length === 0 ? (
                    <p className="text-sm text-slate-500 italic">Aucun rapport d'audit disponible.</p>
                  ) : (
                    reports.map(rep => {
                      let hits = 0;
                      if (typeof rep.matches === "number") hits = rep.matches;
                      else if (Array.isArray(rep.matches)) hits = rep.matches.length;

                      return (
                        <div key={rep.id} onClick={() => setSelectedReport(rep)} className="cursor-pointer group flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-emerald-500 transition-colors gap-4">
                          <div>
                            <span className="text-xs text-slate-400">{formatDateSafely(rep.date)}</span>
                            <div className="flex gap-1 mt-2">
                              {rep.combo?.map(n => (
                                <div key={n} className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-bold">{n}</div>
                              ))}
                            </div>
                          </div>
                          <div className="flex items-center gap-6">
                            <div className="text-right">
                              <span className="block text-xs text-slate-500">Précision</span>
                              <span className="font-bold text-emerald-500">{hits} / 5</span>
                            </div>
                            <button onClick={(e) => handleDeleteReport(rep.id, e)} className="p-2 text-slate-400 hover:text-rose-500"><Trash2 size={16} /></button>
                          </div>
                        </div>
                      );
                    })
