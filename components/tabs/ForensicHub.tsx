...
export const ForensicHub: React.FC<{ drawName: string }> = ({ drawName }) => {
    ...
    // Calcul de la précision agrégée des neurones (Quels algos performent le mieux sur les erreurs ?)
    const algoRadarData = useMemo(() => {
        ...
        return Object.entries(aggregates).map(([algo, data]) => ({
            ...
            fullMark: 100
        })).sort((a, b) => b.precision - a.precision).slice(0, 6); // Top 6 algos
    }, [reports]);

    // Données pour le graphique de tendance temporelle
    const trendData = useMemo(() => {
        return reports.map(r => ({
            date: r.date.slice(0, 5), // JJ/MM
                ...
                <div className="lg:col-span-5 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-700">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h4 className="font-black text-slate-800 dark:text-white uppercase tracking-tight">Vecteurs d'Erreur</h4>
                        </div>
                        
                        <div className="space-y-6">
                            <div className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-slate-800">
                                <div className="text-[10px] font-black text-slate-500 uppercase mb-4">Analyse des "Presque Hits"</div>
                                <div className="space-y-4">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <p className="text-xs text-rose-100 leading-relaxed font-medium">
                                        {algoRadarData.length > 0 
                                            ? `Le neurone "${algoRadarData[0].algo}" avait identifié correctement des numéros manqués. Augmentez son poids dans le module Tuning.`
