...
export const NeuralArchitectureTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 90; i++) {
      ...
      const pruneThreshold = Math.exp(-3.0);
  ...
  const generateNeuralPath = async () => {
    ...
    const prng = new LCG(`${drawName}_neural_path_${selectedNodeData.id}`);
    ...
    while (path.size < 5 && steps < 20) {
      ...
      if (neighbors.length > 0) {
        ...
        if (availableHighEnergy.length > 0) {
          nextId =
            availableHighEnergy[
              Math.floor(prng.next() * availableHighEnergy.length)
            ].number;
        } else {
          // Fallback ultime : aléatoire
          do {
            nextId = Math.floor(prng.next() * 90) + 1;
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h3 className="text-xl font-black text-white uppercase tracking-tighter flex items-center gap-3">
            <Network size={24} className="text-indigo-500" /> Matrice
            Structurelle
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Cartographie des poids synaptiques & propagation.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-black/30 px-4 py-2 rounded-xl border border-white/5 text-center">
            ...
            <div className="text-lg font-black text-white">
              {nodes.filter((n) => n.centrality > 1).length}
            </div>
          </div>
          <div className="bg-black/30 px-4 py-2 rounded-xl border border-white/5 text-center">
            <div className="text-[10px] font-black text-slate-500 uppercase">
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 relative overflow-hidden">
          ...
          <div className="grid grid-cols-10 gap-2 sm:gap-3">
            {nodes.map((node) => {
              ...
              const isLinked = selectedNodeData?.links.some(
                (l) => l.target === node.id && l.strength > minStrength / 100,
              );

              let bgClass =
                "bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-700 text-slate-300";
              let scaleClass = "scale-100";

              if (isSelected) {
                bgClass =
                  "bg-indigo-600 border-indigo-500 text-white shadow-xl z-20";
                scaleClass = "scale-110";
              } else if (isInPath) {
                bgClass =
                  "bg-emerald-500 border-emerald-400 text-white shadow-lg z-20 animate-pulse";
              ...
              return (
                <button
                  ...
                  className={`
                                    ...
                                    `}
                  style={
                    !isSelected && !isLinked && !isInPath && intensity > 0.1
                      ? { opacity: 0.7 + intensity * 0.3 }
                      : {}
                  }
                >
                  {node.id}
                  {intensity > 0.6 && !isSelected && !isInPath && (
                    <div
                      className={`absolute bottom-1 w-1 h-1 rounded-full ${COMMUNITY_BG[node.community]}`}
                    ></div>
                  )}
                </button>
              );
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 shadow-xl flex-1 flex flex-col h-full min-h-[500px]">
              ...
              {selectedNodeData ? (
                ...
                >
                  ...
                  <div className="text-center mb-8 relative">
                    <div
                      className={`absolute inset-0 bg-gradient-to-b ${COMMUNITY_COLORS[selectedNodeData.community].split(" ")[1].replace("text-", "from-")}/20 to-transparent blur-3xl opacity-30`}
                    ></div>
                    <div className="relative z-10">
                      <NumberBall
                        number={selectedNodeData.id}
                        size="xl"
                        isAttractor
                      />
                      <div className="mt-4 flex justify-center gap-4">
                        <div className="text-center">
                          ...
                          <div className="text-xl font-black text-white">
                            {selectedNodeData.centrality.toFixed(1)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className="text-xs font-black text-slate-500 uppercase">
                          ...
                          <div
                            className={`text-xl font-black ${COMMUNITY_COLORS[selectedNodeData.community].split(" ")[1]}`}
                          >
                            #{selectedNodeData.community + 1}
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-1 space-y-3 mb-6 overflow-y-auto custom-scrollbar pr-1 max-h-[250px]">
                    <div className="text-xs font-black text-slate-500 uppercase mb-2 flex justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-12 h-1 bg-slate-800 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(100, link.strength * 100)}%`,
                              }}
                            ></div>
                          </div>
                ...
                >
                  <Share2
                    size={64}
                    className="text-slate-600 mb-6 animate-pulse-slow"
                  />
                  <p className="text-sm font-bold text-slate-400">
                    Sélectionnez un vecteur dans la matrice pour démarrer la
                    propagation.
                  </p>
                </motion.div>
              )}
          ...
          {generatedPath.length > 0 && (
            <div className="bg-emerald-900/20 p-6 rounded-2xl border border-emerald-500/30 animate-slide-up">
