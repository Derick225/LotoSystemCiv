...
const CLUSTER_CONFIG: Record<
  string,
  ClusterSummary & { advice: string; metaphor: string }
> = {
  Sprinter: {
    type: "Sprinter",
    count: 0,
...
export const ClusteringTab: React.FC<ClusteringTabProps> = ({ drawName }) => {
  ...
  const [summary, setSummary] = useState<
    Array<ClusterSummary & { advice: string; metaphor: string }>
  >([]);
  ...
  const handleGenerateFromCluster = async (clusterType: string) => {
    ...
    if (clusterPoints.length < 5) {
      ...
      showToast(
        `Pas assez de ${clusterType}s pour générer un ticket (Min 5).`,
        "error",
      );
      return;
    }
    ...
    if (bestTicket.length === 5) {
      ...
      bestTicket.forEach((num) => {
        breakdown[num] = {
          ...
          spatial:
            clusterType === "Sprinter"
              ? 90
              : clusterType === "Marathonien"
                ? 70
                : clusterType === "Dormeur"
                  ? 80
                  : 50,
      ...
      showToast(
        `Ticket ${clusterType} généré et autopsié (AC:${bestAC}).`,
        "success",
      );
    }
  };

  if (nexusLoading || calculating || history.length === 0)
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-6 bg-slate-900/5 rounded-3xl border border-dashed border-indigo-200 dark:border-slate-700 animate-pulse">
        ...
        <p className="font-black text-indigo-500 text-xs font-mono uppercase tracking-[0.4em]">
          Profilage des numéros...
        </p>
      </div>
    );
        ...
        <div className="flex justify-between items-center px-2">
          ...
          {activeFilter && (
            <button
              ...
              className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full hover:bg-slate-200 transition"
            >
              Voir Tout
            </button>
          )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {summary.map((s) => (
            <div
              ...
              onClick={() => {
                audioEngine.play("click");
                setActiveFilter(activeFilter === s.type ? null : s.type);
            ...
            >
              <div className="flex justify-between items-start mb-2">
                ...
                <span
                  className={`text-xl font-black ${activeFilter === s.type ? "text-white" : "text-slate-800 dark:text-white"}`}
                >
                  {s.count}
                </span>
              </div>
              <div className="mb-4">
                <div
                  className={`text-[11px] font-black uppercase tracking-wide ${activeFilter === s.type ? "text-white" : ""}`}
                  style={{
                    color: activeFilter === s.type ? "white" : (s as any).color,
                  }}
                >
                  {s.type}s
                </div>
                <div
                  className={`text-xs font-medium mt-1 leading-tight ${activeFilter === s.type ? "text-slate-400" : "text-slate-400"}`}
                >
                  {s.metaphor}
                </div>
              </div>
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handleGenerateFromCluster(s.type);
                }}
              ...
              <div
                className="absolute -right-4 -bottom-4 opacity-10 transform rotate-12 scale-150 pointer-events-none"
                style={{ color: (s as any).color }}
              >
                {s.type === "Sprinter" ? (
                  <Zap size={60} />
                ) : s.type === "Dormeur" ? (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-xl border border-slate-100 dark:border-slate-800 h-[450px] relative">
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-slate-900 text-white p-3 rounded-xl text-xs shadow-xl border border-slate-700">
                        ...
                        <div style={{ color: CLUSTER_CONFIG[p.cluster].color }}>
                          {p.cluster}
                        </div>
          ...
          <p className="text-xs text-center text-slate-400 mt-2 italic">
            Cliquez sur un point pour voir sa fiche joueur.
          </p>
        </div>
          ...
          {numberProfile ? (
            <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-2xl border border-slate-800 h-full flex flex-col relative overflow-hidden animate-slide-up">
              <div
                ...
                style={{ backgroundColor: numberProfile.config.color }}
              ></div>

              <div className="relative z-10 flex flex-col items-center text-center">
                <div
                  className="mb-4 transform hover:scale-110 transition-transform cursor-pointer"
                  title="Numéro"
                >
                  <NumberBall number={numberProfile.number} size="xl" />
                </div>
                <h4 className="text-2xl font-black">
                  {numberProfile.config.type}
                </h4>
                <p className="text-xs text-slate-400 font-medium mt-1">
                  {numberProfile.config.metaphor}
                </p>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${numberProfile.scores.form}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${numberProfile.scores.gap}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 w-full grid grid-cols-2 gap-3">
                  ...
                  {correlationData &&
                    correlationData[numberProfile.number]?.affinities && (
                      <div className="bg-slate-800/50 p-3 rounded-2xl border border-slate-700/50 text-left">
