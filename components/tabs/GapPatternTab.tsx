...
export const GapPatternTab: React.FC<{ drawName: string }> = ({ drawName }) => {
    ...
    try {
      ...
      for (const dName of drawNamesToAnalyze) {
        ...
        for (let n = 1; n <= 90; n++) {
          ...
          algoScores[AlgoKey.GAP_BAND_SEQUENCE].push(
            bd[AlgoKey.GAP_BAND_SEQUENCE] || 0,
          );
        }
      }
  ...
  const sortedStatsList = useMemo(() => {
    ...
    return list.sort((a, b) => {
      ...
      if (sortBy === "number") {
        ...
        valA = a.autocorrelationLag1;
        valB = b.autocorrelationLag2;
  ...
  const handleToggleSort = (
    field: "number" | "gap" | "mean" | "lag1" | "signal",
  ) => {
    try {
      audioEngine.play("click");
    } catch (e) {}
        ...
        <div className="bg-slate-950/95 border border-slate-800 p-3 rounded-xl shadow-2xl backdrop-blur-md text-xs font-bold font-sans">
          ...
          {payload[1] && (
            <p className="text-emerald-400">
              Moyenne historique:{" "}
              <span className="text-white font-black">{payload[1].value}</span>
            </p>
          )}
        </div>
      );
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12 w-full overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-indigo-950/40 via-slate-900/40 to-emerald-950/20 p-6 md:p-8 rounded-[2.5rem] border border-indigo-500/10 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl space-y-3">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed max-w-2xl font-medium">
            Ce module modélise mathématiquement les cycles d'écarts de chaque
            numéro en tant que processus autorégressif de Lag-1 déterministe. Il
            calcule l'autocorrélation temporelle, la variance continue
            short/long-term, et extrait la résonance fréquentielle pour isoler
            les dérives de convergence.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-white dark:bg-slate-800/80 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 relative overflow-hidden shadow-sm">
          ...
          <h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white mb-2 flex items-center gap-2 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800/80 pb-4">
            <Sparkles className="text-amber-500 animate-pulse" size={18} /> Top
            10 Résonances Actuelles
          </h4>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black mb-6">
            Numéros dont l'écart actuel correspond parfaitement à la projection
            cyclique d'autocorrélation (Lag-1)
          </p>
        ...
        <div className="bg-white dark:bg-slate-800/80 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 relative overflow-hidden shadow-sm">
          ...
          <h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white mb-2 flex items-center gap-2 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800/80 pb-4">
            <Flame className="text-emerald-500 animate-pulse" size={18} /> Top
            10 Fatigue / Surretard
          </h4>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black mb-6">
            Numéros ayant le retard le plus anormal au vu de leur propre
            historique (Normal CDF inverse)
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white dark:bg-slate-800/80 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 shadow-sm">
          ...
          <div className="lg:col-span-4 flex flex-col justify-between space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div className="bg-slate-50 dark:bg-slate-900/50 p-3.5 rounded-2xl border border-slate-100/30 dark:border-slate-800/60">
                  ...
                  <span className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-1.5">
                    ...
                    {selectedStats.currentGap > selectedStats.meanGap ? (
                      <span className="text-[10px] px-1.5 py-0.5 bg-amber-500/10 text-amber-500 rounded font-bold uppercase">
                        Overdue
                      </span>
                    ) : (
                      <span className="text-[10px] px-1.5 py-0.5 bg-sky-500/10 text-sky-500 rounded font-bold uppercase">
                        Rapid
                      </span>
                    )}
                  </span>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 p-3.5 rounded-2xl border border-slate-100/30 dark:border-slate-800/60">
                  ...
                  >
                    {selectedStats.autocorrelationLag1 > 0 ? "+" : ""}
                    {selectedStats.autocorrelationLag1.toFixed(2)}
                  </span>
                </div>
              </div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">
                    ...
                    <span className="text-emerald-500 font-extrabold">
                      {Math.round(selectedStats.fatigueScore * 100)}%
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-900 h-2 rounded-full overflow-hidden">
                    <div
                      className="bg-emerald-500 h-full rounded-full transition-all duration-500"
                      style={{ width: `${selectedStats.fatigueScore * 100}%` }}
                    />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">
                    ...
                    <span className="text-amber-500 font-extrabold">
                      {Math.round(selectedStats.resonanceScore * 100)}%
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-900 h-2 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${selectedStats.resonanceScore * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">
                    ...
                    <span className="text-indigo-500 font-extrabold">
                      {selectedStats.compressionFactor.toFixed(2)}x
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-900 h-2 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, selectedStats.compressionFactor * 50)}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="bg-gradient-to-br from-indigo-950/20 to-slate-950/20 p-4 rounded-2xl border border-indigo-500/10 text-[10px] text-slate-400 leading-relaxed font-bold">
              <span className="text-indigo-400 font-extrabold uppercase block mb-1">
                Projection Temporelle :
              </span>
              Écart mathématiquement attendu pour le prochain tirage :{" "}
              <span className="text-white font-extrabold">
                {selectedStats.expectedNextGap}
              </span>{" "}
              tirages. La résonance est maximale lorsque l'écart actuel (
              {selectedStats.currentGap}) approche de cette valeur.
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-4 mb-6">
                ...
                <div className="flex items-center gap-4 text-[10px] font-bold">
                  ...
                  <div className="flex items-center gap-1.5 text-slate-400">
                    <span className="inline-block w-3 h-0.5 bg-emerald-500 stroke-dasharray-3" />{" "}
                    Moyenne
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-100 dark:border-slate-800/80 pb-6">
            <div>
              ...
              <h4 className="text-sm md:text-base font-black text-slate-800 dark:text-white flex items-center gap-2">
                ...
                SequencePatternAnalyzer : Détection de Récurrences Séquentielles
              </h4>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black">
                Ajustement en temps réel de l'algorithme d'affinité séquentielle
                pour le numéro {selectedNum}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-6 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100/40 dark:border-slate-800/60">
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  Taille de la Fenêtre Glissante (N-Gaps) :{" "}
                  <span className="text-indigo-500 font-extrabold">
                    {windowSize}
                  </span>
                </label>
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  Seuil de Récurrence Minimal :{" "}
                  <span className="text-indigo-500 font-extrabold">
                    {minRecurrence.toFixed(1)}
                  </span>
                </label>
                <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/40 p-5 rounded-2xl border border-slate-100/50 dark:border-slate-800/50 space-y-4">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                  Derniers écarts consécutifs enregistrés chronologiquement pour
                  ce numéro :
                </p>
              </div>

              <div className="flex items-center gap-2.5">
                {selectedSequencePattern.recentSequence &&
                selectedSequencePattern.recentSequence.length > 0 ? (
                  selectedSequencePattern.recentSequence.map((gap, i) => (
                    <div key={i} className="flex items-center gap-1.5">
                      <div className="flex flex-col items-center justify-center w-11 h-11 bg-indigo-500/10 dark:bg-indigo-500/20 rounded-xl border border-indigo-500/30 text-indigo-500 dark:text-indigo-400">
                        ...
                        <span className="text-[7px] uppercase font-black tracking-wider text-slate-400">
                          Gap {i + 1}
                        </span>
                      </div>
                      {i <
                        selectedSequencePattern.recentSequence.length - 1 && (
                        <ChevronRight
                          size={14}
            ...
            <div className="bg-slate-50 dark:bg-slate-900/40 p-5 rounded-2xl border border-slate-100/50 dark:border-slate-800/50 space-y-4">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                  Pattern historique ayant la plus forte résonance de similarité
                  gaussienne :
                </p>
              </div>
              ...
              {bestMatch ? (
                <div className="space-y-3">
                  ...
                  <div className="text-[11px] font-bold text-slate-500 dark:text-slate-400 space-y-1">
                    <div>
                      Prochain Écart attendu :{" "}
                      <span className="text-slate-800 dark:text-white font-black">
                        {bestMatch.nextExpectedGap}
                      </span>
                    </div>
                    <div>
                      Confiance de résonance :{" "}
                      <span className="text-indigo-500 font-extrabold">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/40 p-5 rounded-2xl border border-slate-100/50 dark:border-slate-800/50 flex flex-col justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                  Mesure de la probabilité de rupture par résonance de pattern
                  stochastique :
                </p>
              </div>
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${selectedSequencePattern.stochasticScore}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <div className="bg-white dark:bg-slate-800/80 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 shadow-sm space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-6">
          <div>
            <h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white mb-1 flex items-center gap-2 uppercase tracking-widest">
              <Sliders className="text-indigo-500" size={18} /> Cartographie
              Complète des Signaux
            </h4>
            <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black">
              Filtrage et tri multi-critères des dynamiques de séquences de
          ...
          <table className="w-full text-left border-collapse text-xs font-sans">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800/80 text-[10px] font-black text-slate-400 dark:text-slate-500 tracking-wider uppercase">
                <th
                  className="pb-3 text-center cursor-pointer hover:text-indigo-500 transition-colors"
                ...
                <th
                  className="pb-3 text-center cursor-pointer hover:text-indigo-500 transition-colors"
                  onClick={() => handleToggleSort("gap")}
                >
                  Écart Actuel{" "}
                  {sortBy === "gap" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="pb-3 text-center cursor-pointer hover:text-indigo-500 transition-colors"
                  onClick={() => handleToggleSort("mean")}
                >
                  Écart Moyen{" "}
                  {sortBy === "mean" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                <th
                  className="pb-3 text-center cursor-pointer hover:text-indigo-500 transition-colors"
                  onClick={() => handleToggleSort("lag1")}
                >
                  Autocorrélation Lag-1{" "}
                  {sortBy === "lag1" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
                ...
                <th
                  className="pb-3 text-center cursor-pointer hover:text-indigo-500 transition-colors"
                  onClick={() => handleToggleSort("signal")}
                >
                  Signal Combiné{" "}
                  {sortBy === "signal" && (sortOrder === "asc" ? "↑" : "↓")}
                </th>
              ...
              {filteredStatsList.slice(0, searchTerm ? 5 : 20).map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`py-2.5 text-center ${s.autocorrelationLag1 > 0.1 ? "text-emerald-500" : s.autocorrelationLag1 < -0.1 ? "text-amber-500" : "text-slate-400"}`}
                    >
                      {s.autocorrelationLag1 > 0 ? "+" : ""}
                      {s.autocorrelationLag1}
                    </td>
                    <td className="py-2.5 text-center text-slate-500">
                      {s.expectedNextGap}
                    </td>
                    <td className="py-2.5 text-center">
                      <span
                        className={`inline-block px-2 py-0.5 text-[9px] font-black rounded-full ${
                          s.fatigueScore > 0.8
        ...
        {!searchTerm && (
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-4 text-center font-bold">
            Affichage des 20 numéros les plus significatifs selon le critère de
            tri actif.
          </p>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-800/80 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 shadow-sm">
        <h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white mb-2 flex items-center gap-2 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800/80 pb-4">
          <Compass className="text-emerald-500" size={18} /> Distribution
          Empirique des Écarts Globaux
        </h4>
        <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black mb-6">
          Densité historique cumulée de toutes les transitions pour analyser
          l'enveloppe exponentielle de décroissance
        </p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white flex items-center gap-2 uppercase tracking-widest">
              <Table className="text-indigo-500" size={18} /> Analyse de
              Corrélation Croisée des Algorithmes d'Écart
            </h4>
            <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest font-black mt-1">
              Vérification empirique de l'indépendance linéaire des signaux
              (Complémentarité vs Redondance)
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <option value="all_4">
                Données Globales (Fusion des 4 Jeux)
              </option>
            </select>
            <button
            ...
            >
              <RefreshCw
                size={14}
                className={isCalculatingCorr ? "animate-spin" : ""}
              />
            </button>
          </div>
        </div>
        ...
        {isCalculatingCorr ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
              Inférence des matrices matricielles en cours...
            </p>
          </div>
        ) : correlationData ? (
