...
async function loadTensorFlow() {
  if (!tfInstance) {
    ...
    try {
      ...
      await tf.ready();
      console.log(
        "[SimulationLab] TensorFlow.js backend initialisé avec succès : WebGL",
      );
    } catch (e) {
      console.warn(
        "[SimulationLab] Impossible d'activer le backend WebGL, repli sur CPU.",
        e,
      );
      try {
        ...
        console.error(
          "[SimulationLab] Échec critique de l'initialisation du backend CPU",
          err,
        );
      }
    }
...
const extractDrawFromRecord = (record: any): number[] | null => {
  ...
  const allNums = Object.values(record)
    .map((v) => Number(v))
    .filter((n) => Number.isFinite(n));
...
const isValidDraw = (nums: number[] | null): nums is number[] => {
  ...
  return nums.every(
    (n) => Number.isInteger(Math.round(n)) && n >= 1 && n <= DOMAIN_SIZE,
  );
};
...
export const SimulationLab: React.FC<{ drawName: string }> = ({ drawName }) => {
  ...
  const [fileDraws, setFileDraws] = useState<{ gagnants: number[] }[] | null>(
    null,
  );
  ...
  >("idle");
  const [trainingDataSource, setTrainingDataSource] = useState<{
    type: "history" | "file";
    sampleCount: number;
  } | null>(null);
  ...
  const [optimizerType, setOptimizerType] = useState<
    "adam" | "sgd" | "rmsprop"
  >("adam");
  const [hiddenLayersConfig, setHiddenLayersConfig] = useState<
    "32x16" | "64x32" | "16x8"
  >("32x16");
  const [activationFn, setActivationFn] = useState<"relu" | "sigmoid" | "tanh">(
    "relu",
  );
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          if (parsedDraws.length < 20) {
            ...
            showToast(
              `Format non exploitable : seulement ${parsedDraws.length} tirage(s) valide(s) détecté(s).`,
              "error",
            );
          } else {
            ...
            showToast(
              `Données chargées : ${parsedDraws.length} tirage(s) valide(s) sur ${data.length} ligne(s).`,
              "success",
            );
    ...
    for (let i = 0; i < samples; i++) {
      ...
      if (selectedFeatures.gaps) {
        for (let j = 0; j < DRAW_SIZE; j++) {
          features.push(
            j < currentDraw.gagnants.length - 1
              ? (currentDraw.gagnants[j + 1] - currentDraw.gagnants[j]) /
                  DOMAIN_SIZE
              : 0,
          );
        }
      ...
      if (selectedFeatures.parity) {
        features.push(
          ...[...currentDraw.gagnants].map((n) => (n % 2 === 0 ? 1 : 0)),
        );
      }
  ...
  const prepareData = (): {
    ...
    if (history.length >= 20) {
      const built = buildTensorsFromDraws(
        history as unknown as { gagnants: number[] }[],
      );
      if (!built) return null;
      const { X, Y, inputDim, sampleCount } = built;
  ...
  const startTraining = async () => {
    ...
    if (!tf) {
      showToast(
        "Le moteur TensorFlow.js n'est pas encore initialisé. Veuillez patienter.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (history.length > 0) {
        ...
        if (selectedFeatures.gaps) {
          for (let j = 0; j < 5; j++) {
            features.push(
              j < currentDraw.gagnants.length - 1
                ? (currentDraw.gagnants[j + 1] - currentDraw.gagnants[j]) / 90
                : 0,
            );
          }
        }
        ...
        const prng = new LCG(
          history[0]?.date || "SimulationLabDeterministicFallback",
        );
      ...
      showToast(
        "Erreur lors de l'entraînement du réseau de neurones.",
        "error",
      );
      audioEngine.play("error");
    } finally {
  ...
  const injectWeights = async () => {
    ...
    if (!trainingDataSource) {
      showToast(
        "Injection refusée : aucune information sur la source des données d'entraînement.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const epochsWithValLoss = epochLogs.filter(
        (e) => typeof e.val_loss === "number",
      );
      if (epochsWithValLoss.length < 5) {
        showToast(
          "Injection refusée : pas assez d'époques avec perte de validation mesurée.",
          "error",
        );
        return;
      }
      const firstValLoss = epochsWithValLoss[0].val_loss as number;
      const finalValLoss = epochsWithValLoss[epochsWithValLoss.length - 1]
        .val_loss as number;
      ...
      if (valLossDelta > 0) {
        ...
        const hasValidKeys = Object.keys(storeWeights).some((k) =>
          validKeys.includes(k as AlgoKey),
        );
        ...
        showToast(
          "Injection annulée : la perte de validation n'a pas diminué (aucune amélioration réelle mesurée).",
          "info",
        );
      }
    } catch (err) {
    ...
    <div className="space-y-8 animate-fade-in w-full">
      <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <p className="text-slate-400 text-sm font-medium mb-8">
          Entraînez un modèle de deep learning multi-couches de type Perceptron
          Séquentiel en temps réel dans votre navigateur, sur des données 100%
          réelles (historique du tirage ou fichier importé et validé). Aucune
          donnée synthétique n'est utilisée : si moins de 20 tirages
          exploitables sont disponibles, l'entraînement est désactivé plutôt que
          simulé.
        </p>
            ...
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-slate-900/50 hover:bg-slate-900 border border-slate-800 transition">
              ...
              <div>
                <span className="block text-xs font-bold text-slate-200">
                  Numéros Gagnants (Norm.)
                </span>
                <span className="text-[10px] text-slate-500">
                  Coordonnées cartésiennes de base [0-1]
                </span>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-slate-900/50 hover:bg-slate-900 border border-slate-800 transition">
              ...
              <div>
                <span className="block text-xs font-bold text-slate-200">
                  Intervalles Dynamiques (Gaps)
                </span>
                <span className="text-[10px] text-slate-500">
                  Vitesse d'écart et de dispersion des tirages
                </span>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl bg-slate-900/50 hover:bg-slate-900 border border-slate-800 transition">
              ...
              <div>
                <span className="block text-xs font-bold text-slate-200">
                  Indicateurs de Parité (Parity)
                </span>
                <span className="text-[10px] text-slate-500">
                  Cartographie binaire Pair/Impair
                </span>
              </div>
            </label>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            {fileData ? (
              fileDraws ? (
                <div className="mt-3 text-[10px] text-emerald-400 font-bold bg-emerald-500/10 px-3 py-1.5 rounded-lg text-center">
                  {fileDraws.length} tirage(s) valide(s) extraits de{" "}
                  {fileData.length} ligne(s) — prêt pour l'entraînement
                </div>
              ) : (
                <div className="mt-3 text-[10px] text-rose-400 font-bold bg-rose-500/10 px-3 py-1.5 rounded-lg text-center flex items-center justify-center gap-1.5">
                  ...
                  {fileParseWarning ||
                    "Fichier non exploitable pour l'entraînement."}
                </div>
              )
            ) : history.length >= 20 ? (
              ...
              <div className="mt-3 text-[10px] text-rose-400 font-bold bg-rose-500/10 px-3 py-1.5 rounded-lg text-center flex items-center justify-center gap-1.5">
                <AlertTriangle size={12} />
                Historique insuffisant ({history.length}/20 tirages minimum) —
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setHiddenLayersConfig(e.target.value as any)
                    }
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-white"
                  >
          ...
          <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800/80 flex flex-col justify-center">
            ...
            {isTfLoading ? (
              ...
              >
                <Activity size={16} className="animate-spin text-fuchsia-500" />{" "}
                Initialisation TensorFlow...
              </button>
            ) : trainingState === "training" ? (
              <button
                onClick={stopTraining}
                className="w-full py-4 bg-rose-500/20 hover:bg-rose-500/30 text-rose-400 border border-rose-500/30 rounded-xl flex items-center justify-center gap-2 font-black text-xs uppercase tracking-widest transition-all"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              <div className="flex flex-col items-center justify-center gap-4 text-center md:border-r md:border-slate-800/80 md:pr-6">
                ...
                <div className="flex gap-2.5">
                  {mlPrediction.map((n, i) => (
                    <div
                      key={i}
                      className="w-11 h-11 rounded-full bg-slate-900 border border-indigo-500 flex items-center justify-center text-indigo-400 font-black text-base shadow-[0_0_12px_rgba(99,102,241,0.25)]"
                    >
                      {n}
                    </div>
                  ))}
                ...
                <p className="text-[10px] text-slate-500 max-w-sm">
                  Ce vecteur de 5 boules a été obtenu par une étape de
                  feedforward sur le dernier état spatial connu du plateau.
                </p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-3 text-center">
                ...
                <p className="text-[10px] text-slate-400 max-w-xs">
                  Vous pouvez injecter la correction mathématique obtenue durant
                  cet entraînement directement pour modifier la balance du
                  moteur prédictif global.
                </p>
                <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800/80 h-72 relative">
            <h4 className="text-[10px] font-black uppercase text-slate-500 mb-4 flex items-center gap-2">
              <Activity size={12} /> Evolution de la Perte (Loss / MSE)
            </h4>
          ...
          <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800/80 h-72 relative">
            <h4 className="text-[10px] font-black uppercase text-slate-500 mb-4 flex items-center gap-2">
              <Activity size={12} /> Précision d'Apprentissage (Simulated
              Accuracy)
            </h4>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={epochLogs}>
