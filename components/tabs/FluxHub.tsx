          ...
          <div className="flex flex-row md:flex-col items-center md:items-start justify-between md:justify-center w-full md:w-auto md:min-w-[140px] text-left">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-xl md:text-2xl font-black text-slate-800 dark:text-white leading-none tracking-tighter">
                  ...
                  <span className="text-slate-400 dark:text-slate-500 text-sm ml-0.5">
                    {[
                      "JAN",
                      "FEV",
                      "MAR",
            ...
            <div className="md:hidden flex gap-2">
              ...
              <button
                ...
                className="p-2.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-xl active:scale-95"
              >
                <GitCompare size={14} />
              </button>
            </div>
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button
                ...
                className="p-3 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl hover:scale-110 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-all shadow-sm"
              ...
              <button
                ...
                className="p-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl hover:scale-110 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all shadow-sm"
                title="Audit Complet"
              >
                <SearchCode size={16} />
              </button>
...
export const FluxHub: React.FC<{ history: DrawResult[] }> = ({ history }) => {
  ...
  const [similarityTarget, setSimilarityTarget] = useState<DrawResult | null>(
    null,
  );
  ...
  const [cyberFilter, setCyberFilter] = useState<
    "all" | "entropy_high" | "entropy_low" | "harmonic_even" | "harmonic_odd"
  >("all");
    ...
    <div className="space-y-6 animate-fade-in pb-4 w-full max-w-7xl mx-auto px-1 md:px-0 h-[calc(100dvh-210px)] md:h-[calc(100dvh-220px)] flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-4 sm:p-5 rounded-[2rem] shadow-xl border border-slate-200/60 dark:border-slate-800/60 relative z-30 mx-auto w-full shrink-0">
        <div className="flex items-center gap-4 px-2 w-full md:w-auto">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wider mb-0.5">
              Master Flux
            </h3>
            <div className="flex items-center gap-2">
              <Clock size={12} className="text-indigo-400" />
              <span className="text-xs font-black text-slate-500 uppercase tracking-widest">
                {filteredHistory.length} Signatures
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2.5 w-full md:w-auto justify-center sm:justify-end">
          ...
          <div className="flex bg-slate-100/80 dark:bg-slate-950/50 p-1.5 rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
            <button
              ...
              className={`p-2.5 rounded-xl transition-all ${viewMode === "list" ? "bg-white dark:bg-slate-800 shadow-md text-indigo-600 dark:text-indigo-400 font-bold" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"}`}
              title="Liste"
            >
              <Activity size={16} />
            </button>
            <button
              ...
              className={`p-2.5 rounded-xl transition-all ${viewMode === "calendar" ? "bg-white dark:bg-slate-800 shadow-md text-indigo-600 dark:text-indigo-400 font-bold" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"}`}
              title="Calendrier"
            >
              <Calendar size={16} />
            </button>
          ...
          >
            ...
            <span>Métrique</span>
            {showAnalytics ? (
              <ChevronUp size={14} className="ml-1 opacity-70" />
            ) : (
          ...
          <div className="flex gap-2.5">
            <button
              onClick={() =>
                ExportService.exportHistoryToCSV(
                  history,
                  `Flux_${currentDrawName}`,
                )
              }
              className="p-3 bg-slate-50 dark:bg-slate-950/50 text-slate-600 dark:text-slate-300 rounded-2xl hover:bg-white dark:hover:bg-slate-800 transition border border-slate-200 dark:border-slate-800 hover:shadow-md"
              title="Export CSV"
            >
              <Download size={18} />
            </button>
            <button
              ...
              className="p-3 bg-emerald-600 text-white rounded-2xl shadow-lg shadow-emerald-500/30 hover:bg-emerald-500 active:scale-95 transition-all ring-2 ring-emerald-500/20 ring-offset-2 ring-offset-white dark:ring-offset-slate-900"
            >
          ...
          >
            <div className="bg-slate-900 dark:bg-slate-950/80 backdrop-blur-3xl p-6 md:p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl shadow-indigo-500/10 mb-4 space-y-8 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-black uppercase tracking-widest text-white">
                      Analyse Cybernétique du Flux Réel
                    </h4>
                    <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">
                      Calcul matriciel de l'entropie spectrale de Shannon et
                      auto-corrélation temporelle de Hurst.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {(
                    [
                      {
                        id: "all",
                        label: "Harmonique Totale",
                        desc: "Flux originel brut",
                      },
                      {
                        id: "entropy_high",
                        label: "Filtre Stochastique Stable",
                        desc: "Sommes équilibrées",
                      },
                      {
                        id: "entropy_low",
                        label: "Anomalies de Dérive",
                        desc: "Sommes déviantes",
                      },
                      {
                        id: "harmonic_even",
                        label: "Cohorte Paire",
                        desc: "Dominance paire",
                      },
                      {
                        id: "harmonic_odd",
                        label: "Cohorte Impaire",
                        desc: "Dominance impaire",
                      },
                    ] as const
                  ).map((cohort) => (
                    <button
              ...
              {trajectoryPoints.length > 0 ? (
                      ...
                      {trajectoryPoints.length > 1 &&
                        (() => {
                          const pathData = trajectoryPoints
                            .map((pt, index) => {
                              const x =
                                (index / (trajectoryPoints.length - 1)) * 1000;
                ...
                <div className="text-center p-4 text-[10px] text-slate-500 uppercase">
                  Aucune trajectoire évaluable avec le filtre actif.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 relative z-10">
                ...
                <div className="bg-slate-950/60 p-5 rounded-[2rem] border border-slate-800/60 hover:border-slate-700/60 transition-all flex flex-col justify-between group/card shadow-lg shadow-black/20">
                  <div className="flex justify-between items-start">
                    ...
                    <div
                      className="text-amber-400 p-1.5 bg-amber-500/10 rounded-lg group-hover/card:bg-amber-500/20 transition-colors"
                      title="Signifie le caractère persistant ou anti-persistant de la suite numérique."
                    >
                      <Info size={14} />
                    </div>
                  </div>
                  <div className="my-3">
                    <div
                      className={`text-3xl font-mono font-black tracking-tighter ${hurstStats.color}`}
                    >
                  ...
                  <p className="text-[10px] text-slate-500/80 leading-relaxed font-medium">
                    Si H &gt; 0.5, la dynamique du tirage est persistante. H
                    &lt; 0.5 signale un effet de retour systématique vers la
                    moyenne.
                  </p>
                </div>
                ...
                <div className="bg-slate-950/60 p-5 rounded-[2rem] border border-slate-800/60 hover:border-slate-700/60 transition-all flex flex-col justify-between group/card shadow-lg shadow-black/20">
                  ...
                  <div className="my-3">
                    <div className="text-3xl font-mono font-black tracking-tighter text-indigo-300">
                      ...
                      <span className="text-xs text-slate-500 font-sans tracking-widest uppercase ml-1">
                        bits
                      </span>
                    </div>
                  ...
                  <p className="text-[10px] text-slate-500/80 leading-relaxed font-medium">
                    Quantifie le désordre pur. Un score de 100% indique une
                    dispersion parfaitement uniforme sur tout le domaine du
                    tirage.
                  </p>
                </div>
                ...
                <div className="bg-slate-950/60 p-5 rounded-[2rem] border border-slate-800/60 hover:border-slate-700/60 transition-all flex flex-col justify-between group/card shadow-lg shadow-black/20">
                  ...
                  <p className="text-[10px] text-slate-500/80 leading-relaxed font-medium">
                    Mesure le déplacement géométrique du vecteur de tirage T à
                    T+1 sur une échelle ordonnée de numéros.
                  </p>
                </div>
                ...
                <div className="bg-slate-950/60 p-5 rounded-[2rem] border border-slate-800/60 hover:border-slate-700/60 transition-all flex flex-col justify-between group/card shadow-lg shadow-black/20">
                  ...
                  <p className="text-[10px] text-slate-500/80 leading-relaxed font-medium">
                    Centre gravitationnel de la somme des numéros du flux. Utile
                    pour identifier les déséquilibres de distribution de masse.
                  </p>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-10 gap-3">
                    {spectrumStats.raw.slice(0, 10).map((sp) => {
                      ...
                      return (
                        <div
                          key={sp.num}
                          className={`border p-3 rounded-[1.25rem] flex flex-col justify-between bg-slate-900/80 backdrop-blur-sm ${numBg} ${numColor} shadow-inner hover:scale-105 transition-transform cursor-default group/sp`}
                        >
              ...
              {topCorrelations.length > 0 && (
                <div className="bg-slate-950/40 p-5 md:p-6 rounded-3xl border border-slate-800/40 relative z-10">
                  <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 mb-4">
                    ...
                    Top 5 des Corrélations de Premier Degré (Paires les plus
                    Fréquentes)
                  </h5>
                  <div className="flex flex-wrap gap-3">
                    {topCorrelations.map((item, index) => (
                      ...
                      >
                        <span className="text-[11px] font-mono text-indigo-400 font-black opacity-80 group-hover/pair:opacity-100">
                          #{index + 1}
                        </span>
                        <div className="flex items-center gap-1.5 font-mono text-sm font-bold text-slate-200">
                          <span className="px-2 py-1 bg-slate-950 rounded-lg shadow-inner">
                            {item.pair[0]}
                          </span>
      ...
      {viewMode === "list" && (
        <div
          ref={parentRef}
          className="flex-1 w-full overflow-y-auto overflow-x-hidden min-h-0 bg-transparent scrollbar-hide py-2"
        >
          <div
            style={{
      ...
      {examiningDraw && (
        <DrawExamine
          result={examiningDraw}
          history={purifiedHistory}
          onClose={() => setExaminingDraw(null)}
        />
      )}
    </div>
  );
