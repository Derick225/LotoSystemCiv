...
export const DecisionTreeTab: React.FC<DecisionTreeTabProps> = ({ drawName }) => {
    ...
    const [globalImportance, setGlobalImportance] = useState<Array<{ name: string; val: number }>>([]);
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20">
            {/* Header Simplifié */}
            <div className={`p-8 md:p-8 rounded-3xl border shadow-2xl relative overflow-hidden transition-all duration-300 ${filterMode === 'shadow' ? 'bg-slate-950 border-rose-500/20' : 'bg-slate-900 border-slate-800'}`}>
            ...
            <div className="grid lg:grid-cols-12 gap-8">
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className={`text-xs font-medium ${selectedCandidate?.candidate === c.candidate ? 'text-white/80' : 'text-slate-400'}`}>
                ...
                <div className="lg:col-span-8 space-y-8">
                    {selectedCandidate ? (
                        <>
                        <div className={`p-6 rounded-3xl shadow-2xl relative overflow-hidden transition-all duration-500 bg-gradient-to-br ${theme.gradient} border ${filterMode === 'shadow' ? 'border-rose-900' : filterMode === 'average' ? 'border-blue-900' : 'border-emerald-900'}`}>
                        ...
                        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 backdrop-blur-md">
                            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center gap-2">
                                <BrainCircuit size={14} className="text-emerald-500 animate-pulse" /> Empreinte de Bifurcation &amp; Signaux d'Inférence
                            </h4>
                            <div className="grid sm:grid-cols-2 gap-4">
                                {selectedFeatures.map((featName, idx) => {
                                    ...
                                    if (featName === 'Critical Gap') {
                                        description = val > 0.7 ? "Écart critique saturé, propice à une sortie imminente." : "Écart stable, sous tension équilibrée.";
                                        badgeColor = val > 0.7 ? "bg-rose-500/10 text-rose-400 border border-rose-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20";
                                    } else if (featName === 'Frequency') {
                                        description = val > 0.7 ? "Dynamique de sortie très élevée sur la fenêtre de Breiman." : "Inertie de sortie basse, en phase de latence.";
                                        badgeColor = val > 0.7 ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border border-amber-500/20";
                                    } else if (featName === 'Shadow') {
                                        description = val > 0.6 ? "Forte probabilité d'ombre (densité d'écart en phase active)." : "Ombre résiduelle faible.";
                                        badgeColor = val > 0.6 ? "bg-purple-500/10 text-purple-400 border border-purple-500/20" : "bg-slate-800 text-slate-400 border border-slate-700/50";
                                    } else if (featName === 'Consensus Trap') {
                                        description = val > 0.5 ? "Attention : Surpoids de consensus détecté. Risque de faux positif." : "Absence de piège de consensus majeur.";
                                        badgeColor = val > 0.5 ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20";
                                    } else if (featName === 'Neighbor') {
                                        description = val > 0.6 ? "Proximité topologique immédiate avec les récents vainqueurs." : "Isolement spatial temporaire.";
                                        badgeColor = val > 0.6 ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-slate-800 text-slate-400 border border-slate-700/50";
                                    } else if (featName === 'Machine Leak') {
                                        description = val > 0.6 ? "Forte congruence harmonique avec la signature machine." : "Signature machine neutre.";
                                        badgeColor = val > 0.6 ? "bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/20" : "bg-slate-800 text-slate-400 border border-slate-700/50";
                                    } else if (featName === 'Norm Gap') {
                                        description = val > 0.7 ? "Probabilité cumulative de retour à la moyenne élevée." : "Index de retour stable.";
                                        badgeColor = val > 0.7 ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "bg-slate-800 text-slate-400 border border-slate-700/50";
                                    }

                                    return (
                                        <div key={featName} className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800 hover:border-slate-700 transition-all flex flex-col justify-between">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className="text-[10px] font-black uppercase text-slate-300 tracking-wider">{featName}</span>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className={`p-6 rounded-2xl border flex flex-col gap-4 ${filterMode === 'average' ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800/30' : 'bg-indigo-50 dark:bg-indigo-900/10 border-indigo-100 dark:border-indigo-800/30'}`}>
                            ...
                            <div className="p-6 rounded-2xl border bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 flex flex-col">
                                ...
                                <div className="space-y-3 flex-1 overflow-y-auto max-h-[150px] custom-scrollbar pr-2">
                                    {globalImportance.slice(0, 5).map((imp, idx) => (
                                        <div key={(imp as any).name} className="flex justify-between items-center group">
                                            <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300 truncate pr-2" title={(imp as any).name}>{(imp as any).name}</span>
                                            <div className="flex items-center gap-2 w-1/3">
                                                <div className="h-1.5 flex-1 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                                    <div className={`h-full ${idx === 0 ? 'bg-purple-500' : 'bg-slate-400 dark:bg-slate-500'}`} style={{ width: `${Math.min(100, Math.max(0, imp.val * 100))}%` }}></div>
                                                </div>
                                                <span className="text-[10px] font-black text-slate-500 w-8 text-right">{(imp.val * 100).toFixed(0)}%</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
