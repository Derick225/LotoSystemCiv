...
export const TemporalTab: React.FC<{ drawName: string }> = ({ drawName }) => {
  ...
  >([]);
  const [decayTrendData, setDecayTrendData] = useState<
    { number: number; score: number }[]
  >([]);
  const [currentMonthName, setCurrentMonthName] = useState("");
  const [dependencies, setDependencies] = useState<DependencyFlow[]>([]);
  const [loadingDeps, setLoadingDeps] = useState(false);
  const [crossMonthResonance, setCrossMonthResonance] =
    useState<CrossMonthResonanceAnalysis | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (history.length > 10) {
        ...
        await Promise.all(
          lastWinners.map(async (sourceNum) => {
            const assoc = await fetchAssociatedNumbers(
              sourceNum,
              drawName,
              history,
            );
    ...
    <div className="space-y-8 animate-fade-in pb-20 w-full overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
                Détection de périodicité (Auto-Corrélation)
              </p>
            </div>
          </div>
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <NumberBall
                    number={c.number}
                    size="md"
                    glow={c.nextDateEstimate === "CRITIQUE"}
                  />
                  <div className="text-right">
                    <div
                      className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg border ${c.nextDateEstimate === "CRITIQUE" ? "text-amber-500 border-amber-500/30 bg-amber-500/10 animate-pulse" : "text-slate-500 border-slate-700"}`}
                    >
                      {c.nextDateEstimate}
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (c.gap / c.avg) * 100)}%`,
                      }}
                    ></div>
                  </div>
                  <div className="text-xs text-slate-500 font-mono text-right">
                    Précision cycle: {Math.round(c.score)}% (σ ±
                    {c.stdDev.toFixed(1)})
                  </div>
                </div>
              </motion.div>
            ...
            <div className="lg:w-7/12 w-full space-y-6">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">
                  Matrice de corrélation temporelle croisée (vs{" "}
                  {crossMonthResonance.currentMonthName})
                </span>
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5">
                  {crossMonthResonance.allMonthsCorrelation.map((m) => {
                    const isPeak =
                      m.monthIndex === crossMonthResonance.sourceMonthIndex;
                    return (
                      <div
                        key={m.monthIndex}
                      ...
                      >
                        <div className="flex justify-between items-center mb-0.5">
                          <span
                            className={`text-[9px] font-black uppercase tracking-tight ${isPeak ? "text-indigo-300 animate-pulse" : "text-slate-500"}`}
                          >
                            {m.monthName}
                          </span>
                          {isPeak && (
                        ...
                        <span
                          className={`text-xs font-mono font-bold ${isPeak ? "text-emerald-400" : "text-slate-500"}`}
                        >
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">
                Noyau de Silverman : Mois de {currentMonthName}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 relative z-10">
            {seasonalData.map((item) => {
              ...
              return (
                <div
                  key={item.number}
                  className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/80 hover:border-indigo-100 dark:hover:border-indigo-900/40 transition-all"
                >
                  <div className="flex items-center gap-3">
                    <NumberBall number={item.number} size="sm" />
        ...
        <div className="bg-white dark:bg-slate-800/80 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest">
                Séries temporelles avec oubli exponentiel adaptatif
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 relative z-10">
            {decayTrendData.map((item) => (
              <div
                key={item.number}
                className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/80 hover:border-emerald-100 dark:hover:border-emerald-900/40 transition-all"
              >
                <div className="flex items-center gap-3">
                  <NumberBall number={item.number} size="sm" />
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                    Force de l'élan amorti
                  </span>
                </div>
                <div className="flex items-center gap-4 w-40">
                  <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-12 bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 relative overflow-hidden">
          ...
          <div className="space-y-4 relative z-10">
            {loadingDeps ? (
              <div className="py-20 text-center animate-pulse text-slate-400 font-bold text-[10px] uppercase tracking-widest">
                Calcul des vecteurs...
              </div>
            ) : (
              dependencies.map((dep, i) => (
                ...
                >
                  ...
                  <div className="flex-1 flex items-center justify-center text-slate-300 relative">
                    ...
                    <ArrowRight
                      size={16}
                      className="bg-white dark:bg-slate-800 rounded-full relative z-10 text-indigo-500"
                    />
                  </div>
                  ...
                  <div className="flex gap-2">
                    {dep.targets.map((tgt) => (
                      ...
                      >
                        <span className="text-[10px] font-bold text-indigo-400 opacity-0 group-hover/target:opacity-100 transition-opacity">
                          x{tgt.count}
                        </span>
