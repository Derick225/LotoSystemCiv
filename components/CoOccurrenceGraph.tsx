...
export const CoOccurrenceGraph: React.FC<CoOccurrenceGraphProps> = ({
  history,
}) => {
  ...
  const { nodes, links, maxCount } = useMemo(() => {
    if (!history || history.length === 0)
      return { nodes: [], links: [], maxCount: 0 };

    const filteredHistory = history.slice(0, timeFilter);
  ...
  return (
    <div className="glass-card p-6 md:p-8 rounded-[2.2rem] md:rounded-3xl shadow-sm neural-border relative overflow-hidden flex flex-col items-center">
      <div className="absolute top-0 left-0 p-4 opacity-5 pointer-events-none">
        <Network size={120} />
      </div>

      <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-center mb-8 relative z-10 gap-4">
        <div>
          <h4 className="text-xs md:text-sm font-black text-white flex items-center gap-2 uppercase tracking-widest mb-1">
            <Network className="text-fuchsia-500" size={18} /> Graphe de
            Symbiose
          </h4>
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">
            Analyse Combinatoire (Top 40 Paires)
          </p>
        </div>

        <div className="flex items-center gap-2 bg-slate-900/50 p-1 rounded-xl border border-white/5">
          <div className="pl-3 pr-2 flex items-center gap-2 text-slate-400">
            <Filter size={14} />
          </div>
          {[20, 50, 100, 500].map((val) => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${timeFilter === val ? "bg-fuchsia-500 text-white shadow-[0_0_10px_rgba(217,70,239,0.3)]" : "text-slate-500 hover:text-white hover:bg-white/5"}`}
            >
              {val} T
            </button>
          ))}
        ...
        {hoveredNode && (
          <div className="hidden md:flex px-4 py-2 bg-fuchsia-500/10 border border-fuchsia-500/20 rounded-xl items-center gap-3 animate-fade-in">
      ...
      <div className="relative w-full max-w-[500px] aspect-square">
        ...
        >
          ...
          {links.map((link, i) => {
            ...
            const controlX =
              center + (sourcePos.x + targetPos.x - 2 * center) * 0.2;
            const controlY =
              center + (sourcePos.y + targetPos.y - 2 * center) * 0.2;
            ...
            return (
              ...
              >
                <circle
                  r={nodeRadius}
                  fill={
                    isHovered ? "#d946ef" : isConnected ? "#8b5cf6" : "#1e293b"
                  }
      ...
      <div className="mt-6 flex items-center gap-4 text-xs font-bold text-slate-500 uppercase tracking-widest">
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-[#1e293b] border border-slate-500"></div>{" "}
          Nœud Standard
        </div>
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-[#8b5cf6]"></div> Symbiote
          Connecté
        </div>
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-[#d946ef] shadow-[0_0_5px_#d946ef]"></div>{" "}
          Nœud Actif
        </div>
      </div>
    </div>
