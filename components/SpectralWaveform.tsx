...
export const SpectralWaveform: React.FC<SpectralWaveformProps> = React.memo(
  ({ energy, hurst }) => {
    ...
    useEffect(() => {
      ...
      const render = () => {
        ...
        // Couleur continue basée sur l'intensité sans utiliser de pôle d'arrêt (Seuils binaires interdits)
        const colorEnergy = `hsl(${250 - Math.min(100, Math.max(0, energy)) * 2}, ${80 + Math.min(20, energy * 0.2)}%, ${60 + Math.min(15, energy * 0.15)}%)`;
        ...
        for (let x = 0; x <= width; x++) {
          ...
          const y =
            centerY + wave(x, t - 0.5, frequency * 1.1, amplitude * 0.8);
          if (x === 0) ctx.moveTo(x, y);
          else ctx.lineTo(x, y);
        }
        ctx.stroke();
