...
import { Cloud, HardDrive, Wifi, WifiOff, RefreshCw, CheckCircle, AlertTriangle, XCircle, Activity, ChevronRight } from 'lucide-react';
import { useSyncStatus } from '../hooks/useSyncStatus';

export const SyncMonitoringDashboard: React.FC = () => {
  const { isOnline, dbConnection, idbStats, isSyncing, lastChecked, checkStatus } = useSyncStatus();
    ...
    <div className="glass-card neural-border rounded-xl p-6 text-white font-mono relative overflow-hidden shadow-2xl">
      ...
      <div className="flex justify-between items-center mb-6 relative z-10 border-b border-slate-700/50 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold tracking-tight text-slate-100">Telemetry & Synchronization</h2>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-slate-500">LAST SYNC: {lastChecked.toLocaleTimeString()}</span>
          <button 
            onClick={checkStatus} 
            disabled={isSyncing}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        ...
        <div className="bg-slate-800/50 rounded-lg p-5 border border-slate-700 relative group overflow-hidden">
          ...
          <div className="space-y-1 mt-4">
             ...
             <div className="mt-4 pt-3 border-t border-slate-700">
               <MetricRow label="Payload Estimate" value={idbStats.totalStorage} />
             </div>
          </div>
        </div>
        ...
        <div className="md:hidden bg-slate-800/50 rounded-lg p-5 border border-slate-700 relative">
          <div className={`absolute top-0 left-0 w-full h-1 ${isOnline ? 'bg-cyan-400' : 'bg-red-500'} rounded-t-lg opacity-80`}></div>
           <div className="flex items-center gap-3">
            {isOnline ? <Wifi className="w-5 h-5 text-cyan-400" /> : <WifiOff className="w-5 h-5 text-red-500" />}
        ...
        <div className="bg-slate-800/50 rounded-lg p-5 border border-slate-700 relative overflow-hidden">
           <div className={`absolute top-0 left-0 w-full h-1 ${dbConnection === 'connected' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : dbConnection === 'checking' ? 'bg-amber-500' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]'} rounded-t-lg opacity-80`}></div>
          <div className="flex items-center gap-3 mb-4">
            <Cloud className={`w-5 h-5 ${dbConnection === 'connected' ? 'text-emerald-400 drop-shadow-[0_0_5px_rgba(52,211,153,0.5)]' : 'text-slate-500'}`} />
          ...
          <div className="flex items-center justify-center p-6 border border-slate-700/50 rounded bg-slate-900/50 mt-4 h-32 relative">
            ...
            <AnimatePresence mode="wait">
                {dbConnection === 'checking' && (
                  ...
                  >
                    ...
                    <span className="text-amber-400/80 text-[10px] tracking-widest font-bold">HANDSHAKING...</span>
                  </motion.div>
                )}
                {dbConnection === 'connected' && (
                  ...
                  >
                    ...
                    <span className="text-emerald-500/80 text-[10px] tracking-widest font-bold">SYNC ESTABLISHED</span>
                  </motion.div>
                )}
                {dbConnection === 'disconnected' && (
                  ...
                  >
                    ...
                    <span className="text-red-500/80 text-[10px] tracking-widest font-bold">NODE UNREACHABLE</span>
                  </motion.div>
                )}
            </AnimatePresence>
          </div>
           ...
           <div className="mt-4 pt-4 border-t border-slate-700 flex justify-between items-center text-[10px] text-slate-500 font-bold">
             <span>PROTOCOL</span>
             <span className={dbConnection === 'connected' ? 'text-emerald-400' : 'text-red-400'}>
