...
export const NeuralFeedbackPanel: React.FC = () => {
  ...
  const setAutonomousAgentActive = useNexusStore(state => state.setAutonomousAgentActive);
  ...
  const loadDriftDiagnostics = useCallback(async () => {
    ...
    try {
      ...
      const result = await LearningService.checkDrift(drawName, predictions, history);
  ...
  const handleForceLearning = async () => {
    if (!drawName) return;
    setLearning(true);
    try { audioEngine.play('scan'); } catch(e) {}
    try {
      ...
      <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-950/40 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col justify-between gap-4">
          <div>
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">MOTEUR CYBERNÉTIQUE</span>
              <span className={`px-2 py-0.5 rounded text-[8px] font-bold tracking-wider uppercase flex items-center gap-1 ${
                ...
                <span className={`h-1.5 w-1.5 rounded-full ${isAutonomousAgentActive ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`} />
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
              Activez le mode autonome pour laisser l'agent exécuter automatiquement les réalignements d'ADN par filtre de Kalman et corriger activement les dérives algorithmiques à chaque publication de tirage.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 mt-2">
            <label className="relative flex items-center justify-between gap-3 p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 w-full sm:w-auto cursor-pointer flex-1">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs font-black text-slate-800 dark:text-slate-200">Mode Autonome</span>
              </div>
            </label>
            ...
            >
              ...
              <span>{learning ? "Optimisation..." : "Forcer le Cycle d'Apprentissage"}</span>
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
          <div className="flex items-center justify-between gap-2 mb-3">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">DIAGNOSTICS DE DÉRIVE EN DIRECT</span>
            <span className="text-[9px] font-bold text-indigo-600 dark:text-indigo-400">{drawName}</span>
          </div>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-850 flex flex-col justify-between">
                ...
                <div className="mt-1">
                  ...
                  <span className="text-[9px] text-slate-400 block font-medium">
                    Moyenne récente (vs {driftMetrics.metrics?.baselineAvgHits?.toFixed(2) || "0.00"} base)
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-850 flex flex-col justify-between">
                ...
                <div className="mt-1">
                  <span className={`text-lg font-black font-mono ${
                    (driftMetrics.metrics?.structuralDivergence || 0) > 0.5 ? "text-rose-500" : "text-emerald-500"
                  }`}>
                    {driftMetrics.metrics?.structuralDivergence?.toFixed(3) || "0.000"}
                  </span>
                  <span className="text-[9px] text-slate-400 block font-medium">
                    Dérive structurelle : {driftMetrics.metrics?.structuralSeverity || "FAIBLE"}
                  </span>
                </div>
              </div>
            </div>
