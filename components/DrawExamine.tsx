...
export const DrawExamine: React.FC<DrawExamineProps> = ({
  result,
  history,
  onClose,
}) => {
  // Mode d'analyse : Structurel (Morphologie Spatiale, AC, Finales) ou Forensic (Benford, KS, Echo, Entropy)
  const [examineMode, setExamineMode] = useState<"structural" | "forensic">(
    "structural",
  );
  ...
  const DeviationGauge = ({
    ...
    const color =
      Math.abs(diff) > target * 0.3 ? "bg-orange-500" : "bg-emerald-500";
        ...
        <div className="md:w-1/2 p-6 sm:p-8 bg-black/20 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-800 overflow-y-auto">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 w-full">
            <div className="bg-slate-800/40 p-4 rounded-2xl border border-slate-700/60 shadow-sm">
              <span className="block text-[9px] font-black text-slate-500 uppercase tracking-wider mb-1">
                Écart Interne Max
              </span>
              <div className="text-lg font-black text-white font-mono">
                {metrics.maxInternalGap}{" "}
        ...
        <div className="md:w-1/2 flex flex-col h-[70vh] md:h-auto">
          <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-extrabold text-white text-md leading-none">
                  Rapport d'Audit
                </h4>
                <p className="text-[9px] text-slate-550 font-mono mt-1 uppercase tracking-wider">
                  HASH ID: {result.id.slice(0, 8)}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <section className="space-y-4">
                  <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    ...
                    DÉVIATIONS STOCHASTIQUES
                  </h5>
                  <div className="grid gap-4 bg-slate-950/20 p-4 rounded-2xl border border-slate-800/40">
                    <DeviationGauge
                      label="Somme Sigma (Σ)"
                      value={metrics.sum}
                      diff={metrics.deviation.sumDiff}
                      target={calibration.stdSum}
                    />
                    <DeviationGauge
                      label="Stabilité AC"
                      value={metrics.acValue}
                      diff={metrics.deviation.acDiff}
                      target={calibration.meanAC}
                    />
                  </div>
                </section>
                ...
                <section className="space-y-3">
                  <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    <Zap size={12} className="text-indigo-400" /> SIGNATURES DE
                    FORMES
                  </h5>
                  <div className="space-y-2">
                    {metrics.patterns.length > 0 ? (
                      metrics.patterns.map((p) => (
                        <div
                          key={p.id}
                          className="flex items-center gap-3.5 p-3.5 bg-slate-950/25 rounded-2xl border border-slate-800 hover:bg-slate-800/30 transition"
                        >
                          <div
                            className={`p-1.5 rounded-lg ${p.severity === "high" ? "bg-rose-500/10 text-rose-400 border border-rose-500/15" : "bg-indigo-500/10 text-indigo-400"}`}
                          >
                      ...
                      <div className="p-4 rounded-xl border border-dashed border-slate-800 text-center">
                        <p className="text-[9px] text-slate-500 font-black italic">
                          Aucune forme asymétrique récurrente
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <section className="space-y-3">
                  <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    <Hash size={12} className="text-indigo-400" /> DISTRIBUTION
                    DES FINALES
                  </h5>
                  <div className="flex gap-1.5">
                    {Object.entries(metrics.finalesCount).map(([f, count]) => (
                      <div
                        key={f}
                        className={`flex-1 py-2 px-1 rounded-xl text-center border transition-all ${count >= 2 ? "bg-indigo-650/15 border-indigo-500/30 text-indigo-450 font-black shadow-inner" : "bg-slate-950/20 border-slate-800 text-slate-500"}`}
                      >
                        <div className="text-[8px] font-mono leading-none">
                          F{f}
                        </div>
                        <div className="text-[11px] font-black font-mono mt-1">
                          x{count}
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-6 animate-fade-in select-none">
                ...
                <div className="bg-slate-950/40 p-5 rounded-3xl border border-slate-800/60 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <span className="block text-[8px] font-black text-slate-550 uppercase tracking-widest leading-none">
                        Intégrité Harmonique
                      </span>
                      <h5 className="text-sm font-black text-white">
                        Analyse des Écarts d'Urne
                      </h5>
                    </div>
                    <div
                      className={`text-2xl font-black font-mono ${forensicAudit.suspicionScore < 40 ? "text-emerald-400" : "text-rose-450"}`}
                    >
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[8px] font-black uppercase text-slate-500">
                      <span>
                        Score Suspicieux : {forensicAudit.suspicionScore}%
                      </span>
                      <span>
                        Confiance: {forensicAudit.benfordCompliance.toFixed(0)}%
                      </span>
                    </div>
                ...
                <section className="space-y-3">
                  <div className="text-[9px] font-black text-slate-505 uppercase tracking-widest flex items-center gap-1.5">
                    ...
                    Analyseurs Actifs ({forensicAudit.indicators.length})
                  </div>
                    ...
                    {forensicAudit.indicators.map((ind, idx) => (
                      <div
                        ...
                        onClick={() => {
                          audioEngine.play("click");
                          setActiveFormulaId(
                            activeFormulaId === ind.type ? null : ind.type,
                          );
                        }}
                      ...
                      >
                        <div className="flex justify-between items-center w-full">
                          <span className="text-[10px] font-black uppercase text-slate-300 flex items-center gap-1.5 leading-none">
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${ind.severity === "critical" ? "bg-rose-500 animate-pulse" : ind.severity === "high" ? "bg-rose-400" : "bg-amber-400"}`}
                            ></span>
                            {ind.label}
                          </span>
                        ...
                        {activeFormulaId === ind.type && (
                          ...
                          >
                            <span className="block text-[7px] text-slate-600 font-black uppercase tracking-wider mb-1">
                            ...
                            <div className="whitespace-pre-wrap select-all">
                              {ind.type === "HARMONY" &&
                                "P(A) = 1 / (1 + e^(β * (σ² - τ)))"}
                              {ind.type === "BENFORD" &&
                                "D_Benford = Σ (f_obs - P_Benford)²"}
                              {ind.type === "KS_TEST" &&
                                "D_n = sup |F_n(x) - F(x)|"}
                              {ind.type === "ECHO" &&
                                "P(X = k) = M_C(k) * M_U-1(n-k) / M_U(n)"}
                              {ind.type === "SIGMA" &&
                                "||Z||_L2 = sqrt(Z_sum² + Z_gap²)"}
                              {ind.type === "ENTROPY" &&
                                "H_norm = - Σ p_i log2(p_i) / log2(U)"}
                              {![
                                "HARMONY",
                                "BENFORD",
                                "KS_TEST",
                                ...
                                "Modèle d'interférence asymptotique continu"}
                            </div>
                            <span className="block text-[7px] text-slate-550 mt-1 font-bold">
                              Calculé à 100% sans marge d'erreur magique.
                            </span>
                          </motion.div>
                        )}
                      </div>
                ...
                <section className="p-4 bg-slate-950/20 rounded-2xl border border-slate-850 space-y-2">
                  <div className="text-[9px] font-black text-slate-505 uppercase tracking-wider">
                    Calibration Bayésienne Prochaine
                  </div>
                  <p className="text-[9px] text-slate-400 font-semibold leading-relaxed">
                    Ce tirage alimente les gradients directionnels de l'Oracle.
                    Son inclusion ne déforme pas l'entropie asymptotique
                    générale, garantissant que l'attracteur converge sagement
                    vers l'espace de Shannon.
                  </p>
                </section>
              </div>
            )}
