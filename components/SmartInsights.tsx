...
export const SmartInsights: React.FC<SmartInsightsProps> = ({ drawName }) => {
  const smartInsights = useNexusStore((state) => state.smartInsights);
  ...
  const handleNavigate = (insight: SmartInsight) => {
    ...
    if (insight.id.includes("vol-") || insight.id.includes("hurst-")) {
      ...
      mainTab = "Signaux";
      subTab = "stats"; // Gaps -> Stats
    } else if (insight.id.includes("clock-")) {
      mainTab = "Signaux";
      subTab = "temporal"; // Cycles -> Temporal
    }

    navigateToModule(mainTab, subTab);
  };

  if (nexusLoading)
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="h-32 animate-pulse bg-slate-900/50 rounded-2xl border border-slate-800"></div>
        <div className="h-32 animate-pulse bg-slate-900/50 rounded-2xl border border-slate-800"></div>
      </div>
    );
      ...
      {smartInsights.map((insight) => {
        ...
        return (
          <div
            ...
            className={`
                            p-6 rounded-2xl border-l-4 shadow-xl relative overflow-hidden flex items-start gap-5 transition-all hover:scale-[1.02] cursor-pointer group
                            ${theme.bg} border-slate-200 dark:border-slate-800 ${theme.border}
                        `}
          >
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner flex-shrink-0 ${theme.iconBg} group-hover:scale-110 transition-transform`}
            >
              <theme.icon size={28} />
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-2">
                <h4
                  className={`font-black text-sm uppercase tracking-tight truncate pr-2 ${theme.text}`}
                >
                  {insight.title}
                </h4>
                <span
                  className={`text-xs font-black px-2 py-1 rounded-lg border uppercase tracking-widest ${theme.iconBg} bg-opacity-20 border-opacity-20`}
                >
            ...
            {/* Decor */}
            <div
              className={`absolute -bottom-6 -right-6 w-24 h-24 rounded-full opacity-5 pointer-events-none ${theme.iconBg.split(" ")[0]}`}
            ></div>
          </div>
        );
      })}
