...
import { 
  ...
  ShieldCheck, RefreshCw, AlertTriangle, CloudOff,
  FlaskConical, Microscope, Clock, Navigation, Brain
} from 'lucide-react';
...
export const DrawDetails: React.FC = () => {
  ...
  const activeMainTab = useNexusStore(state => state.activeMainTab) as MainTab;
  ...
  useEffect(() => {
      if (activeMainTab && activeSubTab) {
          ...
          setTimeout(() => {
              ...
              window.dispatchEvent(new CustomEvent(`NAVIGATE_SUB_${activeMainTab.toUpperCase()}`, { 
  ...
  const allTabs = [
    ...
    { id: 'Simulation', icon: FlaskConical, label: 'Backtest', desc: 'Simulation Historique' },
    { id: 'Labo', icon: FlaskConical, label: 'Labo ML', desc: 'Laboratoire TF.js' },
    { id: 'WhatIf', icon: Activity, label: 'What-If', desc: 'Simulateur Continu' },
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 md:gap-6 bg-slate-900/60 p-5 md:p-8 rounded-3xl md:rounded-3xl border border-white/5 backdrop-blur-xl w-full shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-2 md:space-y-3 w-full md:w-auto relative z-10">
            <div className="flex items-center gap-2 md:gap-3">
                ...
                <span className="text-[10px] md:text-xs font-mono text-slate-500 uppercase">{history.length} Séquences</span>
            </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 relative z-10 w-full md:w-auto justify-end items-center">
            ...
            >
                ...
                <div className="text-left leading-none">
                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-400 transition-colors">Forensic</div>
                    <div className={`text-[11px] font-black uppercase mt-0.5 transition-colors duration-300 ${
                ...
                <div className={`w-8 h-4.5 rounded-full transition-colors duration-300 relative flex items-center p-0.5 ml-1 md:ml-2 ${
                    isForensicOptimized ? 'bg-emerald-500' : 'bg-slate-600'
                }`}>
                    <div className={`w-3.5 h-3.5 rounded-full bg-white shadow-sm transform transition-transform duration-300 ease-out ${
            ...
            >
                <RefreshCw size={16} className={`group-hover:rotate-180 transition-transform duration-300 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <div className={`border px-3 md:px-6 py-2.5 md:py-4 rounded-xl md:rounded-2xl flex items-center gap-2 md:gap-3 backdrop-blur-md ${
                !isOnline ? 'bg-red-500/10 border-red-500/20' :
                dbConnection === 'connected' ? 'bg-emerald-500/10 border-emerald-500/20' : 
                dbConnection === 'checking' ? 'bg-amber-500/10 border-amber-500/20' : 
                'bg-rose-500/10 border-rose-500/20'
            }`}>
               {!isOnline ? <CloudOff size={16} className="text-red-500" /> : 
                dbConnection === 'connected' ? <ShieldCheck size={16} className="text-emerald-500" /> : 
                dbConnection === 'checking' ? <RefreshCw size={16} className="text-amber-500 animate-spin" /> :
                <AlertTriangle size={16} className="text-rose-500" />}
                <div className="text-left">
                    <div className={`text-[10px] md:text-xs font-black uppercase tracking-widest leading-none ${
                         !isOnline ? 'text-red-500' :
                         dbConnection === 'connected' ? 'text-emerald-500' : 
                         dbConnection === 'checking' ? 'text-amber-500' : 
                         'text-rose-500'
                    }`}>Status</div>
                    <div className={`text-xs md:text-xs font-black uppercase mt-0.5 ${
                         !isOnline ? 'text-red-400' :
                         dbConnection === 'connected' ? 'text-emerald-400' : 
                         dbConnection === 'checking' ? 'text-amber-400' : 
                         'text-rose-400'
                    }`}>
                      {!isOnline ? 'Hors Ligne' : 
                       dbConnection === 'connected' ? 'Opérationnel' : 
                       dbConnection === 'checking' ? 'Connexion...' : 
                       'Désynchronisé'}
                    </div>
                </div>
            </div>
        ...
        <nav className="flex bg-slate-100 dark:bg-slate-800/80 p-1 rounded-[2rem] md:rounded-2xl border border-slate-200 dark:border-slate-700 overflow-x-auto scrollbar-hide shadow-inner w-full md:w-fit max-w-full">
          {tabs.map((t) => (
            <button
              ...
              `}
            >
              <t.icon size={14} className={`relative z-10 ${activeMainTab === t.id ? 'animate-bounce-subtle' : ''}`} />
              <span className="text-xs md:text-xs uppercase tracking-widest leading-none relative z-10">{t.label}</span>
              {activeMainTab === t.id && (
        ...
        <LocalErrorBoundary key={activeMainTab}>
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center py-32 gap-6 animate-pulse bg-slate-900/20 rounded-3xl border border-dashed border-slate-800">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">Initialisation du module {activeMainTab}...</p>
            </div>
          }>
            {activeMainTab === 'Flux' && <FluxHub history={history} />}
            {activeMainTab === 'Signaux' && <SignalHub />}
            {activeMainTab === 'Topologie' && <TopologyHub drawName={drawName} />}
            {activeMainTab === 'Oracle' && <OracleHub drawName={drawName} />}
            {activeMainTab === 'Simulation' && <SimulationTab drawName={drawName} />}
            {activeMainTab === 'Labo' && <SimulationLab drawName={drawName} />}
            {activeMainTab === 'WhatIf' && <WhatIfSimulatorTab drawName={drawName} />}
            {activeMainTab === 'Forensic' && <ForensicHub drawName={drawName} />}
          </Suspense>
        </LocalErrorBoundary>
      </div>
    </div>
