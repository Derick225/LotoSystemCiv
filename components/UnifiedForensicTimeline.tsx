...
import { Activity, TrendingUp, Zap, TrendingDown, Trash2 } from "lucide-react";
...
export const UnifiedForensicTimeline: React.FC<
  UnifiedForensicTimelineProps
> = ({ reports, selectedReport, onSelectReport, onDeleteReport }) => {
  ...
  const trendStats = useMemo(() => {
    if (reports.length === 0)
      return { avgDrift: 0, avgAlignment: 0, trend: "STABLE" };

    const totalDrift = reports.reduce(
      (acc, r) => acc + (r.divergenceMetric ?? 0),
      0,
    );
    const avgDrift = totalDrift / reports.length;

    const totalAlignment = reports.reduce((acc, r) => {
      const hits = Array.isArray(r.matches)
        ? r.matches.filter((m) => m.errorType === "Hit").length
        : 0;
    ...
    const firstHalfDrift =
      firstHalf.length > 0
        ? firstHalf.reduce((acc, r) => acc + (r.divergenceMetric ?? 0), 0) /
          firstHalf.length
        : 0;
    const secondHalfDrift =
      secondHalf.length > 0
        ? secondHalf.reduce((acc, r) => acc + (r.divergenceMetric ?? 0), 0) /
          secondHalf.length
        : 0;

    let trend: "IMPROVING" | "DEGRADING" | "STABLE" = "STABLE";
    if (secondHalfDrift < firstHalfDrift - 2)
      trend = "IMPROVING"; // Lower drift is better
  ...
  const svgPath = useMemo(() => {
    ...
    const points = chronologicalReports.map((r, idx) => {
      const x =
        paddingX +
        (idx / (chronologicalReports.length - 1)) * (width - 2 * paddingX);
    ...
    return (
      <div className="p-8 bg-slate-50/50 dark:bg-slate-900/10 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 text-center">
        <p className="text-xs text-slate-500 dark:text-slate-400 italic">
          Aucun rapport d'autopsie disponible pour la frise chronologique.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium">
            Flux chronologique continu des tirages. Visualisation en temps réel
            de l'écart de dérive spatiale et de l'alignement physique.
          </p>
        </div>
        ...
        <div className="relative bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl p-2 border border-slate-100 dark:border-slate-850/50 overflow-hidden">
          ...
          >
            ...
            <line
              ...
              y2={height / 2}
              stroke="currentColor"
              className="text-slate-100 dark:text-slate-900/50"
              strokeWidth="1"
              strokeDasharray="4,4"
            />
            ...
            {chronologicalReports.map((r, idx) => {
              const x =
                paddingX +
                (idx / (chronologicalReports.length - 1)) *
                  (width - 2 * paddingX);
              const driftVal = r.divergenceMetric ?? 40;
              const y = paddingY + (driftVal / 100) * (height - 2 * paddingY);
      ...
      <div className="overflow-x-auto scrollbar-hide pb-4 -mx-2 px-2">
        <div className="flex gap-4 min-w-max">
          {chronologicalReports.map((rep) => {
            const hits = Array.isArray(rep.matches)
              ? rep.matches.filter((m) => m.errorType === "Hit").length
              : 0;
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-1">
                    {(() => {
                      let comboList =
                        Array.isArray(rep.combo) && rep.combo.length >= 5
                          ? rep.combo.slice(0, 5)
                          : [];
                      if (comboList.length < 5) {
                        ...
                        if (Array.isArray(rep.matches)) {
                          rep.matches.forEach((m) => {
                            if (typeof m.actual === "number" && m.actual > 0)
                              setNums.add(m.actual);
                          });
                        }
                        comboList = Array.from(setNums)
                          .sort((a, b) => a - b)
                          .slice(0, 5);
                      }
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-900 font-mono text-[9px]">
                    <div>
                      <span className="block text-slate-400 font-bold uppercase tracking-wide">
                        Dérive
                      </span>
                      <span
                        className={`font-black ${spatialDrift > 60 ? "text-rose-400" : spatialDrift > 30 ? "text-amber-400" : "text-indigo-400"}`}
                      >
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div
                    className={`px-2 py-0.5 rounded text-[8px] font-black border uppercase tracking-wider ${badgeStyle}`}
                  >
                    {hits === 5
                      ? "PARFAIT"
                      : hits >= 3
