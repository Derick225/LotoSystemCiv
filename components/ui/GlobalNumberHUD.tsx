...
export const GlobalNumberHUD: React.FC = () => {
  ...
  const symmetryAnalysis = React.useMemo(() => {
    ...
    const activeTirage =
      ...
      lastPrediction.suggestedNumbers.length > 0
        ? lastPrediction.suggestedNumbers
    ...
    const neighbors = activeTirage.filter(
      (n) => Math.abs(n - hoveredNumber) === 1,
    );
    const sameEndings = activeTirage.filter(
      (n) => n !== hoveredNumber && n % 10 === hoveredNumber % 10,
    );
    const digitMirrors = activeTirage.filter(
      (n) => n !== hoveredNumber && getDigitMirror(n) === hoveredNumber,
    );
    const centerMirrors = activeTirage.filter(
      (n) => n !== hoveredNumber && n + hoveredNumber === 91,
    );
    ...
    return {
      ...
      hasSelfRelations:
        neighbors.length > 0 ||
        sameEndings.length > 0 ||
        digitMirrors.length > 0 ||
        centerMirrors.length > 0,
      relations: {
        neighbors,
        sameEndings,
        digitMirrors,
      ...
      mirrorTarget,
      tirageName: isPredictionMode
        ? "Prédiction Active"
        : "Dernier Tirage (Historique)",
    };
      ...
      {hoveredNumber && (
        ...
        >
          ...
          >
            {/* Background FX Glow */}
            <div
              className={`absolute top-0 inset-x-0 h-48 bg-gradient-to-b ${gradient} opacity-50 pointer-events-none`}
            ></div>
            ...
            <div className="w-full space-y-5 relative z-10">
              ...
              <div className="flex justify-between items-center border-b border-white/10 pb-3">
                <span
                  className={`text-[11px] font-black uppercase tracking-widest ${statusColor} flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full`}
                >
                  <Activity
                    size={14}
                    className={status === "BOUILLANT" ? "animate-pulse" : ""}
                  />{" "}
                  {status}
                </span>
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="flex flex-col items-center justify-between p-3 bg-gradient-to-b from-white/10 to-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                  ...
                  <span className="text-2xl font-black text-white">{freq}</span>
                  {/* Mini gauge */}
                  <div className="w-full h-1 bg-black/40 rounded-full mt-2 overflow-hidden">
                    <div
                      className="h-full bg-emerald-500 rounded-full"
                      style={{ width: `${Math.min(100, (freq / 15) * 100)}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-between p-3 bg-gradient-to-b from-white/10 to-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                  ...
                  <span className="text-2xl font-black text-white">{gap}</span>
                  <div className="w-full h-1 bg-black/40 rounded-full mt-2 overflow-hidden flex transform -scale-x-100">
                    ...
                    <div
                      ...
                      style={{
                        width: `${Math.max(0, 100 - (gap / 30) * 100)}%`,
                      }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-between p-3 bg-gradient-to-b from-white/10 to-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                  <span className="text-xs font-bold text-slate-400 uppercase mb-2 flex flex-col items-center gap-1">
                    <Zap
                      size={12}
                      className={`${energy > 70 ? "text-rose-400/70" : "text-amber-400/70"}`}
                    />
                    Énergie
                  </span>
                  <span
                    className={`text-2xl font-black ${energy > 70 ? "text-rose-400" : energy > 40 ? "text-amber-400" : "text-white"}`}
                  >
                    {Math.round(energy)}
                  </span>
                  <div className="w-full h-1 bg-black/40 rounded-full mt-2 overflow-hidden">
                    <div
                      className={`h-full rounded-full ${energy > 70 ? "bg-rose-500" : energy > 40 ? "bg-amber-500" : "bg-slate-500"}`}
                ...
                <div className="space-y-2.5 p-3.5 bg-slate-950/40 border border-white/5 rounded-2xl relative overflow-hidden">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px]">
                      ...
                      <span
                        className={`font-mono font-black ${symmetryAnalysis.analysis.isAnomalous ? "text-rose-400" : "text-indigo-400"}`}
                      >
                    ...
                    <div className="w-full h-1.5 bg-black/40 rounded-full overflow-hidden relative">
                      <div
                        ...
                        style={{
                          width: `${symmetryAnalysis.analysis.tensionIndex}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  {symmetryAnalysis.analysis.isAnomalous && (
                    ...
                    >
                      <AlertTriangle
                        size={14}
                        className="text-rose-400 shrink-0 self-start mt-0.5 animate-pulse"
                      />
                      <div>
                        <span className="font-black uppercase tracking-wide block text-[8px] text-rose-400 mb-0.5">
                          ⚠️ ALERTE STRUCTURELLE : SYMÉTRIE FORCÉE
                        </span>
                        Le tirage actif exhibe des configurations symétriques
                        non-naturelles. Risque d'attracteur stochastique
                        focalisé.
                      </div>
                    </motion.div>
                  )}
                  ...
                  <div className="space-y-1.5 pt-2 border-t border-white/5">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">
                      Affinités du #{hoveredNumber} :
                    </span>

                    {!symmetryAnalysis.hasSelfRelations ? (
                      <span className="text-[10px] text-slate-400 block italic">
                        Aucune liaison symétrique active dans le tirage.
                      </span>
                    ) : (
                      <div className="grid grid-cols-2 gap-2 text-[9px]">
                        {symmetryAnalysis.relations.neighbors.length > 0 && (
                          <div className="bg-white/[0.02] p-1.5 rounded-lg border border-white/5">
                            <span className="text-indigo-300 block font-semibold leading-none mb-0.5">
                              Voisinage direct
                            </span>
                            <span className="text-white font-mono">
                              {symmetryAnalysis.relations.neighbors.join(", ")}
                            </span>
                          </div>
                        )}
                        {symmetryAnalysis.relations.sameEndings.length > 0 && (
                          <div className="bg-white/[0.02] p-1.5 rounded-lg border border-white/5">
                            <span className="text-emerald-300 block font-semibold leading-none mb-0.5">
                              Même finale
                            </span>
                            <span className="text-white font-mono">
                              {symmetryAnalysis.relations.sameEndings.join(
                                ", ",
                              )}
                            </span>
                          </div>
                        ...
                        {symmetryAnalysis.relations.digitMirrors.length > 0 && (
                          <div className="bg-amber-500/5 p-1.5 rounded-lg border border-amber-500/15">
                            <span className="text-amber-300 block font-semibold leading-none mb-0.5">
                              Miroir Chiffre
                            </span>
                            <span className="text-white font-mono">
                              {symmetryAnalysis.relations.digitMirrors.join(
                                ", ",
                              )}
                            </span>
                          </div>
                        )}
                        {symmetryAnalysis.relations.centerMirrors.length >
                          0 && (
                          <div className="bg-purple-500/5 p-1.5 rounded-lg border border-purple-500/15">
                            <span className="text-purple-300 block font-semibold leading-none mb-0.5">
                              Iso-Centre
                            </span>
                            <span className="text-white font-mono">
                              {symmetryAnalysis.relations.centerMirrors.join(
                                ", ",
                              )}
                            </span>
                          </div>
              ...
              >
                ...
                <ArrowRight
                  size={16}
                  className="group-hover:translate-x-1 group-hover:text-white transition-all text-white/50"
                />
              </motion.button>
            </div>
          </motion.div>
