...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const commands = [
    ...
    {
      id: "nav-oracle",
      label: "Aller à l'Oracle Prédictif",
    ...
    {
      id: "nav-signals",
      label: "Aller aux Signaux Stochastiques",
      icon: <Terminal size={16} />,
      group: "Navigation",
      action: () => onNavigate("Signaux"),
    },
    {
      id: "nav-topology",
      label: "Aller à la Topologie & Réseaux",
      icon: <Network size={16} />,
      group: "Navigation",
      action: () => onNavigate("Topologie"),
    },
    {
      id: "nav-simulation",
      label: "Aller aux Simulations & Replay",
      icon: <Zap size={16} />,
      group: "Navigation",
    ...
    {
      id: "nav-forensic",
      label: "Aller au Forensic & Autopsie",
      icon: <ShieldCheck size={16} />,
      group: "Navigation",
      action: () => onNavigate("Forensic"),
    },
    {
      id: "nav-dna-history",
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-start justify-center pt-[15vh] p-4 animate-fade-in"
      onClick={onClose}
    >
      <div
        className="w-full max-w-xl bg-slate-900 rounded-3xl shadow-2xl border border-slate-700 overflow-hidden flex flex-col"
      ...
      >
        <div className="flex items-center gap-4 p-6 border-b border-slate-800">
          ...
          <div className="hidden md:flex gap-2">
            <kbd className="px-2 py-1 bg-slate-800 rounded-lg text-[10px] font-black text-slate-400 border border-slate-700">
              esc
            </kbd>
          </div>
        </div>
            ...
            filtered.map((cmd, idx) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`p-2 rounded-xl ${selectedIndex === idx ? "bg-white/20" : "bg-slate-800"}`}
                  >
                    {cmd.icon}
                  </div>
                  <div className="text-left">
                    <div
                      className={`text-sm font-bold ${selectedIndex === idx ? "text-white" : "text-slate-300"}`}
                    >
                      {cmd.label}
                    </div>
                    {cmd.group && (
                      <div
                        className={`text-[10px] font-bold uppercase tracking-wider ${selectedIndex === idx ? "text-indigo-200" : "text-slate-600"}`}
                      >
                        {cmd.group}
                      </div>
                    )}
