  ...
  const items = [
    {
      ...
      text: `DERNIER: ${lastDraw.drawName.toUpperCase()} [ ${lastDraw.gagnants.join("-")} ]`,
      color: "text-white",
    },
    {
      icon: <Clock size={12} className="text-indigo-400" />,
      text: `PROCHAIN: ${nextTime}`,
      color: "text-indigo-200",
    },
    {
      icon: <TrendingUp size={12} className="text-emerald-400" />,
      text: `RÉGIME: ${regime?.regime || "CALIBRATION"}`,
      color: "text-emerald-200",
    },
    {
      icon: <AlertTriangle size={12} className="text-rose-400" />,
      text: `VOLATILITÉ: ${volatility?.score || 0}%`,
      color:
        volatility?.score && volatility.score > 60
          ? "text-rose-300"
          : "text-slate-400",
    },
  ];
