...
export const AppShell: React.FC<AppShellProps> = ({ 
  ...
  return (
    <div className={`min-h-screen text-slate-200 selection:bg-indigo-500/30 transition-colors duration-500 font-sans flex flex-col relative overflow-x-hidden w-full ${isFocusMode ? 'bg-black' : ''}`}>
      <header className={`fixed top-0 left-0 right-0 z-50 w-full overflow-hidden transition-all duration-700 ${isFocusMode ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
        <MarqueeTicker />
        <div className={`mx-2 md:mx-4 mt-2 transition-all duration-500 ${scrolled ? 'scale-[0.98]' : 'scale-100'}`}>
            <div className={`container mx-auto px-4 md:px-6 min-h-[4rem] md:min-h-[5rem] py-2 md:py-0 rounded-3xl md:rounded-2xl border shadow-2xl transition-all duration-500 flex justify-between items-center safe-top
                ${scrolled ? 'bg-nexus-950/90 backdrop-blur-2xl border-white/10' : 'bg-nexus-900/40 backdrop-blur-xl border-white/5'}
            `}>
                <div onClick={onReset} className="flex items-center gap-1.5 md:gap-4 cursor-pointer group select-none shrink-0">
                    <img src="/logo-full.svg" alt="LotoPro Platinum Elite" className="h-6 xs:h-8 md:h-11 w-auto drop-shadow-xl group-hover:drop-shadow-2xl transition-all" />
                    {isOffline && (
                        <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-rose-500/10 border border-rose-500/30 rounded-full text-rose-400 ml-2 animate-pulse">
                            <WifiOff size={12} />
                            <span className="text-[10px] font-black tracking-widest">HORS LIGNE</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex items-center gap-1 xs:gap-1.5 md:gap-3">
                    <InstallButton />
                    <button onClick={() => { audioEngine.play('click'); setFocusMode(true); }} className={`p-1.5 xs:p-2 md:p-3.5 rounded-lg md:rounded-2xl transition-all border bg-white/5 text-slate-400 hover:text-white border-white/10 flex items-center justify-center`} title="Mode Focus">
                        <Maximize size={16} />
                    </button>
                    <button onClick={() => { audioEngine.play('click'); setTheme(theme === 'dark' ? 'light' : 'dark'); }} className={`p-1.5 xs:p-2 md:p-3.5 rounded-lg md:rounded-2xl transition-all border bg-white/5 text-slate-400 hover:text-white border-white/10 flex items-center justify-center`}>
                        {theme === 'dark' ? <span className="text-xs xs:text-sm">☀️</span> : <span className="text-xs xs:text-sm">🌙</span>}
                    </button>
                    ...
                    <button onClick={() => { audioEngine.play('click'); onLogout(); }} className="p-1.5 xs:p-2 md:p-3.5 bg-rose-500/10 rounded-lg md:rounded-2xl text-rose-400 border border-rose-500/20 active:scale-90 transition-all flex items-center justify-center"><LogOut size={14} /></button>
                    <button 
                      onClick={() => { 
                        if(confirm("Réinitialiser l'Infrastructure ? Cela videra le cache local et rechargera Nexus.")) {
      ...
      <AnimatePresence>
        {isFocusMode && (
          <motion.button
            ...
            exit={{ opacity: 0, y: -20 }}
            onClick={() => { audioEngine.play('click'); setFocusMode(false); }}
      ...
      <main className={`container mx-auto px-3 md:px-4 max-w-7xl flex-1 relative z-0 w-full overflow-x-hidden transition-all duration-700 ${isFocusMode ? 'pt-8 pb-8' : 'pt-32 md:pt-44 pb-32 md:pb-40'}`}>
      ...
      <div className={`fixed left-6 right-6 z-[90] md:hidden transition-all duration-700 ${isFocusMode ? 'translate-y-[150%] opacity-0' : 'translate-y-0 opacity-100'}`} style={{ bottom: 'calc(1.5rem + env(safe-area-inset-bottom))' }}>
        <div className="bg-slate-900/90 backdrop-blur-3xl border border-white/10 rounded-3xl p-2 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] flex items-center relative overflow-x-auto overflow-y-hidden snap-x snap-mandatory scrollbar-hide hide-scrollbar w-full">
            ...
            <div className="flex gap-2 w-max px-2">
                {navItems.map(item => (
                    ...
                    >
                        {viewMode === item.id && !showWallet && (
                            <motion.div layoutId="nav-pill" className="absolute inset-0 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-600/30 -z-10" />
                        )}
                ...
                >
                    {showWallet && (
                        <motion.div layoutId="nav-pill" className="absolute inset-0 bg-emerald-600 rounded-2xl shadow-lg shadow-emerald-600/30 -z-10" />
                    )}
