...
export const AppShell: React.FC<AppShellProps> = ({ 
  ...
  return (
    <div className="min-h-screen text-slate-200 selection:bg-indigo-500/30 transition-colors duration-500 font-sans flex flex-col relative overflow-x-hidden w-full">
        ...
        <div className={`mx-2 md:mx-4 mt-2 transition-all duration-500 ${scrolled ? 'scale-[0.98]' : 'scale-100'}`}>
            ...
            `}>
                ...
                <div className="flex items-center gap-1.5 md:gap-3">
                    ...
                    <button onClick={() => { audioEngine.play('click'); setShowWallet(!showWallet); }} className={`p-2.5 md:p-3.5 rounded-xl md:rounded-2xl transition-all border ${showWallet ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-white/5 text-slate-400 border-white/10'}`}>
                        <Wallet size={16} />
                    </button>
      ...
      <main className="container mx-auto px-2 md:px-4 pt-28 md:pt-44 pb-36 max-w-7xl flex-1 relative z-0 w-full overflow-x-hidden">
