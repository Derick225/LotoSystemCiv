...
import { SearchCode, RefreshCw, GitCompare, ArrowRight, Binary, ScanEye } from 'lucide-react';
                ...
                {matches.map((m, idx) => (
                    <div key={m.match.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden group hover:border-indigo-300 transition-colors">
                        ...
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${m.similarity > 75 ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-indigo-50 text-indigo-600 border-indigo-100'}`}>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-2 opacity-70 group-hover:opacity-100 transition-all">
                                    <span className="text-[8px] font-black text-slate-400 w-8">TIRAGE</span>
                                    <div className="flex gap-1">
                                        {m.match.gagnants.map(n => {
                                            const isShared = currentDraw.gagnants.includes(n);
                                            return (
                                                <div key={n} className={`w-6 h-6 rounded-md flex items-center justify-center text-[9px] font-black border ${isShared ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500'}`}>
                                                    {n}
                                                </div>
                                            );
                                        })}
