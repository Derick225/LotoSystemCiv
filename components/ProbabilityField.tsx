...
const GridCell = memo(({ num, score }: { num: number, score: number }) => {
    const getStatus = (s: number) => {
        if (s > 85) return { label: "SURCHAUFFE", color: "text-white", bg: "bg-rose-500", border: "border-rose-600" };
        if (s > 65) return { label: "ATTRACTEUR", color: "text-white", bg: "bg-indigo-600", border: "border-indigo-700" };
        if (s < 25) return { label: "ZONE MORTE", color: "text-slate-400", bg: "bg-slate-50", border: "border-slate-100" };
        return { label: "STABLE", color: "text-slate-600", bg: "bg-slate-100", border: "border-slate-200" };
    };
        ...
        <div className="space-y-4 md:space-y-8 animate-fade-in w-full overflow-hidden">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
                {[
                    ...
                    <div key={item.type} className="p-2 md:p-3 bg-white rounded-xl md:rounded-2xl border border-slate-200 flex items-center gap-2 md:gap-3 shadow-sm">
                        <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${item.color} shadow-sm shrink-0`}></div>
                        <div className="min-w-0 overflow-hidden">
                            <div className="text-[8px] md:text-[9px] font-black text-slate-800 uppercase truncate">{item.type}</div>
            ...
            <div className="bg-white p-2 sm:p-4 md:p-8 rounded-2xl md:rounded-[3.5rem] border border-slate-200 shadow-2xl relative overflow-hidden">
                <div className="flex justify-between items-center mb-4 md:mb-6">
                    <div className="flex items-center gap-2">
                        <Thermometer className="text-indigo-600" size={16} />
                        <h4 className="text-slate-800 font-black text-[10px] md:text-sm uppercase tracking-widest">Pression Thermique Matrix</h4>
                    </div>
                </div>
