    ...
    <div className="w-full space-y-6 animate-fade-in font-sans">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {/* COMPARAISON SYSTÉMATIQUE OBSERVATIONS VS PRÉDICTIONS */}
          {report.systematicComparison ? (
            <div id="closed-loop-systematic-comparison-section" className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  <GitCompare className="w-4 h-4 text-cyan-400" />
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-200">
                    Comparaison Systématique : Réel vs Prédit
                  </h4>
                </div>
                <span className="text-[10px] text-cyan-400 font-mono bg-cyan-950/60 px-2.5 py-0.5 rounded border border-cyan-800/40">
                  Dispersion Spatiale : {report.systematicComparison.dispersionIndex}/100
                </span>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-950/60 border border-slate-800/60 rounded-lg p-3 text-center">
                  <span className="text-[10px] text-slate-400 block font-semibold">Hits Directs Top 5</span>
                  <span className="text-lg font-black text-emerald-400">
                    {report.systematicComparison.directHits.length} / 5
                  </span>
                ...
                <div className="bg-slate-950/60 border border-slate-800/60 rounded-lg p-3 text-center">
                  <span className="text-[10px] text-slate-400 block font-semibold">Voisins Immédiats (±1)</span>
                  <span className="text-lg font-black text-amber-400">
                    {report.systematicComparison.neighbors1.length}
                  </span>
                  <span className="text-[9px] text-slate-500 block">
                    {report.systematicComparison.neighbors1.length > 0
                      ? report.systematicComparison.neighbors1.map(n => `${n.actual}→${n.predicted}`).join(', ')
                      : 'Aucun voisin ±1'}
                  </span>
                </div>

                <div className="bg-slate-950/60 border border-slate-800/60 rounded-lg p-3 text-center">
                  <span className="text-[10px] text-slate-400 block font-semibold">Résonances Miroirs & 91</span>
                  <span className="text-lg font-black text-purple-400">
                    {report.systematicComparison.mirrors.length}
                  </span>
                  <span className="text-[9px] text-slate-500 block">
                    {report.systematicComparison.mirrors.length > 0
                      ? report.systematicComparison.mirrors.map(m => `${m.actual}⇌${m.predicted}`).join(', ')
                      : 'Aucun miroir'}
                  </span>
                </div>

                <div className="bg-slate-950/60 border border-slate-800/60 rounded-lg p-3 text-center">
                  <span className="text-[10px] text-slate-400 block font-semibold">Taux Capture Étendu</span>
                  <span className="text-lg font-black text-cyan-400">
                    {(report.systematicComparison.captureRateExtended * 100).toFixed(1)}%
                  </span>
                  <span className="text-[9px] text-slate-500 block">
                    Hits + 0.5×V1 + 0.25×V2
                  </span>
              ...
              {report.systematicComparison.machineLeakages.length > 0 ? (
                <div className="bg-amber-950/20 border border-amber-800/40 rounded-lg p-3 flex items-center gap-3">
                  <AlertCircle className="w-4 h-4 text-amber-400 flex-shrink-0" />
                  <p className="text-xs text-amber-300/90">
                    <strong className="font-semibold text-amber-200">Fuite Machine Détectée :</strong> Les numéros{' '}
                    <span className="font-mono font-bold text-white">
                      {report.systematicComparison.machineLeakages.join(', ')}
                    </span>{' '}
                    sont apparus dans le tirage Machine et ont été attirés dans la prédiction.
                  </p>
                </div>
              ) : null}
            </div>
          ...
          {/* ATTRIBUTION CAUSALE AUTOMATISÉE */}
          {report.causalAttributions && report.causalAttributions.length > 0 ? (
            <div id="closed-loop-causal-attribution-section" className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  <Layers className="w-4 h-4 text-indigo-400" />
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-200">
                    Attribution Causale Automatisée des Écarts
                  </h4>
                </div>
                <span className="text-[10px] text-indigo-400 font-mono bg-indigo-950/60 px-2.5 py-0.5 rounded border border-indigo-800/40">
                  {report.causalAttributions.length} signaux attribués
                </span>
              </div>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {report.causalAttributions.map((attr, idx) => {
                  ...
                  return (
                    <div
                      key={`causal-attr-${attr.number}-${idx}`}
                      className="bg-slate-950/70 border border-slate-800/70 rounded-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs"
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-800/80 border border-slate-700/60 flex items-center justify-center font-mono font-black text-white text-sm flex-shrink-0">
                          {String(attr.number).padStart(2, '0')}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span className="text-[10px] text-slate-400 font-mono">
                              Attribution : {attr.attributionScore}%
                            </span>
                          </div>
                          <p className="text-slate-300 text-xs font-medium">{attr.primaryCause}</p>
                          <p className="text-[10px] text-slate-500 mt-1 font-mono">
                            Correctif Contrefactuel : <span className="text-slate-400">{attr.counterfactualFix}</span>
                          </p>
                        </div>
                      </div>

                      {attr.contributingAlgos.length > 0 ? (
                        <div className="flex items-center gap-1 flex-shrink-0 sm:text-right">
                          <span className="text-[10px] text-slate-400">Source clé :</span>
                          <span className="text-[10px] font-mono font-bold text-slate-200 bg-slate-800/80 px-2 py-0.5 rounded border border-slate-700/50">
                            {attr.contributingAlgos[0].algo} ({attr.contributingAlgos[0].contribution.toFixed(0)}%)
                          </span>
                        </div>
                      ) : null}
                    </div>
          ...
          {report.actionableImprovementReport ? (
            <div id="closed-loop-actionable-report-section" className="bg-gradient-to-br from-slate-900/90 to-emerald-950/20 border border-emerald-800/40 rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-emerald-800/40 pb-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-emerald-400 font-mono font-bold bg-emerald-950/80 px-2.5 py-1 rounded border border-emerald-700/40">
                    Gain Précision Estimé : +{report.actionableImprovementReport.expectedAccuracyGain}%
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {report.actionableImprovementReport.priorityFixes.map((fix, idx) => (
                    <div
                      key={`priority-fix-${idx}`}
                      className="bg-slate-950/60 border border-emerald-900/30 rounded-lg px-3 py-2 flex items-start gap-2.5 text-xs text-slate-300"
                    >
