...
export const TicketXRay: React.FC<TicketXRayProps> = ({ numbers, score = 0, showTitle = true }) => {
    ...
    return (
        <div className="mt-4 p-6 bg-slate-900 rounded-[2rem] border border-indigo-500/30 animate-slide-up relative overflow-hidden">
            {showTitle && (
                <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                    <h5 className="text-xs font-black text-indigo-400 uppercase tracking-widest">
                        Contrôle Qualité
                    </h5>
                    <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-2 border ${isOptimal ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-rose-500/10 text-rose-400 border-rose-500/30'}`}>
                        {isOptimal ? <ShieldCheck size={12}/> : <ShieldAlert size={12}/>}
                        {isOptimal ? 'Bon Ticket' : 'Ticket Fragile'}
                    </div>
                </div>
            )}
                ...
                {checks.map((check, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-white/5 p-3 rounded-xl">
                        <div className="flex items-center gap-3">
                            <div className={`p-1.5 rounded-full ${check.valid ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                                {check.valid ? <Check size={10} className="text-white"/> : <X size={10} className="text-white"/>}
                            </div>
                            <div>
                                <div className="text-[10px] text-slate-400 uppercase font-bold">{check.label}</div>
                                <div className="text-xs text-white font-medium">{check.text}</div>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="mt-6 pt-4 border-t border-white/10 text-center">
                <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Note Finale</span>
                <div className="text-4xl font-black text-white mt-1">{integrityScore}/100</div>
            </div>
        </div>
    );
};
