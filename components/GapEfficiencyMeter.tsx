...
export const GapEfficiencyMeter: React.FC<GapEfficiencyMeterProps> = ({
  data,
}) => {
  ...
  const activeData = useMemo(() => {
    return data
      .filter(
        (d) => d.zScore > 0.5 || (d.kaplanMeierProb && d.kaplanMeierProb > 50),
      ) // Filtre de bruit
  ...
  if (activeData.length === 0)
    return (
      <div className="p-6 text-center text-slate-500 bg-slate-900/50 rounded-3xl border border-slate-800 border-dashed backdrop-blur-sm">
        <ScanLine
          size={48}
          className="mx-auto mb-4 opacity-30 text-indigo-400 animate-pulse-slow"
        />
        <p className="text-xs font-black uppercase tracking-widest">
          Pression Atmosphérique Normale
        </p>
        <p className="text-[10px] mt-2 opacity-60">
          Aucune anomalie d'écart détectée (&gt; 0.5 σ)
        </p>
      </div>
    );
              ...
              <div className="flex items-center gap-4 xl:gap-6">
                ...
                <div className="flex flex-col items-center gap-2 shrink-0 w-12">
                  ...
                  <div className="flex flex-col gap-0.5 mt-1 items-center shrink-0 w-full">
                    {item.zScore > 0.5 && (
                      <span
                        className="text-[7px] font-extrabold tracking-wide text-indigo-400 bg-indigo-950/40 border border-indigo-500/20 px-1 rounded uppercase scale-90"
                        title="Inclus par anomalie de pression Z-score"
                      >
                        Z-score
                      </span>
                    )}
                    {(item.kaplanMeierProb ?? 0) > 50 && (
                      <span
                        className="text-[7px] font-extrabold tracking-wide text-emerald-400 bg-emerald-950/40 border border-emerald-500/20 px-1 rounded uppercase scale-90"
                        title="Inclus par probabilité de survie Kaplan-Meier"
                      >
                        KM Survie
                      </span>
                    )}
                ...
                <div className="flex-1 grid grid-cols-4 gap-2 lg:gap-4 shrink-0 grow">
                  ...
                  <div className="flex flex-col justify-center pl-2 border-l border-white/5 whitespace-nowrap">
                    <span
                      ...
                      title="Probabilité de rupture avant l'écart actuel selon Kaplan-Meier"
                    >
                      Rupture S(t)
                    </span>
                    <span
                      className={`text-base xl:text-lg font-black font-mono ${(item.kaplanMeierProb || 0) > 85 ? "text-amber-400" : "text-emerald-400"}`}
                    >
                  ...
                  <div className="flex flex-col justify-center pl-2 border-l border-white/5 whitespace-nowrap">
                    <span
                      className="text-[10px] font-bold text-slate-500 uppercase mb-0.5"
                      title="Score Z classique"
                    >
                      Pression σ
                    </span>
                    <div className="flex items-baseline gap-1">
                      <span
                        className={`text-sm xl:text-base font-black font-mono ${item.zScore > 2 ? "text-rose-400" : "text-white"}`}
                      >
                      ...
                      {item.zScore > 1.5 && (
                        <ArrowUp
                          size={8}
                          className="text-rose-500 animate-bounce hidden xl:block"
                        />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col justify-center pl-2 border-l border-white/5 whitespace-nowrap">
                    <span
                      className="text-[10px] font-bold text-slate-500 uppercase mb-0.5"
                      title="Écart Actuel / Écart Maximum"
                    >
                      Écart / Max
                    </span>
