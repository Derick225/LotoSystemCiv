...
import { X, Activity, TrendingUp, GitMerge, AlertCircle, Database, RefreshCw } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchHistoryData = async () => {
      ...
      if (typeof window !== 'undefined') {
        try {
          const localData = localStorage.getItem(`nexus_weights_history_${drawName}`);
          if (localData) {
            const parsed = JSON.parse(localData);
            if (Array.isArray(parsed)) {
              parsed.forEach((h: any, idx: number) => {
                entries.push({
                  timestamp: h.timestamp || (Date.now() - (parsed.length - idx) * 3600000),
          ...
          console.warn('[TrainingEvolutionDrawer] LocalStorage parse error:', e);
        }
      }
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-black text-white flex items-center gap-3">
                ...
                Évolution de l'Apprentissage &bull; <span className="text-emerald-400">{drawName}</span>
              </h2>
              <p className="text-slate-400 text-xs uppercase tracking-widest mt-2 font-medium flex items-center gap-2">
                <Database size={12} className="text-indigo-400" />
                Historique complet de calibration continue (RLHF &amp; Gradient Descent)
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          {isLoading ? (
            <div className="h-64 flex flex-col items-center justify-center text-slate-400 gap-3">
              ...
              <p className="text-xs font-bold uppercase tracking-widest">Chargement de l'historique d'apprentissage...</p>
            </div>
          ) : history.length === 0 ? (
            <div className="h-64 flex flex-col items-center justify-center text-slate-500 bg-slate-900/40 rounded-2xl border border-dashed border-slate-800">
              <AlertCircle size={36} className="mb-3 opacity-60 text-amber-500" />
              <p className="text-sm font-bold text-slate-300">Aucun historique d'entraînement pour ce tirage ({drawName}).</p>
              <p className="text-xs text-slate-500 mt-1">Lancez une session dans le Laboratoire Darwinien pour initialiser la télémétrie.</p>
            </div>
          ) : (
            <div className="grid lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-4">
                <div className="flex justify-between items-center">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Activity size={14} className="text-indigo-400" /> Distribution &amp; Mutation des Poids (%)
                  </h3>
                ...
                <div className="h-80 w-full bg-slate-900/60 rounded-2xl p-4 border border-slate-800">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 10, right: 20, left: -20, bottom: 5 }}>
                      ...
                      <Tooltip contentStyle={{ borderRadius: '12px', fontSize: '11px', backgroundColor: '#020617', color: '#fff', border: '1px solid #1e293b' }} />
              ...
              <div className="space-y-4">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <GitMerge size={14} className="text-emerald-400" /> Score de Fitness Global
                </h3>
                <div className="h-80 w-full bg-slate-900/60 rounded-2xl p-4 border border-slate-800 flex flex-col justify-between">
                  <ResponsiveContainer width="100%" height="80%">
                    <LineChart data={chartData} margin={{ top: 10, right: 20, left: -20, bottom: 5 }}>
                      ...
                      <Tooltip contentStyle={{ borderRadius: '12px', fontSize: '11px', backgroundColor: '#020617', color: '#fff', border: '1px solid #1e293b' }} />
                      <Line type="monotone" dataKey="score" name="Fitness Score" stroke="#10b981" strokeWidth={3} dot={{ r: 4, fill: '#10b981' }} />
                    </LineChart>
                  </ResponsiveContainer>
                  ...
                  <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-[10px] text-emerald-300 font-mono flex justify-between items-center">
                    ...
                    <span className="font-black text-sm text-emerald-400">
                      {history.length > 0 ? (history[history.length - 1].score || 0).toFixed(2) : '--'}
                    </span>
                  </div>
                </div>
              </div>
