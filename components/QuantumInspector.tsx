...
export const QuantumInspector: React.FC = () => {
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-2xl animate-fade-in"
      onClick={() => {
      ...
      >
          ...
          <div className="flex items-center gap-4 sm:gap-8 relative z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                />
                <span className="text-[9px] sm:text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] sm:tracking-[0.3em]">
                  Signature Vectorielle
                </span>
              </div>
              <h3 className="text-2xl sm:text-4xl md:text-5xl font-black text-white tracking-tighter leading-none">
                  ...
                  {[
                    {
                      ...
                      icon: <Thermometer size={16} className="text-rose-500" />,
                      sub: "Chaleur relative",
                    },
                    {
                      label: "Hurst Index",
                      val: metrics.hurst.toFixed(2),
                      icon: <Activity size={16} className="text-emerald-500" />,
                      sub: "Persistance",
                    },
                    {
                      label: "Écart Actuel",
                      val: `${metrics.lastGap}t`,
                      icon: <Radio size={16} className="text-amber-500" />,
                      sub: "Depuis sortie",
                    },
                    {
                      label: "Proba Sortie",
                      val: `${metrics.nextProb}%`,
                    ...
                    <div
                      key={i}
                      className="p-4 sm:p-6 bg-slate-800/50 rounded-2xl sm:rounded-[2rem] border border-slate-700/50 hover:bg-slate-800 transition-colors group"
                    >
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                  <div className="bg-slate-950 rounded-2xl border border-slate-800 p-6 relative overflow-hidden shadow-inner">
                    <div className="flex justify-between items-center mb-6 relative z-10">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${metrics.hurst > 0.5 ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-rose-500/10 text-rose-400 border-rose-500/30"}`}
                      >
                        {metrics.hurst > 0.5
                          ? "Signal Cohérent"
                          : "Signal Bruit"}
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-emerald-900/10 p-8 rounded-2xl border border-emerald-500/20 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-6 opacity-10">
                      <GitMerge size={80} className="text-emerald-500" />
                    </div>
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-center">
          <p className="text-[10px] text-slate-500 font-mono text-center max-w-lg">
            ANALYSE GÉNÉRÉE PAR NEXUS KERNEL V11.5. LES PROBABILITÉS SONT DES
            ESTIMATIONS STOCHASTIQUES ET NE GARANTISSENT PAS LE RÉSULTAT FUTUR.
          </p>
        </div>
      </div>
    </div>
