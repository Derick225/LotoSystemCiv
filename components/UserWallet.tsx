...
export const UserWallet: React.FC = () => {
  ...
  const { data: resultsMap = {} } = useQuery<Record<string, DrawResult[]>>({
    ...
    queryFn: async () => {
      ...
      await Promise.all(
        ticketDrawNames.map(async (name) => {
          if (name !== "Unknown") {
            try {
  ...
  const { totalWinnings, totalSpent, financialHistory } = React.useMemo(() => {
    ...
    sortedTickets.forEach((ticket) => {
      ...
      history.push({
        date: creationDate.toLocaleDateString("fr-FR").slice(0, 5),
  ...
  const handleSyncWallet = async () => {
    ...
    if (sess?.user) {
      try {
        ...
        console.warn(
          "[AlmostInstantSync] Erreur lors de la synchronisation manuelle :",
          err,
        );
        showToast(
          "Échec de synchronisation Cloud (temps d'attente dépassé ou réseau isolé).",
          "error",
        );
  ...
  const getTicketStatus = (ticket: SavedTicket) => {
    ...
    if (!history || history.length === 0)
      return { status: "pending", hits: 0, win: 0 };
  ...
  return (
    <div className="animate-fade-in space-y-6 md:space-y-8 pb-20 px-1 md:px-0">
      {subscription && (
        <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-5 md:p-6 rounded-[2rem] md:rounded-2xl shadow-xl border border-indigo-500/30 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <Crown size={80} className="md:w-[120px] md:h-[120px]" />
          </div>
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6">
            <div className="flex items-center gap-4 w-full md:w-auto">
              <div
                className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg ${subscription.plan === "premium" ? "bg-gradient-to-br from-amber-400 to-orange-500 text-white" : "bg-slate-700 text-slate-300"}`}
              >
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1">
                  <span
                    className={`text-[10px] md:text-[10px] font-black uppercase px-2 py-0.5 rounded border ${subscription.status === "active" ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-amber-500/20 text-amber-400 border-amber-500/30"}`}
                  >
                    {subscription.status === "active" ? "Actif" : "Essai"}
                  </span>
                  <span className="text-xs md:text-[10px] text-slate-400 font-bold whitespace-nowrap">
      ...
      <div className="bg-white dark:bg-slate-800 p-5 md:p-8 rounded-3xl md:rounded-3xl shadow-xl relative overflow-hidden group border border-slate-100 dark:border-slate-700">
        <div className="relative z-10 flex flex-col lg:grid lg:grid-cols-2 gap-6 md:gap-8">
          <div>
            <div className="flex justify-between items-start mb-4 md:mb-6">
              <div>
                <h2 className="text-xl md:text-3xl font-black tracking-tighter mb-1 md:mb-2 flex items-center gap-2 text-slate-900 dark:text-white">
                  <Briefcase className="text-emerald-500 w-5 h-5 md:w-6 md:h-6" />{" "}
                  Portefeuille
                </h2>
                <p className="text-slate-400 font-bold uppercase text-[10px] md:text-[10px] tracking-widest">
                  Bankroll Actuel
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-4">
              <div className="bg-slate-50 dark:bg-slate-900 p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-700">
                <div className="text-[10px] md:text-[10px] font-black text-slate-400 uppercase mb-0.5 md:mb-1">
                  Mises Total
                </div>
                <div className="text-sm md:text-xl font-black text-slate-700 dark:text-slate-200 whitespace-nowrap">
                  {totalSpent.toLocaleString()} F
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-900 p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-700">
                <div className="text-[10px] md:text-[10px] font-black text-emerald-500 uppercase mb-0.5 md:mb-1">
                  Gains Estimés
                </div>
                <div className="text-sm md:text-xl font-black text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                  {totalWinnings.toLocaleString()} F
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="h-28 md:h-40 w-full bg-slate-50 dark:bg-slate-900 rounded-xl md:rounded-2xl p-2 md:p-4 border border-slate-200 dark:border-slate-700 relative overflow-hidden">
            <div className="absolute top-2 left-4 text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest z-10">
              Performance
            </div>
      ...
      <div className="space-y-4">
        ...
        {ticketsLoading ? (
          <div className="p-8 text-center animate-pulse text-slate-400 font-black uppercase text-xs">
            Chargement...
          </div>
        ) : tickets.length === 0 ? (
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-3 md:gap-6">
                    <div className="flex-1 w-full md:w-auto">
                      <div className="flex justify-between items-start mb-2 md:mb-3">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] md:text-[10px] font-black uppercase text-white bg-slate-900 px-2 py-0.5 md:px-2.5 md:py-1 rounded-md md:rounded-lg">
                            {ticket.drawName}
                          </span>
                          <span className="text-[10px] md:text-[10px] text-slate-400 font-bold">
                        ...
                        <button
                          ...
                          className="text-slate-300 hover:text-rose-500 p-1 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/20"
                        >
                          <Trash2 size={12} />
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-2 md:pt-0 border-slate-50 dark:border-slate-700">
                      ...
                      <div
                        className={`p-1 rounded-full transition-all ${isExpanded ? "bg-indigo-100 text-indigo-600 rotate-180" : "bg-slate-100 text-slate-400"}`}
                      >
                        <ChevronDown size={12} />
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div
                      className="mt-4 border-t border-slate-50 dark:border-slate-700 pt-4 cursor-default animate-fade-in"
