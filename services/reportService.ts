...
import { DrawResult, Prediction, AlgoWeights, ForensicReport } from '../types';
...
export const generateTacticalReport = (data: ReportData) => {
    ...
    autoTable(doc, {
        startY: y,
        head: [['Numéro', 'Fréquence', 'Écart', 'Neural (LSTM)', 'Score Global']],
        body: suggestedData,
        theme: 'grid',
    ...
    const weightsData = [
        ...
        ['LSTM (Deep Learning)', `${(data.weights.lstm || 0) * 100}%`],
        ['Poisson', `${(data.weights.poisson || 0) * 100}%`]
    ];
