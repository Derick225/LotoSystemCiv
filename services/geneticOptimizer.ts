...
export const runGeneticOptimization = async (
  ...
  if (fullHistory.length < minHistoryRequired) {
    console.debug(`[Genetic] Sample size (${fullHistory.length}) below recommended ${minHistoryRequired}, using available history.`);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(new URL('./workers/genetic.worker.ts?worker', import.meta.url), { type: 'module' });

    const timeoutMs = 45000;
    const timeoutTimer = setTimeout(() => {
      worker.terminate();
      reject(new Error(`Genetic Worker Timeout (${timeoutMs}ms)`));
    }, timeoutMs);
