 ...
 */
export const analyzeIntraDraw = (result: DrawResult, calibration?: EmpiricalCalibration): IntraDrawMetrics => {
    const winners = [...result.gagnants].sort((a, b) => a - b);
    const machineSet = new Set(result.machine || []);
    const currentCalibration = calibration || FALLBACK_CALIBRATION;
    ...
    return {
        ...
        deviation: {
            sumDiff: parseFloat((sum - currentCalibration.meanSum).toFixed(1)), // Calibration empirique (was 227.5)
            acDiff: parseFloat((ac - currentCalibration.meanAC).toFixed(2)) // Calibration empirique (was 8)
        }
    };
};
