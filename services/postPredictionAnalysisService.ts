...
export const healForensicReport = (report: ForensicReport): ForensicReport => {
  ...
  // Reconstruct combo if missing or if combo was set to predictedNumbers erroneously
  const matches = Array.isArray(healed.matches) ? healed.matches : [];
  const predictedSet = new Set(matches.map((m: ForensicEvidence) => m.predicted).filter(Boolean));
  const actualFromMatches = Array.from(
    new Set(matches.map((m: ForensicEvidence) => m.actual).filter((a): a is number => typeof a === "number" && a > 0))
  );

  const isComboIdenticalToPredicted =
    healed.combo &&
    healed.combo.length === predictedSet.size &&
    healed.combo.length > 0 &&
...
export const performForensicAnalysis = async (
  ...
  if (predictionBreakdown && Object.keys(baseWeights).length > 0) {
    ...
    const algoKeys = Object.keys(baseWeights) as import("../shared/prediction.types").AlgoKey[];
    const optimizer = new DNAOptimizer(algoKeys);
    const numAlgos = optimizer.getNumAlgos();
    const dnaMatrix = actualWinningNumbers.map((num) => {
      const bdown = predictionBreakdown[num] || {};
      const vec = new Float32Array(numAlgos);
      algoKeys.forEach((k, i) => {
        vec[i] = (bdown as Record<string, number>)[k] || 0;
      });
      return vec;
    });
    ...
    for (let i = 0; i < dnaMatrix.length; i++) {
      for (let j = i + 1; j < dnaMatrix.length; j++) {
        diversitySum += optimizer.calculateCosineDistance(dnaMatrix[i], dnaMatrix[j]);
        pairs++;
      }
    }
  ...
  if (predictionBreakdown && Object.keys(baseWeights).length > 0) {
    ...
    keyAlgos.forEach(algo => {
      ...
      for (let i = 1; i <= 90; i++) {
        const bdMap = predictionBreakdown as Record<number, Record<string, number>>;
        const score = bdMap[i]?.[algo] || bdMap[i]?.[algo.toUpperCase()] || 0;
  ...
  return {
    ...
    combo: [...actualWinningNumbers].sort((a, b) => a - b),
    predictionId,
    drawResultId,
    matches,
  ...
  algos.forEach((algo) => {
    ...
    if (evalProjected.continuousTopologicalLoss < baseline.continuousTopologicalLoss || isResponsibleForMiss || isResponsibleForFP) {
        ...
        results.push({
            ...
            action: (actionMsg === "BOOST" || actionMsg === "REDUCE" ? actionMsg : "GRADIENT_STEP") as CounterfactualResult["action"],
            description: descMsg,
