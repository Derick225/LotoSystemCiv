import { EmpiricalCalibration, FALLBACK_CALIBRATION, AlgoKey } from "../shared/prediction.types";
...
export const runOrchestrationPipeline = (
  ...
  const rawW_trend = 1.0 + (((normalizedAlgoWeights[AlgoKey.GAPS] || 0.0) + (normalizedAlgoWeights[AlgoKey.FREQUENCY] || 0.0)) * 5.0);
  const rawW_seasonal = 1.0 + (normalizedAlgoWeights[AlgoKey.GAP_CADENCE] || normalizedAlgoWeights[AlgoKey.TEMPORAL] || 0.0) * 10;
  const rawW_structuralCoherence = 1.0 + (normalizedAlgoWeights[AlgoKey.DERIVED_NEIGHBOR] || 0.0) * 10;
...
const orchestrationScoresCache = new Map<string, Record<number, number>>();
const orchestrationAnalysisCache = new Map<string, OrchestrationMetrics & { candidatesDetails: Record<number, ScoreComposition> }>();
...
export const calculateOrchestrationScores = (
  ...
  const result = pipeline ? pipeline.scores : {};
  
  if (orchestrationScoresCache.size > 50) {
    const firstKey = orchestrationScoresCache.keys().next().value;
    if (firstKey) orchestrationScoresCache.delete(firstKey);
  }
  orchestrationScoresCache.set(cacheKey, result);
  return result;
};
...
export const getFullOrchestrationAnalysis = async (
  ...
  const cacheKey = `${drawName || 'default'}_${history.length}_${history[0]?.date || 'nodate'}_${(history[0]?.gagnants || []).join('-')}_${weights ? JSON.stringify(weights) : 'default'}`;
  const cached = orchestrationAnalysisCache.get(cacheKey);
  if (cached) return cached;
  ...
  if (orchestrationAnalysisCache.size > 30) {
    const firstKey = orchestrationAnalysisCache.keys().next().value;
    if (firstKey) orchestrationAnalysisCache.delete(firstKey);
  }
  orchestrationAnalysisCache.set(cacheKey, result);

  return result;
};

export const analyzePredictionError = (_drawName: string, actualResult: DrawResult, predictedNumbers?: number[]): { auditLessons: ImmediateLesson[] } => {
  const lessons: ImmediateLesson[] = [];
