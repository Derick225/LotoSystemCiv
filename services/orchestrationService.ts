...
export const adaptConfigurationToPhase = (history: DrawResult[]): OrchestrationConfig => {
  ...
  const repeatBoost = 1.0 + (repeatRatio > baselineRepeat ? (repeatRatio - baselineRepeat) / baselineRepeat : 0);
  ...
  return {
    ...
    neighborWeight: Math.min(1.5, 1.0 + (repeatRatio * 5.0)),
    thermoState
  };
};
  ...
  lastDrawWinners.forEach(leader => {
    ...
    if (leaderAppearancesWeighted > 0) {
      Object.entries(followersMap).forEach(([numStr, count]) => {
        ...
        // Seuil continu : on ne pénalise/booste que si l'efficacité s'écarte significativement de la baseline
        if (efficiency > baselineProb) {
          // Le score est proportionnel à l'excès de probabilité, normalisé
          const excessRatio = (efficiency - baselineProb) / baselineProb;
          scores[num] = (scores[num] || 0) + (excessRatio * (1.0 / baselineProb));
        }
      });
    }
  });
  ...
  if (predictedNumbers && predictedNumbers.length > 0) {
    ...
    if (hitRate < expectedHitRate) {
      lessons.push({
        ...
        description: 'Le tirage a rompu avec les tendances récentes (bruit statistique).',
        impactScore: Math.max(0, Math.round(impactScore)) 
      });
    } else if (hitRate >= (expectedHitRate * 3.0)) {
      lessons.push({
        pattern: 'Confirmation de Trend',
        description: `${hits.length} numéro(s) prédit(s) sorti(s). Le modèle a bien capté la tendance lourde.`,
        impactScore: Math.min(0, Math.round(impactScore)) // Impact négatif = bonne nouvelle pour l'audit
      });
    }  }
