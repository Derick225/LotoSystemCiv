...
export const getUserFriendlyError = (error: unknown): string => {
    try {
        ...
        if (typeof error === 'string') rawMessage = error;
            ...
            if ('message' in error) rawMessage = String((error as Record<string, unknown>).message);
            else if ('error_description' in error) rawMessage = String((error as Record<string, unknown>).error_description);
            else if ('code' in error) rawMessage = `Erreur Code: ${(error as Record<string, unknown>).code}`;
            else {
