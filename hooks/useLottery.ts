import { isFirebaseConfigured, db } from "../services/firebaseClient";
...
export const useDrawHistory = (drawName: string) => {
  ...
  useEffect(() => {
    ...
    const q = normalizedDrawName === 'ALL' 
        ? query(collection(db, 'draw_results'))
        : query(collection(db, 'draw_results'), where('draw_name', '==', normalizedDrawName));
...
export const useDrawMutation = (drawName: string) => {
  ...
  return useMutation({
    mutationFn: async (newResult: Omit<DrawResult, 'id'>) => {
      ...
      const docId = `${drawName.replace(/\s+/g, '_')}_${normalizeDate(newResult.date)}`;
...
export const useDeleteDrawMutation = (drawName: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      const docRef = doc(db, 'draw_results', id);
